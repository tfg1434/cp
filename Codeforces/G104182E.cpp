...
ll count_reachable(vector<int>& w_a) {
    int n = sz(w_a);
    string s;
    for (int i = 0; i < n-1; i++) {
        s += w_a[i] < w_a[i + 1] ? '<' : '>';
    ...
    vl dp(n+1), predp(n+1), sufdp(n+1);
    dp[1] = 1;
    partial_sum(all(dp), bg(predp), op);
    partial_sum(rall(dp), rbegin(sufdp), op);

    FOR(i, 2, n+1) {
        FOR(j, 1, i+1) {
            if (s[i-2] == '<') dp[j] = predp[j-1];
            if (s[i-2] == '>') dp[j] = sufdp[j];
        }
        // ps(dp);
        fill(all(predp), 0);
        fill(all(predp), 0);
...
void solve() {
    ...
    cout << count_reachable(w_a) << ' ' << ans << endl;
}
