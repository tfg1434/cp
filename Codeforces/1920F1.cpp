...
// Bounding box of island was my initial novel idea...
// Like when you are below the island, you can't move left
// Might work, but the problem is easier than that
void solve() {
    def(ll, n, m, q);
    V<str> ma(n);
    re(ma);
    
    V<vl> dist(n, vl(m, BIG));
    F0R(j, m) {
        ll last = -1;
        F0R(i, n) {
            if (ma[i][j] == 'v') last = i;
            if (last != -1) ckmin(dist[i][j], i-last);
        }
        last = -1;
        R0F(i, n) {
            if (ma[i][j] == 'v') last = i;
            if (last != -1) ckmin(dist[i][j], last-i);
        }
    }

    FOR(j, 1, m) F0R(i, n) ckmin(dist[i][j], 1ll+dist[i][j-1]);
    ROF(j, 0, m-1) F0R(i, n) ckmin(dist[i][j], 1ll+dist[i][j+1]);
    ...
    // }
    queue<pl> Q;
    F0R(i, n) F0R(j, m) if (ma[i][j] == '#') Q.push({i, j});

    auto in = [&](ll r, ll c) {
        return min(r, c)>=0 && r<n && c<m;
    };
    auto is_border = [&](ll r, ll c) {
    ...
    F0R(i, q) {
        ...
        ll k = 0; // each cell we sail has dist >= k
        for (ll stp = 18; stp >= 0; stp--) {
            V<vl> vis(n, vl(m));
            auto dfs = yy([&](auto rec, ll r, ll c) -> void {
                if (dist[r][c] < k+p2(stp)) return;
                vis[r][c] = 1;

                for (ll dr:{-1,0,1}) for (ll dc:{-1,0,1}) if (abs(dr)+abs(dc)==1) {
                    ll nr = r+dr, nc = c+dc;
                    if (in(nr, nc) && !vis[nr][nc] && dist[nr][nc] >= k+p2(stp) && ma[nr][nc] != '#') rec(nr, nc);
                }
            });
            dfs(R, C);
            ...
            while (sz(q)) {
                ...
                F0R(j, siz) {
                    auto [r,c] = q.ft; q.pop();            
                    // w/o this line asymptotic is horrible
                    if (VIS[r][c]) continue;
                    VIS[r][c] = 1;
                    // ps(VIS[r][c]);
                    // test(r, c);
                    ...
                    // for (ll dr:{-1,0,1}) for (ll dc:{-1, 0, 1}) if(check(r+dr, c+dc)) {
                    for (ll dr:{-1,0,1}) for (ll dc:{-1, 0, 1}) {
                        ll nr = r+dr, nc = c+dc;
                        if (in(nr, nc) && !VIS[nr][nc] && !vis[nr][nc]) 
                            q.push({nr, nc});
                    }
                }
            }
