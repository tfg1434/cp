...
bool cmp(const complex<ll>& a, const complex<ll>& b) {
...
void solve() {
    ...
    V<complex<ll>> pre(n+1);
    f1(i, n) {
        pre[i] = pre[i-1]+d[i];
    }

    multiset<complex<ll>, decltype(&cmp)> st(&cmp);
    complex<ll> o;
    vl ans(q);
    ...
    f0(i, B) {
        ...
        // l = 1?
        ll prv = 1;
        for (auto&[c, l, r, idx] : buk[i]) {
            FOR(j, prv, r+1) {
                st.ins(pre[j]-pre[prv-1]);
            }

            gg(c, l, r, idx);
            if (l < prv) {
                //adding new values to st
                //we adjust offset
                o += pre[prv-1]-pre[l-1];
                FOR(j, l, prv) {
                    st.ins(pre[j]-pre[l-1]-o);
                }

            } else if (prv < l) {
                //deleting elements from st
                FOR(j, prv, l) {
                    gg(j, prv-1);
                    st.erase(st.find(pre[j]-pre[prv-1]-o));
                }
                // we don't change o because it's stll valid
            }

            prv = l;

            auto x = pre[r]-c;
            ans[idx] = st.find(x) != st.end();
        }
    }
