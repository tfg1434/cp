...
void solve() {
    ll px, py, ax, ay, bx, by; re(px, py, ax, ay, bx, by);

    auto check = [&](double w) -> bool {
        bool con = dist(ax, ay, bx, by) < 2*w;
        ll touchO = 0;
        touchO += dist(ax, ay, 0, 0) < w;
        touchO += 2*(dist(bx, by, 0, 0) < w);
        ll touchP = 0;
        touchP += dist(ax, ay, px, py) < w;
        touchP += 2*(dist(bx, by, px, py) < w);
        // gg(w, touchO, touchP, con);

        return (touchO && touchP && (con || touchO == touchP || touchO == 3 || touchP == 3));
    };

    double w = 2e6;
    for (double b = 2e6; b > 1e-9; b /= 2) {
        while (check(w - b)) w -= b;
    }

    cout << fixed << setprecision(7) << w << endl;
}
