...
void solve() {
    ...
    vl cyc;
    // returns true if we found a cycle. false if it's a tree
    auto dfs = yy([&](auto rec, ll u, ll p) -> void {
        par[u] = p;
        lvl[u] = lvl[p]+1;
        vis[u] = 1;
        vl stk;
        each(v, g[u]) if (v != p && vis[v]) stk.pb(v);
        gg(u, stk);
        if (stk.empty()) {
            each(v, g[u]) if (!vis[v]) {
                rec(v, u);
        ...
        ll v = *max_element(all(stk), [&](ll x, ll y) -> bool {
            return lvl[x] < lvl[y];
        });
        ll x = u;
        for (; x != v; x = par[x]) {
