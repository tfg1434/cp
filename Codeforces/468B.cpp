...
void solve() {
    ...
    bool ok = true;
    F0R(i, n) {
        bool A = pos.count(a-p[i]), B = pos.count(b-p[i]);

        if (A) sat.add_clause_and(i, 0, pos[a-p[i]], 0);
        if (B) sat.add_clause_and(i, 0, pos[b-p[i]], 0);
        if (!A) sat.add_clause_or(i, 1, i, 1); // force it to be in B

        if (A) sat.add_clause_and(i, 1, pos[a-p[i]], 1);
        if (B) sat.add_clause_and(i, 1, pos[b-p[i]], 1);
        if (!B) {
            sat.add_clause_or(i, 0, i, 0); // force x not in B
            // gg(i);
        }
    ...
    if (ok && sat.satisfiable()) {
        ps("YES");
        // ps(sat.answer);
        F0R(i, n) {
            pr(sat.answer[i], ' ');
            // if (sat.answer[i]) pr(0, ' ');
            // else pr(1, ' ');
        }
        ps();
    } else {
