...
void solve() {
    ...
    vl pre_div(n+1), pre_sub(n+1), pre_nop(n+1), pre_both(n+1);
    partial_sum(all(a), bg(pre_nop)+1);
    {
        vl A = a; each(x, A) x = (x+1)/2;
        partial_sum(all(A), bg(pre_div)+1);
    }
    {
        vl A = a; each(x, A) x = max(0ll, x-b);
        partial_sum(all(A), bg(pre_sub)+1);
    }
    {
        vl A = a; each(x, A) x = max(0ll, (x+1)/2-b);
        partial_sum(all(A), bg(pre_both)+1);
    }

    ll cost = BIG;
    for (int p = 0; p <= k2; p++) for (int q = 0; p+q <= k1; q++) if (k1+k2-p <= n) {
        ll res = pre_both[p];
        res += pre_div[p+q]-pre_div[p];
        res += pre_sub[p+q+k2-p]-pre_sub[p+q];
        res += pre_div[p+q+k2-p+k1-p-q]-pre_div[p+q+k2-p];
        res += pre_nop[n]-pre_nop[k1+k2-p];
