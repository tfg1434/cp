...
void solve() {
    ll n; re(n);
    vl f(n+1); f1(i, n) re(f[i]);
    V<vl> g(n+1); f1(i, n) g[f[i]].pb(i);
    ...
    f1(i, n) if (!vis[i]) {
        ...
        vl cyc = {t};
        while (f[cyc.bk] != t) cyc.pb(f[cyc.bk]);
        each(u, cyc) vis[u] = 1;
        gg(cyc);
        if (sz(cyc) == 1) ok = false;
        ...
        each(u, cyc) {
            if (mark[u] == -1 && mark[f[u]] == -1) mark[f[u]] = 1;
        }
    }
    gg(mark);
    if (!ok) {
