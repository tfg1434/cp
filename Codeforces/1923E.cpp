...
// The idea is to compute how many points of each colour there are that
// have a path up to u where along the path the colour condition is 
// satisfied. Then we can find the answer by the transitions that we
// described
//
// It remains to track which nodes are of each colour. For each vertex,
// let's keep a map 
...
void dfs(ll u, ll p) {
    ...
    each(v, g[u]) if (v != p) {
        dfs(v, u);
        if (big == -1 || sz(m[v]) > sz(m[big])) big = v;
    }

    each(v, g[u]) if (v != p && v != big) {
        for (auto[c, f] : m[v]) {
            if (c != colour[u]) ans += m[big][c]*f;
