...
void solve() {
    ...
    for (auto[d, pairs] : qs) {
        ...
        map<ll, vl> val, pre, PRE;
        for (auto s : ss) {
            val[s].resize(n/d+3);
            pre[s] = PRE[s] = val[s];

            ll j = 1;
            for (ll i = s; i < n; i += d, j++) {
                val[s][j] = a[i];
            }
            partial_sum(all(val[s]), bg(pre[s]));
            partial_sum(all(pre[s]), bg(PRE[s]));
        }

        for (auto[tmp, i] : pairs) {
            auto [s, k] = tmp;

            k += s/d;
            ll x = k*pre[s%d][k]-PRE[s%d][k-1];
            if (s/d > 0) {
                x -= (s/d)*pre[s%d][k];
                x += PRE[s%d][s/d-1];
            }
