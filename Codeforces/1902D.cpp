...
using SET = set<complex<ll>, decltype(&cmp)>;
...
void solve() {
    ...
    complex<ll> o;
    SET gtr(&cmp);
    f0(i, n+1) gtr.ins(pre[i]);

    for (auto&[c, six, r, ans, _] : qs) {
        while (j <= r) {
            o += c;
            st.ins(d[j]-o);
    ...
    SET lt1(&cmp);
    vb ANS(q);
    for (auto&[c, l, r, ans, i] : qs) {
        while (j <= l) {
            lt1.ins(pre[j]); 
            j++;
        }
        ans |= lt1.find(c) != lt1.end();
