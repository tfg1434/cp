...
void solve() {
    ...
    fill(all(dp[0]), 1);
    F1R(i, n) F1R(j, k) {
        F1R(p, min(i/j, k-j+1)) (dp[i][j] += dp[i-j*p][p]) %= P;
    }

    ll ans = 0;
    F1R(j, k) (ans += dp[n][j]) %= P;

    // vl p(n+1);
    // F1R(i, n) {
        // for (ll a1 = 1; a1*a1 <= i; a1++) if (i%a1 == 0){
            // ll b1 = i/a1;
            // if (a1+b1-1 <= k) p[i]++;
        // }
    // }
    ...
    // // initial wrong dp... I tried to fix the value of the last elem but it doesn't work
    // // because I overlooked that two pairs share the previous value. Need to fix the 
    // // last value as well.
    // FOR(i, 2, n+1) {
        // vl DP(n+1);

        // F1R(j, n) {
            // F1R(k, j) (DP[j] += p[k]*dp[j-k]%P);
        // }
