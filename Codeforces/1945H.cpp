...
// remove these bits so that the bitwise AND is 0
pl zero() {
    V<vl> has_bit(LOGN);
    F0R(i, n) {
        F0R(j, LOGN) {
...
void solve() {
    ...
    a = vl(n); re(a);
    globsum = vl(LOGN);
    V<vl> where(*max_element(all(a))+1);
    F0R(i, n) {
        where[a[i]].pb(i);
    }
    ...
    // N * d * LOGN
    F0R(i, n) {
        for (auto d : divs[a[i]]) if (sz(where[d])) {
            ll pos = where[d][0];
            if (pos == i) {
                if (sz(where[d]) < 2) continue;
                pos = where[d][1];
            }

            // calculate the AND where i and pos are missing
            auto cpy = globsum;
            F0R(j, LOGN) {
                cpy[j] -= (a[i] & (1<<j));
                cpy[j] -= (a[pos] & (1<<j));
