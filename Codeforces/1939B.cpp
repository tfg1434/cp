// it gets RTE on just the last test of each group? 
...
ll calc_greedy(ll u, ll p) {
    map<ll, ll> ret;
    vpl ls;
    for (auto[w, v] : g[u]) if (v != p) {
        ll x = calc_greedy(v, u);
        if (x == -1) return -1;
        ls.pb({ w-x, v });
    }
    sor(ls);
    ...
    if (u == p) {
        for (ll i = 0; i < sz(ls); i++) {
            sons[u][stickers[u][i]].pb(ls[i].s);
        }

        bool ok = 1;
        for (ll idx = 0;idx<sz(ls); idx++) {
            if (stickers[u][idx] < ls[idx].f) ok = 0;
        }
        return ok;
    }

    assert(sz(stickers[u]) == sz(ls)+1);
    F0R(i, sz(ls)) {
        if (stickers[u][i+1] < ls[i].f) return -1;
    }

    ll idx = 0;
    for (;idx<sz(ls); idx++) {
        if (stickers[u][idx] < ls[idx].f) break;
    }

    {
        ...
        for (ll i = idx; i < sz(ls); i++) {
            sons[u][stickers[u][i+1]].pb(ls[i].s);
        }
    }

    return ret_to_par[u] = stickers[u][idx];
}

// ret_to_par -- done
// which -- done
// sons[val] = which children i assigned weights of this val to
void global_order(ll u, ll p) {
    bool par = 0;

    F0R(i, sz(stickers_order[u])) {
        ll val = stickers_order[u][i];
        if (val == ret_to_par[u] && !par)  {
            par = 1;
            ans.pb(which[{u, p}]);
            continue;
        }

        ll v = sons[u][val].bk; sons[u][val].pop_back();
        global_order(v, u);
    }
}
...
void solve() {
    ...
    memset(ret_to_par, -1, sizeof ret_to_par);

    F0R(i, n-1) {
        def(ll, u, v, w); u--; v--;
        g[u].pb({w, v});
        g[v].pb({w, u});
        which[{u, v}] = which[{v, u}] = i;
    }
    F0R(i, n) {
        F0R(j, sz(g[i])) {
            def(ll, x);
            stickers[i].pb(x);
            stickers_order[i].pb(x);
        }
        sor(stickers[i]);
    }

    bool exists = calc_greedy(0, 0);
    ps(exists ? "Yes" : "No");

    if (exists) {
        global_order(0, 0);
        reverse(all(ans));
        F0R(i, sz(ans)) pr(ans[i]+1, " \n"[i==n-2]);
    }
}
