...
void solve() {
    ...
    auto dfs = yy([&](auto rec, ll u, ll p) -> void {
        ...
        while (sz(stk) > 1) {
            ps(stk.bk, u, end(stk)[-2]); 
            stk.pop_back(); stk.pop_back();
        }

        assert(sz(stk) <= 1);
        if (sz(stk)) be[u] = stk.bk;
        if (!deg[u]) {
            if (be[u]) q.pb(u);
            else q.push_front(u);
        } 
    ...
    f1(i, n) if (!vis[i]) {
        ...
        gg(q, be);
        while (!q.empty()) {
            ll u = q.back(); q.pop_back();
            ll v = par[u];
            if (par[u] == u || par[v] == v) continue;
            if (be[u]) {
                deg[v]--;
                if (isLeaf(v)) {
                    if (be[v]) q.pb(v);
                ...
                deg[par[v]]--;
                if (isLeaf(par[v])) {
                    if (be[par[v]]) q.pb(par[v]);
                    else q.push_front(par[v]);
                }
                ps(u, v, par[v]);
            } else {
                assert(be[v]);
