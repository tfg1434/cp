...
void solve() {
    ...
    for (ll sum = 0; sum < k; sum++) {
        vl bad(k);
        for (ll i = 0; i < k; i++) bad[i] = 2*i%k == sum;
        Matrix trans(k, k);
        for (ll i = 0; i < k; i++) for (ll j = 0; j < k; j++) 
            trans.mat[i][j] = !bad[(j-i+k)%k];
        Matrix init(k, 1);
        init.mat[0][0] = 1;
        ll p = n;
        while (p > 0) {
            if (p & 1) init = trans*init;
            p >>= 1;
            trans *= trans;
        }

        complement = (complement + init.mat[sum][0]) % mod;
    }

    ps((pw(k, n, mod)-complement+mod)%mod);
}
