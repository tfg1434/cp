...
void solve() {
    ...
    FOR(i,1, n) {
        F0R(x, k+1) {
            ll H = hot[a[i+1]], C = cold[a[i+1]];
            // run a[i+1] on the same cpu as a[i]
            ckmin(dp[(i+1)%2][x], dp[i%2][x]+(a[i+1]==a[i]?H:C));
            // run a[i+1] on the same cpu as x
            ckmin(dp[(i+1)%2][a[i]], dp[i%2][x]+(x==a[i+1]?H:C));
        }
