...
bool solve() {
    ...
    vl nxt(n+1), prv(n+1);
    F1R(i, n) nxt[i] = i+1, prv[i] = i-1;
    // nxt[n] = n;
    // prv[1] = 1;

    auto good = [&](ll i) {
        return (nxt[i] <= n && a[nxt[i]]==a[i]-1) || (prv[i] > 0 && a[prv[i]]==a[i]-1);
    };
    ...
    while (sz(st)) {
        ...
        cnt++;
        st.erase(*st.rbegin());
        if (prv[i] > 0 && good(prv[i])) st.ins({a[prv[i]], prv[i]});
        if (nxt[i] <= n && good(nxt[i])) st.ins({a[nxt[i]], nxt[i]});
        // gg(st);
    }
