...
void solve() {
    ll n, m, k; re(n, m, k);
    vl h(n+1); f1(i, n) re(h[i]);
    vl dp(n+1, -1), indeg(n+1);
    V<vl> g(n+1);
    f0(i, m) {
        ll u, v; re(u, v);
        g[u].pb(v); 
    ...
    set<pl> st;
    // initialize st with the imaginary value
    // greedily assign the times for each cc
    auto calc = [&](ll t) {
        vl deg = indeg;
        f1(i, n) if (!indeg[i]) {
            st.insert({ h[i], i });
        ...
        while (!st.empty()) {
            ...
            each(v, g[u]) {
                deg[v]--;
                if (!deg[v]) st.insert({ h[v], v });
            }
        }

        f1(i, n) {
            st.insert({ dp[i]*k + h[i], i });
        }
    };
    auto dfs = yy([&](auto rec, ll u) -> void {
        //u completed before v
        each(v, g[u]) {
            ll d = dp[u];
            if (h[v] < h[u]) d++;
            if (dp[v] < d) {
                st.erase({ dp[v]*k + h[v], v });
                dp[v] = d;
                st.insert({ dp[v]*k + h[v], v });
                rec(v);
            }
        }
    });
