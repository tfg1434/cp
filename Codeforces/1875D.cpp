...
void solve() {
    ...
    vl dp(n+1, BIG); 
    dp[0] = 0;
    if (!b[0].f) dp[0] = b[0].s - 1;
    auto f = yy([&](auto rec, ll idx) -> ll {
        if (dp[idx] != BIG) return dp[idx];
        //defined as b[idx] is one less than MEX

        f0(i, idx+1) {
            ll x;
            if (i == mexIdx) x = (b[idx].f+1) * (b[i].s-1);
            else x = (b[idx].f+1) * (b[i].s);
            if (i > 0) x += rec(i-1);
            ckmin(dp[idx], x); 
        }
    ...
    // f0(i, n) {
        // if (a[i] >= mex) {
            // flag=true;
        // }
        // if (!flag) pre[i+1] = pre[i]+a[i]+1;
        // else pre[i+1] = pre[i];
    // }

    // ll ans = BIG;
    // f0(i, n) {
        // //doesn't make sense to delete things >mex
        // //also breaks our calculation
        // if (a[i] >= mex) break;
        // ll cnt = 1;
        // ll j = i;
