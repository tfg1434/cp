...
void solve() {
    ...
    ll kh = (k+1)/2;
    ll ans = 0;
    V<V<vl>> where(2, V<vl>(N));
    F1R(i, n) where[i%2][a[i]].pb(i);
    F1R(i, n) {
        // where kh is k/2
        // ll lo = 2*kh-i;
        // ll hi = n+n-2-2*kh-i;
        ll lo = 2*kh-i;
        ll hi = 2*n-2*kh+2-i;
        ckmax(lo, 1ll); ckmax(hi, 1ll);
        ckmin(lo, i-1); ckmin(hi, i-1);
        auto p = lwb(where[i%2][a[i]], lo);
        auto P = upb(where[i%2][a[i]], hi);
        assert(P>=p);
        ans += P-p;
    }
