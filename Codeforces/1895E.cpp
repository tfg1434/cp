...
void solve() {
    ...
    vl ay(n); re(ay);
    vpl A(n), a;
    F0R(i, n) A[i] = {ax[i], ay[i]};
    sort(rall(A));
    F0R(i, n) if (!sz(a) || (a.bk.f != A[i].f && A[i].s > a.bk.s)) a.eb(A[i].f, A[i].s);
    n = sz(a);
    // ps(a);
    ...
    vl by(m); re(by);
    vpl B(m), b;
    F0R(i, m) B[i] = {bx[i], by[i]};
    sort(rall(B));
    F0R(i, m) if (!sz(b) || (b.bk.f != B[i].f && B[i].s > b.bk.s)) b.eb(B[i].f, B[i].s);
    m = sz(b);

    // ps(a);
    // ps(b);

    V<vl> vis(2, vl(max(n, m)));
    auto dfs = yy([&](auto rec, ll side, ll u) -> void {
        vis[side][u] = 1;

        ll v = lstTrue(0ll, (!side?n:m)-1, [&](ll j) {
            return (!side?a:b)[j].f > (side?a:b)[u].s;
        });
        ...
        if (!vis[1-side][v]) rec(1-side, v);
        vis[side][u] = vis[1-side][v];
    });
    F0R(i, m) dfs(0, i);
    ...
    ll mc = 0, bc = 0, draw = 0;
    for (auto [HP, CNT] : freq) {
        ll hp = HP, cnt = CNT;
        ll idx = lstTrue(0ll, m-1, [&](ll j) {
            return b[j].f > hp;
        });
        if (idx < 0) {
            mc += cnt;
        ...
        // ps(n, m, idx);
        if (vis[0][idx] == 1) draw += cnt;
        if (vis[0][idx] == 2) bc += cnt;
        if (vis[0][idx] == 3) mc += cnt;
    }
