// I hereby declare that I will never again miss a filtering problem during any contest
//
...
void solve() {
    ...
    if (k < n) {
        multiset<int> st;
        int sum_a = 0;
        for (int i = 0; i < k; i++) sum_a += pairs[i].s, st.insert(pairs[i].s);
        int sum_profit = 0;
        for (int i = k; i < n; i++) sum_profit += max(0ll, pairs[i].f-pairs[i].s);

        if (k == 0) {
            ckmax(ans, sum_profit);
        } else {
            for (int i = k; i < n; i++) {
                ckmax(ans, sum_profit - sum_a);
                if (pairs[i].s < *rbegin(st)) {
                    sum_a -= *rbegin(st);
                    st.erase(prev(end(st)));
                    st.insert(pairs[i].s);
                    sum_a += pairs[i].s;
                }
                if (pairs[i].f - pairs[i].s > 0)
                    sum_profit -= pairs[i].f - pairs[i].s;
            }
        }
    }
