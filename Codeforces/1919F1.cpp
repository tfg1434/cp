...
void solve() {
    vl a(n+1), b(n+1); F1R(i, n) re(a[i]); F1R(i, n) re(b[i]);
    vl v(n+1); F1R(i, n) v[i] = a[i]-b[i];
    vl trash(n-1); re(trash);

    vl suf(n+1);
    partial_sum(rall(v), rbegin(suf));
    SegmentTree st; st.init(n); st.build(1, 1, n, suf);
    ...
    rep(q) {
        ...
        ll delta = -v[idx]+(A-B);
        v[idx] += delta;
        assert(v[idx] == a[idx]-b[idx]);
        st.update(1, 1, n, 1, idx, delta);
