...
void solve() {
    ...
    f0(i, n) {
        if (a[i] == 0 && i != lo) {
            if (a[lo] == 0 && !two) {
                //we need to exclude right endpoint
                lo = i;
            } else {
                lo = i+1;
            }
...
// Thinking
// It took me a while to realize that the problem is not super easy,
// I made the false assumption that we always colour the first elem
// Shows how things collapse when you fix something!
// Interesting proof in editorial how "each subsegment is good" leads
// to greedily expanding. Also to prove it they did cases
// whether the owner is in the subsegment
signed main() {
    setIO();
