...
void solve() {
    ...
    while (i <= n) {
        ll I = i;
        while (i <= n && (I == i || a[i] > a[i-1])) i++;
        if (i-I > 1) segments.eb(I, i);
    }

    vl pre(mx+2);
    for (auto [l, r] : segments) {
        ll x = a[l], y = a[r-1];
        vl events; // interesting values of k
        for (ll i = 1; i <= ceil(sqrt(y)); i++) events.pb(i);
        for (ll i = sz(events)-1; i >= 0; i--) {
            ll x = (y+events[i]-1) / events[i];
            if (x > events.bk) events.pb(x);
        }
        ps(x, y, events);
        assert(is_sorted(all(events)));
        assert(events.bk == y);
        events.pb(mx+1);

        for (ll i = 0; i < sz(events)-1; i++) {
            // considering k in [events[i], events[i+1])
            ll dif = (y+events[i]-1)/events[i] - (x+events[i]-1)/events[i];
            pre[events[i]] += dif;
            pre[events[i+1]] -= dif;
        }
    }

    for (ll k = 1; k <= mx; k++) pre[k] += pre[k-1];
    for (ll k = 1; k <= mx; k++) pr(pre[k], " \n"[k==mx]);
}
