...
vl masks[2];
void precalc() {
    F0R(i, p2(13)) if (pct(i) <= 4) 
        masks[0].pb(i);
    F0R(i, p2(12)) if (pct(i) <= 4) 
        masks[1].pb(i);
...
void solve() {
    ...
    V<str> tmp(7); re(tmp);
    V<vl> ma(7, vl(7));
    F0R(i, 7) F0R(j, 7) ma[i][j] = tmp[i][j] == 'W' ? 0 : 1;

    ll ans[2]; ans[0] = ans[1] = 4;
    F0R(R, 2) {
        for (ll m : masks[R]) {
            bool ok = true;

            {
                ...
                F0R(j, 13-R) {
                    auto [r, c] = d[R][j];
                    ok &= !(cpy[r][c] && cpy[r-1][c-1] && cpy[r+1][c-1] && cpy[r-1][c+1] && cpy[r+1][c+1]);
                }
            }

            if (ok) ckmin(ans[R], pct(m));
        }
    }
