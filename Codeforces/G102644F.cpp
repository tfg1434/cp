...
struct Matrix {
    ...
    Matrix<T> operator *= (const Matrix<T>& m) {
        ...
        cols = m.cols;
        vector<vector<T>> nmat(rows, vector<T>(cols, BIG));
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
                for (int k = 1; k < m.rows; ++k) {
                    // T t = (mat[i][k] * m.mat[k][j]) % P;
                    // nmat[i][j] = (nmat[i][j] + t) % P;
                    ckmin(nmat[i][j], mat[i][k]+m.mat[k][j]);
                }
            }
        }
        mat = nmat;
...
void solve(ll n, ll m, ll k) {
    ...
    gg(a(1, 2));
    ll ans = BIG;
    F1R(i, n) F1R(j, n) ckmin(ans, a(i, j));
    ps(ans > 1e18 ? "IMPOSSIBLE" : to_string(ans));
}
