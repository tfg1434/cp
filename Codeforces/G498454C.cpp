...
void solve() {
    ...
    ll MAX_IDX = (n+4)*(m+2);
    auto C = [&](ll r, ll c) { return r*(m+2)+c + 3*m; };
    auto in = [&](ll r, ll c) { return min(r,c) >= 0 && r<n && c<m; };
    V<vl> g(MAX_IDX);

    F0R(i, n) F0R(j, m) if (ma[i][j] == 'o') {
        g[C(i-1, j)].pb(C(i+1, j)); g[C(i+1, j)].pb(C(i-1, j)); 
        g[C(i, j-1)].pb(C(i, j+1)); g[C(i, j+1)].pb(C(i, j-1));
    }
    
    // I SPENT SO MUCH TIME DEBUGGING MY INDICES
    // FIRST N INSTEAD OF M, THEN NOT BIG ENOUGH M... bro
    // simplifying the code led me to find the bug
    vb occ(MAX_IDX);
    FOR(i, -1, n+1) FOR(j, -1, m+1) if (!in(i, j) || ma[i][j] != '.') occ[C(i, j)] = true;
    // FOR(i,-1, n+1) {FOR(j,-1, m+1) pr(C(i, j), ' ');ps(); }
    // F0R(i, n) {
        // F0R(j, m) pr(occ[C(i, j)], ' '); ps();
    // }

    ll id = 0;
    ll nodes = 0, edges = 0, occupied = 0;
    vl vis(MAX_IDX);
    ...
    ll ans = 1;
    F0R(i, MAX_IDX) if (!vis[i]) {
        nodes = 0, edges = 0, occupied = 0;
        id++;
        dfs(i);
        ...
        if (nodes == 1) continue;
        if (nodes-1 == edges) {
            if (occupied == 0) (ans *= nodes) %= P;
            if (occupied >= 2) ans = 0;
        }
        if (nodes == edges) {
            if (occupied == 0) (ans *= 2) %= P;
            if (occupied) ans = 0;
        }
        if (edges > nodes) ans = 0;

        // ps(nodes, edges, occupied);
    }
