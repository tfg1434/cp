...
void solve() {
    ...
    f0(i, n) re(c[i].f);
    sor(c);
    vl preCost(n+1), preSz(n+1), preProd(n+1);
    f0(i, n) preCost[i+1] = preCost[i] + c[i].f;
    f0(i, n) preSz[i+1] = preSz[i] + c[i].s;
    f0(i, n) preProd[i+1] = preProd[i] + c[i].s * c[i].f;

    ll ans = BIG;
    f0(i, n) {
        ll res = (i+1)*p;
        ll pos = upb(preSz, n-i-1);
        res += preProd[pos-1];
        res += c[pos-1].f * (n-i-1 - preSz[pos-1]);
        ckmin(ans, res);
    }
