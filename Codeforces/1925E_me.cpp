...
ap zero = {0, 0};
ap operator += (ap& a, const ap& b) {
    a.a += b.a;
    a.d += b.d;
    return a;
};
ll eval(ap a, ll terms) {
    return a.a*terms + terms*(terms-1)/2 * a.d;
}
...
struct ST {
    ...
    void build(int u, int l, int r, vl& a) {
        if(l==r) {
            t[u] = a[l];
            lz[u] = zero;
            return;
    ...
    void update(int u, int l, int r, int x, int y, ap val) {
        // if(lz[u].a != 0 || lz[u].d != 0)
        lazy(u, l, r);
        ...
        int mid=(l+r)/2;
        update(u*2+1, mid+1, r, x, y, val);
        if(y>mid) {
            int rig = (min(y, r)-mid);
            val.a += (rig*val.d);
        }
        update(u*2, l, mid, x, y, val);
        t[u] = t[u*2] + t[u*2+1];
        return;
    }
    ll query(int u, int l, int r, int x, int y) {
        if(lz[u].a != 0 || lz[u].d != 0) lazy(u, l, r);
    ...
    // void lazy(ll u, ll l, ll r) {
        ...
        // lz[u] = zero;
    // }
    // void update(ll u, ll l, ll r, ll lq, ll rq, ap val) {
        // lazy(u, l, r);
        // if (r < lq || rq > l || r < l) return;
        ...
        // ll m = (l+r)/2;
        // update(2*u+1, m+1, r, lq, rq, val);
        // if (rq > m) {
            // ll rig = min(rq, r)-m;
            // val.a += val.d*rig;
        // }

        // update(2*u, l, m, lq, rq, val);
        // t[u] = t[2*u]+t[2*u+1];
    // }
    // ll query(ll u, ll l, ll r, ll lq, ll rq) {
        // lazy(u, l, r);
        ...
        // ll m = (l+r)/2;
        // return query(2*u, l, m, lq, rq)+query(2*u+1, m+1, r, lq, rq);
    // }
};
...
void solve() {
    ...
    // st.build(1, 0, n-1, a);
    F0R(i, n) {
        st.update(1, 0, n-1, i, i, {a[i], 0});
    }

    rep(q) {
        def(ll, t, a, b); a--, b--;
        if (t == 1) {
            st.update(1, 0, n-1, a, b, {b-a+1, -1});
        } else {
            ps(st.query(1, 0, n-1, a, b));
        }
    }
}
