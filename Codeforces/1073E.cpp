...
ll l, r, k;
ll pd[10][10][3], PD[10][10][3];
ll dp[10][10][3], DP[10][10][3];
vl p10{1};
...
// Big mistake - I thought you could just track last digit but
// you have to make a mask to keep track of which digits uve seen
ll solve(ll n) {
    ...
    ll x = n%10; n /= 10;
    F0R(i, 10) {
        PD[i][i!=0][i==x?1:i>x?2:0] = 1;
        DP[i][i!=0][i==x?1:i>x?2:0] = i;
    }

    // pd(last digit, number distinct, cmp)
    // it's not the number of values but the sum of numbers
    FOR(i, 1, m) {

        ll b = n%10; n /= 10;
        F0R(d, 10) FOR(c, 1, k+1) {
            // auto upd = [&](ll d, ll c, ll cmp) {
                // pd[d][c][cmp] += cmp==-1 ? get(PD[d][c]) : PD[d][c][cmp];
                // dp[d][c][cmp] += pd[d][c][cmp]*p10[i]*d + cmp==-1 ? get(DP[d][c]) : DP[d][c][cmp];
            // };
            ll off = p10[i]*d;

            // keep the same digit
            if (d == b) F0R(cmp, 3) {
                pd[d][c][cmp] += PD[d][c][cmp];
                dp[d][c][cmp] += pd[d][c][cmp]*off + DP[d][c][cmp];
            } else {
                pd[d][c][d>b?2:0] += get(PD[d][c]);
                dp[d][c][d>b?2:0] += pd[d][c][d>b?2:0]*off + get(DP[d][c]);
            }

            // introduce a new digit
            // d == 0 is special
            F0R(D, 10) if (d != D) {
                if (!d) {
                    if (d == b) F0R(cmp, 3) {
                        pd[d][c][cmp] += PD[D][c][cmp];
                        dp[d][c][cmp] += DP[d][c][cmp];
                    } else {
                        pd[d][c][0] += get(PD[D][c]);
                        dp[d][c][0] += get(DP[d][c]);
                    }
                    ...
                    if (d == b) F0R(cmp, 3) if (c-1-!D>0) {
                        pd[d][c][cmp] += PD[D][c-1-!D][cmp];
                        dp[d][c][cmp] += pd[d][c][cmp]*off + DP[d][c-1-!D][cmp];
                    } else pd[d][c][d>b ? 2 : 0] += get(PD[D][c-1-!D]);
                }
            }
        }
    ...
    ll ans = 0;
    F0R(d, 10) FOR(c, 1, k+1) ans += PD[d][c][0]+PD[d][c][1];
    return ans;
}
...
signed main() {
    ...
    F0R(i, 18) p10.pb(p10.bk*10);
    re(l, r, k);
    ps(solve(r)-solve(l-1));
