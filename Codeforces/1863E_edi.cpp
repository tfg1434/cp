...
const int N = 2e5, INF = 1e9 + 100, mod = 998244353;

vector<int> g[N + 1];
int k;
int DEG[N + 1], h[N + 1], dp[N + 1];
int mnn = LLONG_MAX, mxx = 0;
set<pii> st;

void calc(int n, int START) {
    vector<int> deg(n + 1);
    for(int i = 1; i <= n; i++) {
        if(!DEG[i]) {
            st.insert({h[i], i});
    ...
    int time = START, day = 0;
    while(!st.empty()) {
        set<pii> :: iterator it = st.lower_bound({time, 0});
        if(it == st.end()) {
            day++;
            it = st.begin();
        }
...
void dfs(int v) {
    for(int u : g[v]) {
        ...
        if(dp[u] < days) {
            st.erase({dp[u] * k + h[u], u});
            dp[u] = days;
            st.insert({dp[u] * k + h[u], u});
            dfs(u);
        }
    }
}
