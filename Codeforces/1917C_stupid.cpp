...
node* build(ll tl, ll tr, vl& a) {
    if (tl == tr - 1) return new node(a[tl]);
    else {
        ll tm = (tl + tr) / 2;
        return new node(build(tl, tm, a), build(tm, tr, a));
    }
};
...
void solve() {
    ...
    vl b(k+1); F1R(i, k) re(b[i]);
    vl net(n+1), lo(n+1), hi(n+1), cnt(n+1), need(n+1);
    F1R(i, k) {
        net[1]++;
        if (b[i] < n) net[b[i]+1]--;
    }
    F1R(i, n) net[i] += net[i-1];

    F1R(i, n) {
        need[i] = (i-a[i])%net[i];
    }
    
    auto tr = build(1, k+1, b);
    F1R(i, n) if (i >= a[i]){ // i is the value i'm calculating lo and i for
        assert(need[i] >= 0);
        ll idx = 1;
        ll tmp = need[i];
        while (tmp) {
            ll p = k;
            for (ll b = k; b > 0; b /= 2) {
                while (p-b >= idx && query(tr, 1, n+1, idx, p-b+1) >= i) {
                    p -= b;
                }
            }
            tmp--;
