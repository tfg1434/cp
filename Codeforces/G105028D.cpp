...
constexpr ll base = 9973;
 
struct chash {
    size_t operator()(const pl& p) const {
        return (p.f*base%mod + p.s) % mod;
    }
};
...
unordered_set<pl, chash> del; // hashing is unnecessary but whatever
 
unordered_map<ll, ll> sub;
void calc(ll u, ll p) {
    vis[u] = 1;
    sub[u] = a[u];
    each(v, g[u]) if (v != p && !del.count({ v, u })) {
        calc(v, u);
        sub[u] += sub[v];
    }
}
...
ll glob;
// is it possible to split the tree that contains u
ll split(ll u, ll p) {
    if (p == -1) {
        // sub = vl(n); THIS CAUSED ME SO MUCH GRIEF
        sub = unordered_map<ll, ll>();
        calc(u, p);
        glob = sub[u];
        // the tree is a single node
        if (glob == a[u]) return true;
        if (glob & 1) return false;
    }
 
    each(v, g[u]) if (v != p && !del.count({ v, u })) {
        if (split(v, u)) return true;
        if (sub[v] == glob/2) {
            del.ins({u, v}); del.ins({v, u});
            return true;
        }
    }
