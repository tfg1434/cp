...
void dfs(ll r, ll c) {
    ...
    if (c+2 < n && ma[r][c+1] == '>' && !vis[r][c+2]) dfs(r, c+2);
    if (c-2 >= 0 && ma[r][c-1] == '<' && !vis[r][c-2]) dfs(r, c-2);
    if (c > 0 && ma[1-r][c] == '<' && !vis[1-r][c-1]) dfs(1-r, c-1);
    if (c+1 < n && ma[1-r][c] == '>' && !vis[1-r][c+1]) dfs(1-r, c+1);
}
