...
ll n;
vl a, pre;
// dp[l][r][p][x] = elems [l, r) remain, 
// p==0?Alice:Bob turn, opponent processing for x more seconds
// my score - opponent score
map<tuple<ll, ll, ll, ll>, ll> memo;

ll dp(ll l, ll r, ll p, ll x) {
    if (l == r) {
        return 0;
    }
    if (memo.count({l, r, p, x})) {
        return memo[{l, r, p, x}];
    }

    ll res = 0;
    if (a[l] > x || (a[l] == x && p == 1)) {
        ckmax(res, -dp(l+1, r, 1-p, a[l]-x) + a[l]);
    } else {
        ckmax(res, dp(l+1, r, p, x-a[l]) + a[l]);
    }
    // first index from the right where it will change turn
    ll rr = lstTrue(l, r, [&](ll j) {
        if (p == 1) return pre[r]-pre[j] >= x;
        return pre[r]-pre[j] > x;
    });
    ckmax(res, -dp(l, rr, 1-p, pre[r]-pre[rr]-x) + pre[r]-pre[rr]);
