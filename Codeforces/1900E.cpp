...
void solve() {
    ...
    vl s, vis(n+1), val(n+1), siz(n+1);
    V<vl> g(n+1), r(n+1), G(n+1), R(n+1);
    ll id = 0;
    ...
    F1R(i, n) if (!vis[i]) dfs(i);
    fill(all(vis), 0);
    while (sz(s)) {
        ll u = s.bk; s.pop_back();
        if (vis[u]) continue;
        id++;
        DFS(u);
    }
    ...
    F1R(i, id) {
        dp[i] = {siz[i], -val[i]};
        each(j, R[i]) if (ckmax(dp[i], { dp[j].f+siz[i], dp[j].s-val[i] })) {
            P[i] = j;
        }
        if (ckmax(ans, dp[i])) {
            fin = i;
