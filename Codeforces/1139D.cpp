...
ll f(ll y, ll x) {
    ...
    // goal: count b on 1..m/y that share no primes with a
    // + share 1 prime
    // - share 2 primes
    ll res = 0;
    // F1R(b, m/y) if (gcd(a, b) == 1) res++;
    // return res;
    F0R(i, p2(sz(primes[a]))) if (i > 0) {
        ll z = 1;
        F0R(j, sz(primes[a])) {
            if (i&(1<<j)) z*=primes[a][j];
        }
        res += ((pct(i)&1)?1:-1)*(m/y/z);
    }
    return m/y - res;
}
