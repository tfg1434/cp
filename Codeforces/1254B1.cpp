...
void solve() {
    ...
    def(ll, n);
    vl a(n); re(a);
    vl points; F0R(i, n) if (a[i]) points.pb(i);
    ...
    ll ans = BIG;
    vl primes;
    for (ll p = 1; p*p <= sz(points); p++) if (isPrime[p] && sz(points)%p == 0) {
        if (p > 1) primes.pb(p);
        if (p*p != sz(points)) primes.pb(sz(points)/p);
    }
    ...
    each(p, primes) {
        ...
        F0R(i, sz(points)) {
            ll block = i/p;
            res += abs(points[i] - points[block*p+p/2]);
        }
        ckmin(ans, res);
    }
