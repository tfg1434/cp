...
const ll mx_n = 5000;
const ll mx_sum = 5000;
ll dp[mx_n+1][mx_sum+1];

void solve() {
    def(ll, n);
    vl a(n); re(a); sort(all(a));
    vl pre(n+1); partial_sum(all(a), bg(pre)+1);
    dp[0][0] = 1;
    for (ll i = 0; i < n; i++) for (ll j = 0; j <= mx_sum; j++) if (dp[i][j]) {
        (dp[i+1][j+a[i]] += dp[i][j]) %= P;
        (dp[i+1][j] += dp[i][j]) %= P;
    }

    ll ans = 0;
    for (ll i = 0; i < n; i++) {
        // case 1: sum < mx. there are mx groups.
        for (ll j = 0; j < a[i]; j++) {
            ans += a[i]*dp[i][j];
        }

        // case 2: sum >= mx. there are (sum+mx+1)//2 groups
        for (ll j = a[i]; j <= pre[i]; j++) {
            ans += (a[i] + j + 1) / 2 * dp[i][j];
        }
