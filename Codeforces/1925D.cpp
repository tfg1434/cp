...
ll fact[N], ifac[N];
ll C(ll n, ll r) {
    return fact[n]*ifac[r]%P*ifac[n-r]%P;
}

void init() {
    fact[0] = 1;
    for (int i = 1; i < N; i++) fact[i] = fact[i-1]*i%P;
    ifac[N-1] = pw(fact[N-1], P-2);
...
void solve() {
    ...
    ll ans = sum*k%P*pw(d, P-2)%P;

    ll cont = 0;
    // fix the number of times this pair (u, v) is chosen!
    for (ll i = 0; i <= k; i++) {
        // cont += i*(i-1)/2%P * pw(p, i)%P*pw(q, k-i)%P * C(k, i)%P;
        ll x = i*(i-1)/2 % P;
        (x *= pw(p, i)*pw(q, k-i)%P) %= P;
        (x *= C(k, i)) %= P;
        (cont += x) %= P;
    }
