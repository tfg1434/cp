// Failed idea: Fix the value x and how many there will be; 
// run knapsack to find how many ways to do the sum of the rest;
// there are n choose k ways to put the xs among non-xs
// 
// It's wrong because you will count the same string several 
// times with different values of x. Since 2x = S mod k
// has several solutions, even when k is prime
...
void solve() {
    ...
    V<vl> C(n+1, vl(n+1));
    C[0][0] = 1;
    for (ll i = 1; i <= n; i++) { // i put < n :skull:
        C[i][0] = C[i][i] = 1;
        for (ll j = 1; j < i; j++) {
    ...
    for (ll x = 0; x < k; x++) {
        // calculate knapsack...
        V<vl> knapsack(n, vl(k));
        knapsack[0][0] = 1;
        for (ll i = 0; i < n-1; i++) {
            for (ll j = 0; j < k; j++) if (j != x) {
                for (ll val = 0; val < k; val++)
                    knapsack[i+1][(j+val)%k] |= knapsack[i][val];
            }
        }

        // how many x
        for (ll i = 1; i <= n; i++) {
            ll target = ((x*(2-i) % k) + k) % k;
            ll ways = knapsack[n-i][target];
            ways *= C[n][i];
            ans = (ans+ways) % m;
        }
    }
