// my random not good enough, but i learned about how to make a 
// random distribution where each block has the same probability 
// of being size X
...
void solve() {
    ...
    rep(200) {
        ll cur = 0;
        bool palin = 0;
        auto distribution = gen_split(n);
        for (auto l : distribution) {
            if (h.get_hash(cur, cur+l-1) == h.rev_hash(cur, cur+l-1)) {
                palin = 1;
                break;
            }
            cur += l;
