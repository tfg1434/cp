...
void solve() {
    ...
    vl df(n+1), f(n+1);
    F1R(i, n) {
        df[i] = a[i] == a[i-1] ? hot[a[i]] : cold[a[i]];
    }
    partial_sum(all(df), bg(f));
    // ps(df);
    vl best_from(k+1, BIG);
    dp[0] = 0;
    ll mn = 0;
    FOR(i,1, n+1) {
        ckmin(dp[i], (a[i]!=a[i-1] ? 0 : cold[a[i]]-hot[a[i]]) + 
                min(mn, best_from[a[i]]+hot[a[i]]-cold[a[i]]));

        ckmin(best_from[a[i-1]], dp[i]);
        ckmin(mn, dp[i]);
        // ps(i, dp[i]);
    }
...
signed main() {
    ...
    //sum is the cost were everything the same colour
    //the idea is to let 
    //dp[i] = min time to get to i - default time to get to i.
    //we can retrieve the answer by adding sum to dp[n]
    //
    // so now we have transitions dp[i] = -def[i] + mindp+cold[a[i]]
    // read: the min time to arrive at i is mindp + cold[a[i]]
    // since mindp is the min time to arrive at j - default time to get to j,
    //
    //
    // also
    // dp[i] = -def[i] + best_from[a[i]]+hot[a[i]]
    // dp[i] <= 0. It's (negative) the amount you save with an optimal seq
    // of jumps
