...
const ll mx_buckets = 100;

// returns a distribution into mx_buckets(ish),
// combining so that there are no 0s or 1s
vl gen_split(ll n) {
    V<db> v;
    ll sum = 0;
    for (ll i = 0; i < mx_buckets; i++) {
        v.pb(1.0 - rand() / RAND_MAX);
        sum += v.bk;
    }
    each(x, v) x *= 1.0*n/mx_buckets;
    ...
    rep(missing) {
        floored[rand()%sz(floored)]++;
    }
    
    vl res;
    each(x, floored) {
        if (!sz(res)) res.pb(x);
        else if (res.bk <= 1 || x <= 1) res.bk += x;
        else res.pb(x);
    }
    return res;
}
...
void solve() {
    ...
    {
        if (h.get_hash(0, n-1) != h.rev_hash(0, n-1)) {
            ps("YES");
            ps(1);
            ps(s);
            return;
    ...
    ll i = 1;
    for (; i < n; i++) if (s[i] != s[0]) break;
    ll len = i+1;

    if (h.get_hash(i+1, n-1) != h.rev_hash(i+1, n-1)) {
        ps("YES");
        ps(2);
        ps(s.substr(0, len), s.substr(len, n-len));
        return;
    }

    assert(len >= 2 && len <= (n+1)/2);
    if (len == 2) {
        ps("NO");
        return;
    }
    ...
    ps("YES");
    ps(2);
    ll split = len*((n / len)/2)+1; // or len+1, simpler
    ps(s.substr(0, split), s.substr(split, n-split));
}
