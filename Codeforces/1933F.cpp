...
void solve() {
    ...
    while (sz(q)) {
        ...
        if (C == m-1) {
            // ckmin(ans, vis[R][C]+((R-(n-1+vis[R][C]))%n+n)%n);
            // careful, not -vis[R][C], we are working with a different
            // static grid here
            ll fin = n-1+vis[R][C];
            fin = ((fin%n)+n)%n;
            if (fin <= R) ckmin(ans, vis[R][C]+R-fin);
            else ckmin(ans, vis[R][C]+n-(fin-R));
        }

        // Right
        if (C+1 < m) {
            ll r = (R+1)%n, c = C+1;
            if (!get(r, c) && vis[r][c] == -1) {
                q.push({r, c});
                vis[r][c] = vis[R][C]+1;
            }
        }
        ...
        {
            ll r = (R+2)%n, c = C;
            if (!get(R+1, c) && !get(r, c) && vis[r][c] == -1) {
                q.push({r, c});
                vis[r][c] = vis[R][C]+1;
            }
        }
