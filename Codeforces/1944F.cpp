...
void solve() {
    ...
    ll ans = 0;
    // dp(i, last element, parity of the number of bad indices)
    // base case: there is one way to be at the first imaginary zero elem, there are no bad positions

    V<vl> dp(V<vl>(k+1, vl(2)));
    V<vl> pd(V<vl>(k+1, vl(2)));
    V<vl> third(k+1, vl(2));
    pd[0][0] = 1;//when u accidentally assign dp and spend so much time debugging...
    ...
    for (ll i = 1; i <= n+1; i++) {
        anylast = vl(2);
        for (ll x = 0; x <= k; x++) for (ll m = 0; m < 2; m++) (anylast[m] += pd[x][m]) %= p;

        for (ll x = 0; x <= k; x++) for (ll m = 0; m < 2; m++) {
            // for (ll y = 0; y <= k; y++) dp[x][m] += pd[y][m];
            (dp[x][m] += anylast[m]) %= p;

            if (i > 1) {
                ll cont = 0;
                // for (ll z = 0; z < k-x; z++) cont += third[z][1-m]*(k-z);
                (cont += preb[(m+1)%2][k-x]) %= p;
                // for (ll z = 0; z < k-x; z++) cont -= third[z][1-m]*x;
                (cont += p - x*prec[(m+1)%2][k-x]%p) %= p;
        ...
        preb = V<vl>(2, vl(k+2));
        for (ll m = 0; m < 2; m++) for (ll y = 0; y <= k; y++) 
            preb[m][y+1] = preb[m][y] + pd[y][m]*(k-y) % p;
        prec = V<vl>(2, vl(k+2));
        for (ll m = 0; m < 2; m++) for (ll y = 0; y <= k; y++) 
            prec[m][y+1] = prec[m][y] + pd[y][m] % p;

        if (i == n+1) {
            (ans = dp[0][0] + p - dp[0][1]) %= p;
        }
