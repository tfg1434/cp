...
void solve() {
    def(ll, a, b);
    vl A, B;
    while (a > 0) A.pb(a%10), a/= 10;
    while (b > 0) B.pb(b%10), b/= 10;
    if (sz(A) < sz(B)) {
        ps(str(sz(A), '9'));
        return;
    }
    ...
    auto plus = [&](pl p, ll x) {
        if (p.f == -1) return 0ll;
        if (x < p.f) return p.s-x;
        if (x > p.s) return x-p.f;
        return p.s-p.f;
    };
    auto upd = [&](pl& p, ll x) {
        if (p.f == -1) {
    ...
    F0R(i, sz(B)) {
        ll u = 10;
        if (curA == curB) ckmin(u, B[i]);
        FOR(j, A[i]+1, u) if (ckmin(best, plus(pa, j))) {
            ans = str(sz(B)-i, j+'0');
            if (curA > 0) ans = to_string(curA)+ans;
        }
        u = -1;
        if (curA == curB) ckmax(u, A[i]);
        FOR(j, u+1, B[i]) if (ckmin(best, plus(pb, j))) {
            ans = str(sz(B)-i, j+'0');
            if (curB > 0) ans = to_string(curB)+ans;
        }

        curA = curA*10+A[i], curB = curB*10+B[i];
        upd(pa, A[i]); upd(pb, B[i]);
    }

    if (ckmin(best, pa.s-pa.f)) ans = to_string(curA);
    if (ckmin(best, pb.s-pb.f)) ans = to_string(curB);
