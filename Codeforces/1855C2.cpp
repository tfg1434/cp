...
// Thinking
// I was so convinced that it's bitmask dp that I didn't consider
// anything else.
// Show min(x1+x2, y1+y2) < k: show x1+x2+y1+y2 / 2 < k
//
// Implementation
// Kind of slow bc of bugs i introduced
void solve() {
    ll n; re(n);
    ll neg = 0;
    ll mx = 0, pmx;
    ...
    //these variables have caused me so much grief
    f1(i, n) {
        if (a[i] > a[pp]) pp = i;
        if (a[i] < a[pn]) pn = i;
    }
    ...
    // ll x = a[n] * binpow(2, 32-n);
    // vl cnt(n+1); cnt[n] = 32-n;
