...
void solve() {
    ...
    for (ll i = 0; i < n; i++) if (a[i] > 0 && a[(i+1)%n] > 0 && a[(i+2)%n] > 0) {
        ll K = a[(i+1)%n]/a[i];
        auto L = lstTrue(0ll, K, [&](ll k) {
            return a[(i+2)%n] - k*a[(i+1)%n] + gauss(k)*a[i] > 0;
        });

        if (L == K) {
            a[(i+1)%n] = 0;
            a[(i+2)%n] += gauss(K+1)*a[i] - (K+1)*a[(i+1)%n];
            ckmax(a[(i+2)%n], 0ll);
        } else {
            a[(i+1)%n] -= K*a[i];
            ckmax(a[(i+1)%n], 0ll);
        ...
        assert(!(a[i] > 0 && a[(i+1)%n] > 0 && a[(i+2)%n] > 0));
    }

    for (ll i = 0; i < n; i++) if (a[i] > 0 && a[(i+1)%n] > 0) {
        a[(i+1)%n] = 0;
    }

    vl sol;
    for (ll i = 0; i < n; i++) if (a[i] > 0) sol.pb(i+1);
    ps(sz(sol));
    for (ll i = 0; i < sz(sol); i++) pr(sol[i], " \n"[i==sz(sol)-1]);
}
