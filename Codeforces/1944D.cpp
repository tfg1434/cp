...
void solve() {
    ...
    V<pl> alt_pairs;
    {
        ...
        while (em < n) {
            ...
            // for (; i+2<n && s[i+2] == s[em]; i += 2);
            // for (; j+2<n && s[j+2] == s[em+1]; j += 2);
            while (true) {
                ll ops = 0;
                if (i+2<n && s[i+2] == s[em]) i += 2, ops++;
                if (j+2<n && s[j+2] == s[em+1]) j += 2, ops++;
                if (abs(i-j) > 1) break;
                if (ops == 0) break;
            }
            ll en;
    ...
    ll pos = 0;
    for (ll i = 0; i < n; i++) {
        if (pos+1 < sz(alt_pairs) && alt_pairs[pos+1].f <= i) pos++;
        max_r[i] = alt_pairs[pos].s;
    }
    ...
    for (ll i = 0; i < 26; i++) {
        for (ll j = 1; j <= n; j++) {
            pre[i][j] = pre[i][j-1] + (s[j-1] == (char)('a'+i));
        }
    }

    auto h = PolyHash(s.c_str());
    auto is_alt = [&](ll l, ll r) {
        return max_r[l] >= r;
        // auto idx = lstTrue(0ll, sz(alt_pairs)-1, [&](ll j) {
            // return alt_pairs[j].f <= l;
        // });
        // if (idx == -1) return false;
        // auto p = alt_pairs[idx];

        // return p.f <= l && p.s >= r;
    };
    auto is_same = [&](ll l, ll r) {
        return pre[s[l]-'a'][r]-pre[s[l]-'a'][l] == r-l;
    };
    auto is_perm = [&](ll l, ll r) {
        if ((r-l) % 2 == 0) {
            ll m = (l+r)/2;
            ll halflen = (r-l)/2;
            return h.get_hash(m, m+halflen-1) == h.rev_hash(m-halflen, m-1);
        } else {
            ll m = (l+r)/2;
            ll halflen = (r-l)/2 - 1;
            return h.get_hash(m+1, m+1+halflen) == h.rev_hash(m-1-halflen, m-1);
        }
    };
