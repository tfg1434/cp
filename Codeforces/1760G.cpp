...
// A litany of silly implementation mistakes
// thought the xor should never be zero outside of a :skull:
// thought there was use to adding to set in dfs2
// in fact, thought that xor necessarily were all distinct :)
void solve() {
    ll n, a, b; re(n, a, b);
    V<vpl> g(n+1);
    f0(i, n-1) {
        ll u, v, w; re(u, v, w);
        g[u].eb(w, v); g[v].eb(w, u);
    }

    set<ll> st;
    bool ok = false;
    auto dfs = yy([&](auto rec, ll u, ll p, ll cur) -> void {
        if (u == b) return;
        st.insert(cur);
        for (auto [w, v] : g[u]) {
            if (v == p) continue;
    ...
    auto dfs2 = yy([&](auto rec, ll u, ll p, ll cur) -> void {
        if (u != b) ok |= st.count(cur);
        for (auto [w, v] : g[u]) {
            if (v == p) continue;
            rec(v, u, cur ^ w);
        }
