...
// Initially i tried to maintain a fenwick tree with the permutation
// indices as oindicies, but i didn't have a good way to clear the
// BIT when moving between subtrees. THis solution uses small to large
// merging to solve the problem.
// How to prove small to large? Count the # of times each individual 
// item moves
void solve() {
    ll n, q; re(n,q);
    V<vl> g(n+1);
    ...
    yy([&](auto rec, ll u, ll p) -> void {
        ...
        for (auto[idx, l, r] : qs[u]) {
            auto it = s[u].lb(l);
            ans[idx] = it != end(s[u]) && *it <= r;
        }
    })(1,1);
