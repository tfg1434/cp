...
void solve() {
    ...
    auto get = [&](ll x) -> pl{
        ll idx = lwb(xs, x);
        ll lo, hi;
        hi = idx < sz(xs) ? xs[idx] : n;
        lo = idx > 0 ? xs[idx-1] : 1;
        return {lo, hi};
    };

    vl val(n+1), cost(n+1);
    auto add_harbor = [&](ll x, ll v) {
        auto [prv, nxt] = get(x);
        ll old_val = val[prv];
        
        for (int i = x+1; i < nxt; i++) {
            if (!old_val) {
                cost[i] = (nxt-i)*v;
            } else {
                cost[i] /= old_val;
    ...
    F0R(i, q) {
        ...
        if (t == 1) {
            ...
            def(ll, l, r);
            ll res = 0;
            for (int i = l ; i <= r; i++) res += cost[i];
            ps(res);
            // ps(st.query(1, 1, n, l, r));
        }
    }
}
