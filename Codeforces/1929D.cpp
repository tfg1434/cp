...
void solve() {
    ...
    yy([&](auto rec, ll u, ll p) -> void {
        ...
        // v is the one who is 2
        each(v, children) {
            ll ways = dp[v][2];
            each(w, children) if (w != v) {
                (ways *= dp[v][0]) %= P;
            }
            (dp[u][2] += ways) %= P;
        }
        ...
        // }
        // each(v, children) {
            // ll div = IN*dp[v][1]%P*OUT%P*dp[v][0]%P;
            // dp[u][0] += tot*pw(div, P-2) % P;
            // dp[u][0] %= P;
        // }

        F0R(i, p2(sz(children))) {
            ll ways = 1;
            F0R(j, sz(children)) {
                (ways *= dp[children[j]][i&(1<<j) ? 1 : 0]) %= P;
            }
            (dp[u][pct(i) & 1] += ways) %= P;
        }
    })(1, 1);
