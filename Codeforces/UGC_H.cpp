...
void build(ll u, ll l, ll r, vl& a) {
...
ll query(ll u, ll l, ll r, ll lq, ll rq, ll x) {
...
void solve() {
    ...
    vl ans;
    rep(q) {
        def(ll, l, r, mod); l -= 1; r -= 1;

        if (mod < sq) {
            ans.pb(pre[mod][r+1]-pre[mod][l]);
        } else {
            ll sum = PRE[r+1]-PRE[l];
            ll cur = 0;
            for (ll j = mx/mod * mod; j >= mod; j -= mod) {
                ll num = query(1, 0, n-1, l, r, j);
