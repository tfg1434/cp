...
const ll P = 999999893; // 998244353;
const ll MX = 2e5+5;
...
struct Num {
    ...
    Num(const Num& num) : Num(num.a, num.b) {}
    
    Num operator *= (const Num& o) {
        ll A = a*o.a%P + 2*b%P*o.b%P;
        ll B = a*o.b%P + b*o.a%P;
        a = A%P, b = B%P;
        return *this;
    }
    Num operator*(const Num& o) {
    ...
    ll get_answer(Num den) {
        ll c = den.a, d = den.b;
        ll p = b*c%P+P-a*d%P;
        ll q = c*c%P + P-2*d*d%P;
        return p*pw(q, P-2)%P;
    }

    friend ostream& operator << (ostream& out, const Num& num) {
        if (num.b && num.a) out << num.a << '+' << num.b << "√2";
        else if (num.a) out << num.a;
        else if (num.b) out << num.b << "√2";
        else out << 0;
        return out;
    }
};
...
struct Matrix {
    ...
    Matrix<T> operator *= (const Matrix<T>& m) {
        ...
        cols = m.cols;
        vector<vector<T>> nmat(rows, vector<T>(cols, Num(0, 0)));
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
...
void solve() {
    ...
    F0R(i, q) {
        ...
        auto nmat = mat.expo(n);
        ll ans = nmat(0, 2).get_answer(nmat(1, 2));
        // ps(ans);
        cout << ans << '\n';
    }
}
