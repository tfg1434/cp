...
str ans, s;
// str _add(ll b, ll p) {
    // str B = b > 1 ? to_string(b-1) : "";

    // if (p == 0) return to_string(b);

    // str ret = B + str(p-1, '9') + B;
    // if (B != "") ret += "+" + to_string(10-stoi(B));
    // else if (b == 1) ret += "+1";
    // return ret;
// }
...
void add(ll n) {
    ll p = 0;
    ll N = abs(n);
    vl v; while (N > 0) v.pb(N%10), N /= 10, p++;
    reverse(all(v));
