...
void solve() {
    map<char, ll> key = { { 'p', 2}, {'a', 3}, {'s', 1} };

    ll n, m, k; re(n,m,k);
    ll K = 5e6 / n / m;
    ll x1, y1, x2, y2; re(x1, y1, x2, y2);
    V<string> temp(n); each(x, temp) re(x);
    V<vl> ma(n+1, vl(m+1));
    f1(i, n) f1(j, m) ma[i][j] = key[temp[i-1][j-1]];
    V<V<vl>> dp(n+1, V<vl>(m+1, vl(K, -1)));
    V<V<vl>> vis(n+1, V<vl>(m+1, vl(K)));
    dp[y1][x1][ma[y1][x1] == 2] = 1;
    auto f = yy([&](auto rec, ll y, ll x, ll c) -> ll {
        if (x < 1 || y < 1 || y > n || x > m || c < 0 || vis[y][x][c]) return 0;
