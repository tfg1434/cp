...
void solve() {
    ...
    F0R(i, n) {
        ll par = n;
        F0R(j, n) if (i != j)  {
            if (seg[j].f < seg[i].f && seg[i].s < seg[j].s
                    && seg[j].s - seg[j].f < seg[par].s - seg[par].f) {
                par = j; 
            }
        }
    ...
    // dp[u][i] = ways to delete i times from subtree u
    // if we do not remove the node u
    //
    // root - whether we are forced to delete all children
    // if root is false, the children are deleted is arbitrary
    auto count_dp = yy([&](auto rec, ll u, bool root) -> void {
        dp[u].resize(1);
        dp[u][0] = 1;
        for (auto v: g[u]) {
            rec(v, 0);
            vl new_dp(sz(dp[u])+sz(dp[v]));

            F0R(i, sz(dp[u])) {
                F0R(j, sz(dp[v])+1) {
                    ll hm = 0;
                    // Case 1 - not deleting root
                    if (j < sz(dp[v]) && !root) {
                        hm += dp[v][j]; 
                    }
                    // Case 2 - deleting root
                    if (j > 0) {
                        hm += dp[v][j-1];
                    }

                    (new_dp[i+j] += dp[u][i]*hm % P * c[i+j][i]) %= P;
                }
            }
            dp[u] = new_dp;
        }
