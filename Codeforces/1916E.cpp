...
// Segtree node stores the maximum of its subtree
// Lazy value of node stores the amount to increment the entire subtree by, except the current node
class SegmentTree {// range set value, range query for sum segtree, can be easily modified for other things {
    public:
        ...
        void lz(ll node) {
            ...
            segtree[node << 1] += lazy[node];
            segtree[node << 1|1] += lazy[node];
            lazy[node] = 0;
        }
        ...
        ll query(ll node, ll L, ll R, ll Lq, ll Rq) {
            ...
            if (Lq <= L && R <= Rq) return segtree[node];
            lz(node);

            ll mid = (L+R)/2;
            ll res = 0;
            ckmax(res, query(node<<1, L, mid, Lq, Rq));
            ckmax(res, query(node<<1|1, mid+1, R, Lq, Rq));
            return res;
        }
};
...
void solve() {
    ...
    auto dfs = yy([&](auto rec, ll u) -> void {
        if (sz(stk[c[u]])) e[stk[c[u]].bk].pb(u);
    ...
    WTF st;
    st.build(1, 1, n);
    auto dfs2 = yy([&](auto rec, ll u) -> void {
        each(v, g[u]) {
            rec(v);
        }

        st.update(1, 1, n, E[u], F[u], 1);
        each(v, e[u]) st.update(1, 1, n, E[v], F[v], -1);

        ll mx = 1;
        for (auto v : g[u]) {
            ll t = st.query(1, 1, n, E[v], F[v]);
            ckmax(ans, t*mx);
            ckmax(mx, t);
        }
    });
