...
void solve() {
    ...
    q.push({0, { s[0], 0 }});
    while (sz(q)) {
        auto [d, su] = q.top(); q.pop();
        auto [rate, u] = su;
        if (dist[{rate, u}] != d) continue;

        for (auto[w, v] : g[u]) {
            ll RATE = min(rate, s[v]);
            if (!dist.count({RATE, v}) || w*rate+d < dist[{RATE, v}]) {
                dist[{RATE, v}] = w*rate+d;
                q.push({dist[ {RATE, v} ], {RATE, v}});
            }
        }
    }

    ll ans = BIG;
    each(rate, s) if (dist.count({rate, n-1})) ckmin(ans, dist[{ rate, n-1 }]);
    ps(ans);
}
