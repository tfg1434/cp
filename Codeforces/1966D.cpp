// I missed this solution during the contest. Here's how to come up with it.
// First, you should set n=1e6 always, since otherwise there are too many cases
// and it will be much too hard for D/Codeforces
// Next, it's at least worth a shot to put the bits 1~(msb-1) and (msb+1)~19
// All we did is filter the numbers with bit msb
//
// It's natural to make two cases for x < k and x > k
// If x < k, then we have yet to describe how to get (2^msb)~(k-1).
// We will make it by introducing a number < k. 
// Specifically, introduce k - 2^msb because k - 1 - (2^msb - 1) = k-2^msb
// Now we have to describe how to make a number x on k+1 to 2^(msb+1)-1
// Use the information from this case! Think at introducing numbers > k
// Consider x' = x-(k+1), x' >= 2^msb. We know x' <= 2^msb; in this case just
// add 2^msb. So introduce k+1 and k+1+2^i.
//
//
//
// I didn't commit to the cases < k and > k because I didn't realise how
// they helped; they let you focus on adding a set of numbers with global sum
// < k and adding numbers > k, respectively.
//
//
//
//
...
void solve() {
    ...
    int msb = bits(k);
    cout << 22 << endl;
    for (int i = 0; i < 20; i++) if (i != msb) cout << p2(i) << ' ';
    cout << k - p2(msb) << ' ' << k+1 << ' ' << k+1+p2(msb) << endl;
}
