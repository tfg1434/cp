...
void solve() {
    ...
    F0R(i, N) C[i][i] = C[0][i] = C[i][0] = 9999;
    auto mex = [&](ll r, ll c) {
        // gg(r, c);
        ll has[10000]; memset(has, 0, sizeof has);
        F0R(i, r) has[C[i][c]] = 1;
        F0R(i, c) has[C[r][i]] = 1;
        F1R(i, min(r, c)) has[C[r-i][c-i]] = 1;

        ll ret = 0;
        F0R(i, 10000) {
            if (!has[ret]) return ret;
            ret++;
        }
        return -1ll;
    };
    ...
    // F0R(i, N) {
        ...
        // F0R(j, N) {
            // if (col[j] || row || diag[j-i]) C[i][j] = 1;
            // else {
                // col[j] = row = diag[j-i] = 1;
            // }
        // }
    // }
