...
void solve() {
    ...
    if (k-n-m+2 < 0 || (k-n-m+2) & 1) {
        ps("NO");
        return;
    }

    V<vl> ansr(n+1, vl(m)), ansc(n, vl(m+1));
    f1(i, m-1) ansr[1][i] = !ansr[1][i-1];
    ansc[0][m] = ansr[1][m-1];
    FOR(i, 1, n) ansc[i][m] = !ansc[i-1][m];

    auto o = [&]() {
        ps("YES");

        f1(i, n) {
            f1(j, m-1) pr(ansr[i][j] ? 'R' : 'B', ' ');
            ps();
        }
        f1(i, n-1) {
            f1(j, m) pr(ansc[i][j] ? 'R' : 'B', ' ');
            ps();
        }
    };
    ...
    ansc[n-1][m] = ansc[n-1][m-1] = ansc[n-2][m];
    ansr[n-1][m-1] = ansr[n][m-1] = !ansc[n-1][m];

    //extra circuit
    ansc[n-1][m-2] = !ansc[n-1][m-1];
    ansr[n-1][m-2] = ansr[n][m-2] = !ansr[n][m-1];
