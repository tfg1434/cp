// was supposed to be an O(n^2) dp partial, but alas 
// i have an implementation bug
...
void solve() {
    ...
    l.insert(bg(l), a0);
    r.insert(bg(r), b0);
    pre = vl(n+1); partial_sum(all(d), bg(pre));

    for (ll i = 0; i <= n; i++) fill(all(dp[i]), -BIG);

    // dp(i, elements since last capped a) = max elems since last capped b
    dp[0][0] = 0;
    for (ll i = 0; i < n; i++) {
        for (ll since = 0; since <= i; since++) {
            if (dp[i][since] == -BIG) continue;

            // the sum of <since>+1 elems plus the old cap
            ll sumA = pre[i+1]-pre[i+1-since-1] + l[i-since];
            ll sumB = pre[i+1]-pre[i+1-dp[i][since]-1] + r[i-dp[i][since]];

            if (sumA <= l[i+1] || sumB <= r[i+1]) {
                ckmax(dp[i+1][since+1], dp[i][since]+1);
            } else {
                // reset A
                ckmax(dp[i+1][0], dp[i][since]+1);
                // reset B
                ckmax(dp[i+1][since+1], 0ll);
            }
        }
    }
    ...
    // gg(n);
    // for (ll i = 0; i <= n; i++) {
        // for (ll since = 0; since <= n; since++) {
            // pr(dp[i][since]==-BIG ? "-" : to_string(dp[i][since]), ' ');
        // }
        // ps();
    // }
    ...
    ll ans = 0;
    for (ll since = 0; since <= n; since++) {
        if (dp[n][since] == -BIG) continue;

        ll cont_a = pre[n]-pre[n-since]+l[n-since];
        ll cont_b = pre[n]-pre[n-dp[n][since]]+r[n-dp[n][since]];
        ckmax(ans, cont_a+cont_b);
    }
