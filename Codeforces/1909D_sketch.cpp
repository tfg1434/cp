...
vl divK(ll x, ll mx) {
    ...
    for (ll i = 1; i*i <= x; i++) if (x%i == 0) {
        if (i <= mx) res.pb(i);
        if (i != x/i && x/i <= mx) res.pb(x/i);
    }
    return res;
}

// Mistake - not considering f < k
// Mistake - a[0] vs a[1], not doing UNIQUE first
// I abandoned this one cuz it's sketchy and I don't have time...
// I think it's good for USACO though
void solve() {
    ll n, k; re(n, k);
    ll sum = 0, mn = 0;
    vl a(n); re(a); 
    each(x, a) sum += x, ckmin(mn, x);
    UNIQUE(a);
    if (sz(a) == 1 || a[0] == a[1]) {
        ps(0);
        return;
    ...
    vl dif;
    F0R(i, sz(a)-1) if (a[i+1]-a[i]) dif.pb(a[i+1]-a[i]);
    UNIQUE(dif);

    ll fk = 0;
    vl ds = divK(dif[0], a[0]-k);
    // vl DS;
    // FOR(i, 1, min(10ll, sz(dif))) {
        // swap(ds, DS);
        // ds.clear();
        // vl tmp = divK(dif[i], a[0]-k);
        // set_intersection(all(DS), all(tmp), back_inserter(ds));
    // }
    sort(rall(ds));
    each(d, ds) {
        ll g = 0; each(x, a) g = gcd(g, x-(d+k));
        if (g % d == 0) {
            fk = d;
            break;
        }
    ...
    if (!fk) {
        // try to find f < k
        ll kf = 0;
        vl ds = divK(dif[0], k-a.bk);
        // vl DS;
        // FOR(i, 1, min(10ll, sz(dif))) {
            // swap(ds, DS);
            // ds.clear();
            // vl tmp = divK(dif[i], k-a.bk);
            // set_intersection(all(DS), all(tmp), back_inserter(ds));
        // }
