...
void solve() {
    ...
    re(n, x);    
    a = vl(n); re(a); mx = *max_element(all(a));
    F0R(j, logn) {
    ...
    vl mark(n);
    ll init_and = p2(logn)-1;
    each(x, a) init_and &= x;

    // case 1: the AND increases
    F0R(i, n) {
        bool found = false;
        ll AND = init_and;
        vl extra;
        ...
        F0R(j, logn) {
            ...
            if ((sz(v) == 1 || sz(v) == 2) && count(all(v), i)) {
                // cost me TIME bc i didn't add the second guard that i \in v
                found = true;
                // i resize here bc found will only happen 2*logn times
                extra = vl(n);
        ...
        F0R(k, n) if (i != k) {
            auto ANDD = AND + extra[k];
            if (gcd(a[i], a[k]) > x+ANDD) {
                OUT(i, k);
                return;
            }
        }
    ...
    ps("casetwo");
    // case 2: the AND doesn't increase
    vl cnt(mx+1);
    vpl b;
    F0R(i, n) if (!mark[i]) b.pb({ a[i], i });
    F0R(i, sz(b)) for (auto d : divs[b[i].f]) cnt[d]++;

    for (ll g = 1; g <= mx; g++) {
        if (cnt[g] >= 2 && g>x+init_and) {
            // will only run once
            F0R(i, sz(b)) if (b[i].f % g == 0) {
                if (ans1 == -1) {
                    ans1 = b[i].s;
                } else {
                    OUT(ans1, b[i].s);
