...
// variables
// array cont -- we take prefix sums at the end
//
// precomputations
// sum when p=4
// finding the primes <= 1e6 (7e4 of them)
...
void solve() {
    ...
    for (int i = 2; i*i <= max_n; i++) if (is_prime[i]) {
        for (int j = i*i; j <= max_n; j += i) is_prime[j] = false;
    }

    vi cont(max_n+1);
    for (int i = 2; i <= max_n; i++) if (is_prime[i]) {
        for (int k = 1; k*i <= max_n; k++) {
            int delta = ((-k%i)+i) % i;
            (cont[i*k] += delta) %= P;
            if (i*(k+1) <= max_n) (cont[i*(k+1)] += P-delta) %= P;
        }
    }
    // special case of j=4
    for (int k = 1; k*4 <= max_n; k++) {
        int delta = 2*k % 4;
        (cont[4*k] += delta) %= P;
        if (4*(k+1) <= max_n) (cont[4*(k+1)] += P-delta) %= P;
    }

    for (int i = 2; i <= max_n; i++) (cont[i] += cont[i-1]) %= P;
    for (int i = 2; i <= max_n; i++) (cont[i] += cont[i-1]) %= P;
