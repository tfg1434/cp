...
void solve() {
    ...
    vl a(n); re(a);
    vl freq(n+1);
    V<deque<ll>> where(n+1);
    F0R(i, n) {
        freq[a[i]]++;
        if (sz(where(a[i])) num[i] = num[where[a[i]].bk]+1;
        else num[i] = 0;
        where[a[i]].pb(i);
    }
    deque<pl> q;
    F0R(i, n) q.pb({a[i], num[i]});
    ...
    ll r = 0;
    set<pl> st;
    F0R(i, n) if (!st.count(div(q[i]))) {
        sum += min(k, freq[q[i].f]);
        freq[q[i].f] -= min(k, freq[q[i].f]);
    ...
    F1R(i, n-1) {
        pl del = q.ft;
        del.s = num[where[del.f].bk]+1;
        where[del.f].pop_front();
        where[del.f].pb(n-1+i);
        q.pb(del);
        q.pop_front();
        ...
        st.erase(div(del));
        off[del.f]++;
        for (; r-i < n; r++) if (!st.count(div(q[r-i]))) {
            sum -= min(k, )

            sum -= min(k, freq[del]);
            sum += min(k, freq[q[r-i]]);
            st.ins(q[r-i]);
            if (sz(st) == m) break;
        }
        r++;
