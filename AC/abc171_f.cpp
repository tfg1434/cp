...
void init() {
    ...
    // DONT make mistake of not defining inv[0]
    // otherwise you get C(n, n) = 0
    F0R(i, N) inv[i] = binpow(fact[i], P-2);
}

void solve() {
    def(ll, n);
    str s; re(s); ll k = sz(s);

    ll ans = 0;
    F0R(i, n+1) {
        ll x = C(i+k-1, k-1);
        (ans += binpow(25, i)*binpow(26, n-i)%P*x%P) %= P;
        
        // ps(25, i, 26, n-i, "choose:", i+k-1, i);
    }
...
signed main() {
    ...
    // ll ans = 0;
    // F0R(i, 6) {
        // (ans += binpow(25, i) * (i+2)*(i+1)/2 %P * binpow(26, 5-i) % P) %= P ;
    // }
    // ps(ans);
    // return 0;
