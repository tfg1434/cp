...
void solve() {
    def(ll, n);
    vl a(n); re(a); each(x, a) x--;
    vl where(n); F0R(i, n) where[a[i]] = i;

    set<ll> st; F0R(i, n-1) st.ins(i);
    vl ans(n);
    F0R(i, n) {
        ...
        if (i > 0) {
            ll prv = where[i-1];
            if (prv > 0 && a[prv-1] < a[prv]) {
                st.ins(prv-1);
            }
            if (prv < n-1 && a[prv+1] < a[prv]) {
                st.ins(prv);
            }
        }
        ...
        if (sign(a[p-1]-a[p]) == sign(a[p+1]-a[p])) {
            ans[p] = 3;
            continue;
        }
