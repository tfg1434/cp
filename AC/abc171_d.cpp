...
bool solve() {
    ...
    vl dp(p2(n+1), BIG), ok(p2(n+1), 1);
    for (auto[u,v]:edges) {
        ll mask = (p2(n+1)-1) ^ (1<<u) ^ (1<<v);
    ...
    // F0R(i, p2(n+1)) if (!ok[i]) {
        // ps(bitset<4>(i));
    // }
    // dp[i] = min cost (colours) to colour this set of nodes.
    dp[0] = 0; // when we have 0 nodes in our subgraph, we need 0 colors
    for (int i = 1; i < p2(n+1); i++) {
        // i ^ s is the nodes we are coloring with a new color
        for (int s = i; ; s = (s-1)&i) {
            if (!ok[i^s]) {
                if (s == 0) break;
                continue;
