...
void solve() {
    ...
    F0R(i, n) {
        ...
        if (s < SQRT) {
            F0R(j, s) FOR(k, j, s) {
                ll pk = where[i][k], pj = where[i][j];
                if (k-j+1 > (pk-pj+1)/2) {
                    ll maxi = (k-j+1)*2-1 - (pk-pj+1);
                    ll lj = min(pj-(j>0 ? where[i][j-1]:-1)-1, maxi);
                    ll rk = min((k+1<s ? where[i][k+1] : n)-pk-1, maxi);
                    if (lj + rk <= maxi) {
                        ans += (lj+1)*(rk+1);
                    } else {
                        ll lt_rk = rk+1-(maxi-lj+1);
                        ans += gauss(rk)-gauss(rk-lt_rk);
                        ans += (lj+1-lt_rk)*(rk+1);
                    }
                }
            }
