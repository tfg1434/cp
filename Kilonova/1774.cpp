...
ll ask(ll r, ll c) {
    ...
    auto work = [&](ll x, vl&on) {
        ...
        F0R(i, m) on[i] = L[i]/2 + (x&p2(i) ? 1 : 0)*(R[i]+1);
    };
    ...
    ll res = 0;
    F0R(i, bits(r)+1) {
        (res += p2(i)*(on[i]*(r-ON[i])%Q + (c-on[i])*ON[i]%Q)) %= Q;
    }
...
void solve() {
    ...
    rep(T) {
        def(ll, r1, c1, r2, c2);
        ll ans = ask(r2, c2)+Q-ask(r1-1, c2)+Q-ask(r2, c1-1)+ask(r1-1, c1-1);
        ans %= Q;
        ps(ans);
    }
}
