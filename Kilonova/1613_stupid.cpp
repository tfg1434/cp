...
void solve2(ll n) {
    ...
    dp[0][1][0] = 1;
    ll m = 0;
    vl v; while (n>0) v.pb(n%2), n /= 2, m++;
    v.pb(0); reverse(all(v)); 
    F1R(i, m) FOR(k, 0, K) {
        ll x = 0, y = 0, z = 0;
        if (v[i] == 1) {
            // adding 1
            if (k > 0) x = dp[i-1][0][k-1];
            // adding 0
            x += dp[i-1][0][k] + dp[i-1][1][k];
        } else {
            x = dp[i-1][0][k];
        }

        if (v[i] == 1) {
            if (k > 0) y = dp[i-1][1][k-1];
        } else {
            y = dp[i-1][1][k];
        }

        if (v[i] == 1) {
            if (k > 0) z = dp[i-1][2][k-1];
        } else {
            z = dp[i-1][2][k];
            if (k > 0) z += dp[i-1][2][k-1] + dp[i-1][1][k-1];
        }
