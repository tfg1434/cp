...
// dp(c, r) = number of occurences of r consecutive c's in all numbers from 1..g
//
ll solve(ll g) {
    if (g<=0) return 0;
    ...
    FOR(c, 1, 10) F1R(r, m) {
        ...
        FOR(p, 1, m-r+1 + 1) {
            ...
            // Case 1, we have values before p are <= g
            dx += (x+1)*p10[m-p-r+1];
            // Case 3, cccc equals a_p ... a_p+r-1
            // a_p+r ... a_m
            // the last m-p-r+1 digits
            dx += g%p10[m-p-r+1];
            if (p+r-1 < m) dx++;
        }
