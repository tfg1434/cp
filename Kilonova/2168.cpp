...
void add(int node)
{
    for(; node < n; node |= node+1) fen[node]++;
}
int ask(int node)
{
    int ans = 0;
...
long long fenpw[200001];
void add_pw(int node, long long val)
{
    for(; node < n; node |= node+1) fenpw[node] = (fenpw[node] * val) % P;
}

long long ask_prod(int node) {
    long long ans = 1;
    for(; node >= 0; node=(node&(node+1))-1) ans = (ans * fenpw[node]) % P;
    return ans;
}

// It's wrong rn but I got the idea
// (hopefully) look at it again tomorrow?
void solve() {
    re(n, q);
    a.resize(n); 
    F0R(i, n) {
    ...
    for (ll i = 18; i >= 0; i--) {
        ...
        ll idx = 0;
        for (ll j = 0; j < q; j++) {
            // ans for each query is the smallest value where there are at least
            // k values >= than it in the array (i.e. it's the k-th largest value)
            // So, here i want to add all the values that are >= ans+step. This way
            // we can check if it's still the k-th largest with this value of ans.
            // while (idx < n && qs[idx].ans+p2(i) >= qs[j].f) {
            while (idx < n && qs[j].ans-p2(i) <= a[idx].f) {
                add(a[idx].s);
                idx++;
            }

            if (ask(qs[j].r)-ask(qs[idx].l-1) < qs[j].k) qs[j].ans -= p2(i);
        }
    }
    ...
    F0R(i, n) fenpw[i] = 1;
    vl sol(q);
    for (auto[l, r, k, idx, ans] : qs) {
        while (pos < n && a[pos].f <= ans) {
            add_pw(a[pos].s, a[pos].f);
            add(a[pos].s);
            pos++;
        }

        ll cnt = ask(r)-ask(l-1);
        ll prod = ask_prod(r) * pw(ask_prod(l-1), P-2) %P;
        (prod *= pw(pw(ans, cnt-k), P-2)) %= P;
        sol[idx] = prod;
    }

    partial_sum(all(sol), ostream_iterator<ll>(cout, "\n"));
}
