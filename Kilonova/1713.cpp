...
const ll N = 102;
ll dp[N][N][N]; //dp[i][j][k] = ways to put k dogs on i rows, j cols
                //it's regular dp
...
void init() {
    ...
    ROF(i, 0, N-1) ifac[i] = ifac[i+1]*(i+1)%P;
}
...
void solve() {
    ...
    // dp[0][0][0] = 1;
    F0R(i, N) F0R(j, N) dp[i][j][0] = 1;

    F1R(i, n) F1R(j, m) F1R(k, K) {
        dp[i][j][k] = dp[i-1][j][k];

        ll J = j;
        if (i == n && j == m) J--;

        // dog with no attackers
        dp[i][j][k] += J*dp[i-1][j-1][k-1];

        // dog with vertical attacker
        if (i > 1 && k > 1) dp[i][j][k] += J*(i-1)*dp[i-2][j-1][k-2];

        // dog with horizontal attacker
        if (j > 1 && k > 1) dp[i][j][k] += J*(J-1)/2*dp[i-1][j-2][k-2];
