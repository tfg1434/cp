...
void solve() {
    ...
    F1R(i, N) F1R(j, K) {
        if (f[i] <= K*S) {
            ...
            ll p = 0;
            for (ll b = MAXN; b > 0; b/=2) {
                if (p+b < i && f[i]-f[p+b] >= S) {
                    p += b;
                }
            }

            ckmin(dp[i][j], S-(f[i]-f[p])+dp[p-1][j-1]);
            ckmin(dp[i][j], S-(f[i]-f[p+1])+dp[p][j-1]);
        }
    }

    F1R(i, KK) ps((K+KK)*S - S*i);
    FOR(i, KK+1, K+1) {
        ps(inev + dp[N][i]);
    }
}
