...
void solve() {
    int n, m; re(n, m);
    V<vl> ma(n+1, vl(m+1)); f1(i, n) f1(j, m) {
        char c; re(c);
        ma[i][j] = c - '0';
    }

    V<vi> d0(n+1, vi(m+1, BIGG)), dn(n+1, vi(m+1, BIGG)), vis(n+1, vi(m+1)); 
    auto bfs = [&](queue<pl>& q, V<vi>& d) -> void {
        f1(i, n) f1(j, m) vis[i][j] = 0;
        int dd = 0;
        while (!q.empty()) {
            int cnt = sz(q);
            f0(i, cnt) {
                pl p = q.front(); q.pop();
                d[p.s][p.f] = min(d[p.s][p.f], dd);
                vis[p.s][p.f] = 1;
                f0(j, 4) {
                    int mx = p.f + dx[j], my = p.s + dy[j];
                    if (mx > 0 && my > 0 && mx <= m && my <= n && !vis[my][mx] && !ma[my][mx]) {
                        q.push({ mx, my });
                    }
                }
            }
    ...
    queue<pl> q; q.push({ 1, 1 });
    bfs(q, d0);
    q = queue<pl>(); q.push({ m, n });
    bfs(q, dn);
    int D = BIGG;
    f1(i, n) f1(j, m) D = min(D, d0[i][j] + dn[i][j]);
    ...
    f1(i, n) {
        ...
        f1(j, m) {
            ...
            int d0Here = BIGG, dnHere = BIGG;
            f0(k, 4) {
                int mx = j+dx[k], my = i+dy[k];
                if (mx > 0 && my > 0 && mx <= m && my <= n) {
                    d0Here = min(d0Here, d0[my][mx]+1);
                    dnHere = min(dnHere, dn[my][mx]+1);
                }
            }
            if (dnHere + d0Here < D) {
                s += '1';
