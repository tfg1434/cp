...
void solve() {
    ...
    rep(m) {
        def(ll, pos, year); pos--;
        fp[year] = min(fp[year], pos);
        ckmax(mx, year);
    }
    fp[1] = 0;
    // move the last backwards, ie require at least p
    // at the end.
    if (mx != -1) {
        while (fp[mx]+p-1 >= n) fp[mx]--;
    }
    ...
    for (ll i = 1; i < n; i++) {
        ...
        if (placed == q) {
            cur++;
            placed = 0;
        // we need to increment cur if the number you can place 
        // (including here) is exactly the number you need to fill with p.
        } else if (nxt < sz(b) && b[nxt]-i == p*(a[b[nxt]]-cur-1)) {
            cur++;
            placed = 0;
        }
