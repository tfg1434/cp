...
void solve() {
    ...
    vl dp(n+1, BIG);
    yy([&](auto rec, ll u, ll p, ll c) -> void {
        dp[u] = c;
        ll sum = 0;
        for (auto[c, w, v] : g[u]) if (v != p) {
            rec(v, u, c);
            sum += dp[v];
        }
        if (sum) ckmin(dp[u], sum);
    })(1,1,0);

    ll MX = 0;
    yy([&](auto rec, ll u, ll p, ll d) -> void {
        ckmax(MX, d);
        for (auto[c, w, v] : g[u]) if (v != p) {
            rec(v, u, d+w);
        }
    ...
    vl need(n+1, -BIG);
    yy([&](auto rec, ll u, ll p, ll d) -> ll {
        need[u] = d;

        for (auto[c, w, v] : g[u]) if (v != p) {
            ckmax(need[u], rec(v, u, d+w));
        }
    ...
    ll ans = 0;
    yy([&](auto rec, ll u, ll p, ll cur) -> void {
        if (u != 1) {
            need[u] += cur;
            if (need[u] < 0) {
                cur -= need[u];
