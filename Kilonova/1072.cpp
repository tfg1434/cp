...
// Hint: pre[y]-pre[x-1]
// Thinking
// I needed the hint to use prefix sums, the issue with trying
// to calculate between x and y directly is that for some i
// the vy < vx and then it becomes meaningless and unhelpful
ll solve(ll g) {
    memset(dp, 0, sizeof dp);
    ll G = log10(g);
    if (G+1 < n) return 0;

    FOR(i, 1, g%10) dp[i][0] = 1;
    if (g%10 > 0) dp[g%10][1] = 1;
    FOR(i, g%10+1, 10) dp[i][2] = 1;

    FOR(i, 1, 10) assert(get(dp[i]) == 1);

    FOR(i, 1, n) {
        FOR(sum, 0, n+9) FOR(cmp, 0, 3) {
            ll vg = g/p10[i]%10;

            ll dx = 0;
            if (cmp == 2) {
                FOR(d, vg, 10) if (sum >= d) {
                    if (d == vg) {
                        dx += dp[sum-d][2];
                    } else {
                        dx += get(dp[sum-d]);
                ...
                if (sum >= vg) dx += dp[sum-vg][1];
            } else if (cmp == 0) {
                FOR(d, 1, vg+1) if (sum >= d) {
                    if (d == vg) {
                        dx += dp[sum-d][0];
                    } else {
                        dx += get(dp[sum-d]);
