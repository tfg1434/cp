...
// I mixed count and contribution in the same arrays which gives obviously
// wrong answer. Lesson? Solidify your idea before start implementing. It 
// might be hard to stay motivated but we can remedy this by moving to another
// problem in the meantime
ll solve(ll g) {
    if (g<=0) return 0;

    ll G = log10(g)+1;
    FOR(d, 1, 10) dp[d][1][d==g%10?1 : (d>g%10?2:0)] = 1;
    FOR(d, 1, 10) assert(get(dp[d][1]) == 1);

    FOR(i, 1, G) {
        ll vg = g/p10[i]%10;
        ll prv = g/p10[i-1]%10;

        FOR(d, 1, 10) FOR(len, 1, i+2) F0R(cmp, 3) {
            ll dx = 0; 
            if (cmp == 1) {
                if (prv != vg) dx = dp[]
            } else {
                ll lo, hi;
                if (cmp == 0) lo = 1, hi = vg;
                if (cmp == 2) lo = vg+1, hi = 10;

                dx += dp[d][len-1][cmp];
                if (len == 1) {
                    FOR(LEN, 1, N) FOR(D, lo, hi) {
                         dx += get(dp[D][LEN])*D*LEN*LEN;
                    }
                }
            }
    ...
    ll res = 0;
    FOR(d, 1, 10) FOR(len, 1, G+1) F0R(cmp, 2) res += dp[d][len][cmp]*d*len*len;
    memset(dp, 0, sizeof dp);
    return res;
}
...
signed main() {
    setIO();
    F0R(i, N) p10.pb(p10.bk*10);
    
    gg(solve(10));
    gg(solve(100));
    re(A, B);
    if (A > B) ps(0);
    else ps(solve(B)-solve(A-1));
    gg(solve(B));
    gg(solve(A-1));
