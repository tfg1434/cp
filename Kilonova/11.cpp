...
ll n, m, u, v;
const ll N = 101, M = 10001;
ll dp[N][M], sum[N][M];
vl primes[M];
vl g[N];

void init() {
    for (ll i = 2; i < M; i++) if (primes[i].empty()) {
        for (ll j = i; j < M; j += i) {
            primes[j].pb(i);
        }
    }
...
void solve() {
    init();
    re(n, m); // m is NOT the number of edges 
    rep(n-1) {
        re(u, v);
        g[u].pb(v);
        g[v].pb(u);
    ...
    auto dfs = yy([&](auto rec, ll u, ll p) -> void {
        ...
        for (ll i = 2; i <= m; i++) {
            ...
            each(v, g[u]) if (v != p) {
                ll x = 0;
                ll numprimes = sz(primes[i]);

                FOR(mask, 1, p2(numprimes)) {
                    ll sign = -1;
                    ll prod = 1;

                    F0R(j, numprimes) if (mask & (1 << j)) {
                        prod *= primes[i][j];
                        sign *= -1;
                    }

                    (x += P+sum[v][prod]*sign) %= P;
                }
