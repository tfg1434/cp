...
void solve() {
    ...
    if (C == 1) {
        ...
        // Next smaller Element to the right, NSE to the left
        // also these values have to be in the same type of square
        // How to calculate NSE in a 1d array?
        // Work backwards, nse(back) = null, push elements into a stack
        // pop all elements from the stack that are larger than current element
        // stack is monotonically increasing (recent elements are bigger)
        // If the brick changed, pop all elements from the stack
        ll ans = 0;
        ll x1, y1, x2, y2;
        f1(i, n) {
            vl R(m+1), L(m+1);
        ...
        auto check = [&](ll k) -> bool {
            ...
            V<vl> dif(n+2, vl(m+2));
            f1(i, k) {
                ll r1 = q1[i].f, c1 = q1[i].s, r2 = q2[i].f, c2 = q2[i].s;
                ll d = ms[i];
                dif[r1][c1] += d; dif[r2+1][c2+1] += d;
                dif[r2+1][c1]-= d; dif[r1][c2+1] -= d;
            }

            f1(i, n) f1(j, m) {
                dif[i][j] = dif[i][j-1] + dif[i][j];
            }
            f1(j, m) f1(i, n) {
                dif[i][j] = dif[i-1][j] + dif[i][j];
            }

            ll mn = BIG;
            f1(i, n) f1(j, m) ckmin(mn, a[i][j] + dif[i][j]);

            return mn >= K;
        };

        ll p = Q;
        for (ll base = Q; base > 0; base /= 2) {
            while (check(p-base)) p -= base;
        }
