...
void build(ll u, ll l, ll r) {
    FOR(i, l, r+1) t[u].pb(a[i]);
    sort(all(t[u]));
    suf[u].resize(sz(t[u]));
    partial_sum(rall(t[u]), rbegin(suf[u]), [&](ll x, ll y) {
        return x*y%P;
    });
...
// Query how many values are >= x
pl query(ll u, ll l, ll r, ll lq, ll rq, ll x) {
    if (r < lq || rq < l) return {0, 1};
    if (lq <= l && r <= rq) {
        pl res {0, 1};
        auto gteq_x = fstTrue(0ll, r-l, [&](ll j) {
            return t[u][j] >= x;
        });
        if (gteq_x == r-l+1) {
            return {0, 1};
        }
        return { r-l+1-gteq_x, suf[u][gteq_x] };
    }

    pl res {0, 1};
    ll m = (l+r)/2;
    res += query(2*u, l, m, lq, rq, x);
    res += query(2*u+1, m+1, r, lq, rq, x);
    return res;
}
...
void solve() {
    ...
    // assert(query(1, 0, n-1, 0, n-1, 9).f == 3);
    // assert(query(1, 0, n-1, 0, 3, 4).f == 3);

    rep(q) {
        def(ll, l, r, k);
        // I want to find the largest value of x where there are 
        // at least k values >= x
        ll minx = lstTrue(0ll, MX, [&](ll x) {
            return query(1, 0, n-1, l, r, x).f >= k;
        });

        auto res = query(1, 0, n-1, l, r, minx);
        ll extra = pw(minx, res.f-k);
        ll sol = res.s * pw(extra, P-2)%P;
        ps(sol);
    }
