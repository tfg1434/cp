// (Not used here but how to count rectangles)
// Fix the bottom left and top right corners, then check whether the two corners exist
//
#include <bits/stdc++.h>
using namespace std;
...
void solve() {
    ...
    for (auto[_m, v] : mp) {
        map<ll, ll> cnt;
        map<ll, map<ll, ll>> sub, SUB;
        db m = fix[_m];

        for (sg s : v) {
            auto o = (s.f+s.s)/2;
            auto mv = cl(1, -1.0/m);
            if (isinf(m)) mv = cl(1, 0);
            if (m == 0) {
                cnt[BIG+cnv(o.x())]++;
                sub[BIG+cnv(o.x())][(ll)o.x()^(ll)o.y()]++;
                SUB[BIG+cnv(o.x())][dist(s.f, s.s)]++;
                continue;
            }
            auto tmp = o - mv*o.x();
            assert(abs(tmp.x()) == 0);
            if (tmp.y() == 8) {
                gg(cnv(s.f.x()), cnv(s.f.y()), cnv(s.s.x()), cnv(s.s.y()));
                gg(m);
            }
            cnt[cnv(tmp.y())]++;
            sub[cnv(tmp.y())][(ll)o.x()^(ll)o.y()]++;
            SUB[cnv(tmp.y())][dist(s.f, s.s)]++;
        }
