...
void unite(ll u, ll v, ll cur) {
    if ((u=find(u))!=(v=find(v))) {
        if (sz(qs[u]) > sz(qs[v])) swap(u, v);
        for (ll i : qs[u]) {
            if (qs[v].count(i)) ans[i] = cur;
            else {
                qs[v].ins(i);
            }
        }
...
void solve() {
    ...
    for (auto[w, p] : t) {
        ...
        for (ll dr:{-1, 0, 1}) for (ll dc:{-1,0, 1}) if (abs(dr)+abs(dc) == 1) {
            ll nr = p.f+dr, nc=p.s+dc;
            if (in(nr, nc) && vis[idx[nr][nc]]) {
                gg("yo!");
                unite(idx[p.f][p.s], idx[nr][nc], w);
            }
        }
    }
