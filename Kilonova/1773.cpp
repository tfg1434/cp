...
void solve() {
    ...
    F1R(z, T) {
        ...
        // F0R(i, n) F0R(j, n) {
            // F0R(k, n) F0R(l, n) {
                // if (i==k && j==l) continue;
                // ckmax(MX[i][j], ma[k][l]-(abs(i-k)+abs(j-l)));
            // }
        // }
        ...
        // ps("-----Just columns--------");
        // F0R(i, n) {F0R(j, n) pr(mx[i][j], ' '); ps();}
        // ps("-------------");

        auto COLS = cols;
        FOR(j, 1, n) {
            F0R(i, n) {
                ckmax(cols[i][j], max(ma[i][j-1], cols[i][j-1])-1);
            }
        }
        ROF(j, 0, n-1) {
            F0R(i, n) {
        ...
        auto mx = cols;
        F0R(i, n) F0R(j, n) ckmax(mx[i][j], COLS[i][j]);

        // F0R(i, n) { F0R(j, n) pr(ma[i][j], ' '); ps(); }
        // ps("------------Wrong------------");
        // F0R(i, n) { F0R(j, n) pr(mx[i][j], ' '); ps(); }
        // ps("---------Correct-------------");
        // F0R(i, n) { F0R(j, n) pr(MX[i][j], ' '); ps(); }
        // F0R(i, n) F0R(j, n) assert(MX[i][j] == mx[i][j]);

        F0R(i, n) F0R(j, n) {
            ma[i][j] = z+1+(ma[i][j]+z*mx[i][j])%Q;
        }
    }

    ll ans = 0;
    F0R(i, n) F0R(j, n) (ans += ma[i][j]) %= Q;
    ps(ans);
}
