...
ll work(vl& dist) {
    ...
    auto dfs = [&](ll i, ll j) {
        ls.pb(j);
        while (j+1 < m && ma[i][j+1]) {
            j++;
        } 
        rs.pb(j);
    };

    F0R(i, n) F0R(j, m) if (ma[i][j]) {
        if ((!i || !ma[i-1][j]) && (!j || !ma[i][j-1])) {
            dfs(i, j);
        }
    }

    sort(rall(ls)); sort(rall(rs));
    ll sl = 0, cl = 0; each(x, ls) sl += x, cl++;
    ll sr = 0, cr = 0;
...
void solve() {
    xd.resize(m), yd.resize(n);
    ma = V<vl>(n, vl(m)); re(ma);
    work(xd);
    auto cpy = V<vl>(m, vl(n));
    F0R(i, n) F0R(j, m) cpy[j][i] = ma[i][j];
    swap(ma, cpy);
    swap(n, m);
    work(yd);
    swap(n, m);
    ...
    // F0R(i, n) {
        ...
        // ps();
    // }
    F0R(i, n) F0R(j, m) if (!cpy[i][j]) {
        if (ckmin(best, xd[j]+yd[i])) {
            r = i, c = j;
        }
    }
