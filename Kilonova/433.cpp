...
void solve() {
    ll n, q; re(n, q);
    vpl a;
    V<vl> emp(n+1), fin(n+1);
    vl pts;
    F0R(i, q) {
        def(ll, x, y);
        a.eb(x, y);
        pts.pb(x); pts.pb(y);
        emp[x].pb(y), fin[y].pb(x);
    }
    pts = vl(all(set<ll>(all(pts))));

    auto F = [&](ll f, ll x) {
        return fact[f+x-1]*binpow(fact[f-1], P-2);
    };
    ...
    each(p, pts) {
        ...
        ll x = F(prv, dist-(sz(emp[p]) > 0));
        (ans *= x) %= P;

        each(x, emp[p]) st.ins(x);
        each(x, fin[p]) st.erase(st.find(x));
        ll l = !sz(st) ? 1 : *rbegin(st);

        prv = (p-l+1);
        pp = p;

        if (t) st.erase(st.find(t));
    }
