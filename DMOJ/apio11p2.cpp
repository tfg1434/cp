// after applying coordinate compression, each coordinate is <= 2e3
// apparently, N^2 lgn works even though there are 20 tests
// fails samples but i'm tired of debugging
...
bool cmp_x(pair<pl, ll> a, pair<pl, ll> b) {
    if (a.f.s == b.f.s) return a.f.f < b.f.f;
    return a.f.s < b.f.s;
}
bool cmp_y(pair<pl, ll> a, pair<pl, ll> b) {
...
void solve() {
    ...
    UNIQUE(ys);
    for (auto&[x1, x2, y1, y2] : rect) {
        x1 = lwb(xs, x1); x2 = lwb(xs, x2);
        y1 = lwb(ys, y1); y2 = lwb(ys, y2);
    }

    auto calc_dist = [&](ll x1, ll y1, ll x2, ll y2) {
        return abs(xs[x1]-xs[x2]) + abs(ys[y1]-ys[y2]);
    };

    V<V<vpl>> g(2*n, V<vpl>(2*n));
    for (ll i = 0; i < n; i++) {
    ...
    V<pair<pl, ll>> x_events;
    {
        for (ll i = 0; i < n; i++) {
            auto [x1, x2, y1, y2] = rect[i];
            for (ll y = y1; y <= y2; y++) {
                x_events.eb(mp(x1, y), 1ll);
                x_events.eb(mp(x2, y), -1ll);
            }
        }
        sort(all(x_events), cmp_x);
        ll cnt = 0;
        for (ll i = 0; i < sz(x_events); i++) {
            if (i && !cnt && x_events[i].f.s == x_events[i-1].f.s) {
                g[x_events[i].f.f][x_events[i].f.s].eb(x_events[i-1].f.f, x_events[i-1].f.s);
                g[x_events[i-1].f.f][x_events[i-1].f.s].eb(x_events[i].f.f, x_events[i].f.s);
            }
            cnt += x_events[i].s;
            assert(cnt < 2);
        }
    ...
    V<pair<pl, ll>> y_events;
    {
        for (ll i = 0; i < n; i++) {
            auto [x1, x2, y1, y2] = rect[i];
            for (ll x = x1; x <= x2; x++) {
                y_events.eb(mp(x, y1), 1ll);
                y_events.eb(mp(x, y2), -1ll);
            }
        }
        sort(all(y_events), cmp_y);
        ll cnt = 0;
        for (ll i = 0; i < sz(y_events); i++) {
            if (i && !cnt && y_events[i].f.f == y_events[i-1].f.f) {
                g[y_events[i].f.f][y_events[i].f.s].eb(y_events[i-1].f.f, y_events[i-1].f.s);
                g[y_events[i-1].f.f][y_events[i-1].f.s].eb(y_events[i].f.f, y_events[i].f.s);
            }
            cnt += y_events[i].s;
            assert(cnt < 2);
        }
    ...
    V<vl> dist(2*n, vl(2*n, BIG));
    pqg<pair<ll, pl>> q;
    dist[lwb(xs, sx)][lwb(ys, sy)] = 0; q.push({0, {lwb(xs, sx), lwb(ys, sy)}});
    while (sz(q)) {
        auto [d, xy] = q.top(); q.pop();
        auto [x, y] = xy;
        if (d != dist[x][y]) continue;
        for (auto [X, Y] : g[x][y]) if (ckmin(dist[X][Y], d+calc_dist(x, y, X, Y))){
            q.push({dist[X][Y], {X, Y}});
        }
    }

    ll ans = dist[lwb(xs, tx)][lwb(ys, ty)];
