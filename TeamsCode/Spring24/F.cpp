...
ll dp[mx_log+1][mx_n+1][3];

void solve() {
    def(ll, n, l, r);    
    vl a(n); re(a);
    vl num_on(mx_log);
    for (ll i = 0; i < n; i++) for (ll j = 0; j < mx_log; j++) 
        if (a[i] & p2(j)) num_on[j]++;

    auto getcmp = [](ll x, ll y, ll old) -> ll {
        if (x > y) return 2;
        if (x == y) return old; 
        return 0;
    };
    ...
    auto work = [&](ll x) {
        ...
        for (ll i = 0; i < mx_log; i++) for (ll c = 0; c < mx_n; c++) for (ll cmp = 0; cmp < 3; cmp++) {
            {
                // set X[i] = 0
                ll cnt = num_on[i]+c;
                dp[i+1][cnt/2][getcmp(cnt%2, (bool)(x & p2(i)), cmp)] += dp[i][c][cmp];
            }
            {
                // set X[i] = 1
                ll cnt = n-num_on[i]+c;
        ...
        return dp[mx_log][0][0] + dp[mx_log][0][1];
    };
