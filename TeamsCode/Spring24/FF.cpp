...
void solve() {
    def(ll, n, l, r);
    vl a(n); re(a);
    ll logn = bits(r) + 1; // how many digits in r
    vl num_on(logn);
    each(y, a) for (ll j = 0; j < logn; j++) num_on[j] += (bool)(y & p2(j));
    
    auto work = [&](ll m) {
        V<V<vl>> dp(logn+1, V<vl>(n, vl(2)));
        dp[0][0][0] = 1;

        for (ll i = 0; i < logn; i++) for (ll c = 0; c < n; c++) for (ll cmp = 0; cmp < 2; cmp++) {
            if (!dp[i][c][cmp]) continue;

            for (auto here : {c+num_on[i], c+n-num_on[i]}) {
                bool gt = (here % 2 > (bool)(m & p2(i))) || (here%2 == (bool)(m & p2(i)) && cmp == 1);
                assert(here/2 < n);
                dp[i+1][here/2][gt] += dp[i][c][cmp];
            }
        }
