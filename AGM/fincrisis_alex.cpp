...
void solve() {
    ...
    V<vl> pc(max_p, vl(m+1));
    auto precalc = [&](ll c) {
        map<ll, ll> cnt;
        for (ll i = 0; i < sz(a[c]); i++) cnt[a[c][i]]++;

        vpl pairs;
        for (auto[u, v] : cnt) {
            assert(u > 0);
            if (u*2 < x && cnt.count(x-u)) {
                pairs.eb(v, cnt[x-u]);
            }
        }
        V<vl> dp(sz(pairs)+3, vl(m+1));
        dp[0][0] = 1; // dp[i][j] = ways to highlight elems that can be 
                      // deconstructed into j * X invoice
        for (ll i = 0; i < sz(pairs); i++)  {
            for (ll j = 0; j <= m; j++) {
                auto [f1, f2] = pairs[i];
                for (ll k = 0; k <= min(pairs[i].f, pairs[i].s) && j+k <= m; k++) {
                    dp[i+1][j+k] += dp[i][j] * C[f1][k] % P * C[f2][k] % P;
                    dp[i+1][j+k] %= P;
                }
            }
        }
        {
            ll i = sz(pairs);
            ll f_x2 = x % 2 == 1 ? 0 : cnt[x/2];
            for (ll j = 0; j <= m; j++) {
                for (ll k = 0; k <= f_x2 && j+k/2 <= m; k += 2) {
                    dp[i+1][j+k/2] += dp[i][j] * C[f_x2][k];
                    dp[i+1][j+k/2] %= P;
                }
            }
        }
        {
            ...
            ll f_x = cnt[x];
            for (ll j = 0; j <= m; j++) {
                for (ll k = 0; k <= f_x && j+k <= m; k++) {
                    dp[i+1][j+k] += dp[i][j] * C[f_x][k];
                    dp[i+1][j+k] %= P;
                }
            }
        }
        for (ll i = 0; i <= m; i++) {
            pc[c][i] = dp[sz(pairs)+2][i];
        }
    };
    for (ll i = 0; i < max_p; i++) precalc(i);

    V<unordered_map<ll, ll>> freq(max_p);
    for (ll c = 0; c < max_p; c++) {
        for (ll d : a[c]) {
            freq[c][d]++;
        }
    }
    V<vl> dp(max_p+1, vl(m+1)); // how many segments we have considered,
                              // how many Xs we have highlighted
    dp[0][0] = 1;
    for (ll i = 0; i < max_p; i++) {
        for (ll j = 0; j <= m; j++) {
            for (ll k = 0; k <= sz(a[i]) && j+k <= m; k++) {
                dp[i+1][j+k] += dp[i][j] * pc[i][k];
                dp[i+1][j+k] %= P;
            }
        }
    }
