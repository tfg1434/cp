...
V<str> a;
const ll N = 1e5+1;
unordered_set<ll> hashes[N];

bool solve() {
    re(n);
    a = V<str>(n); re(a);
    sort(all(a), [&](str s, str t) {return sz(s) < sz(t);});

    PolyHash bh(a[n-1].c_str());

    while (sz(a) > 1 && a[n-1] == a[n-2]) a.pop_back();
    if (sz(a) > 1 && sz(a[n-1]) == sz(a[n-2])) return false;

    vl lens;
    for (ll i = 0; i < n-1; i++) if (!sz(lens) || lens.bk != sz(a[i]))
        lens.pb(sz(a[i]));

    for (auto i : lens) {
        for (ll j = 0; j+i-1 < sz(a[n-1]); j++) {
            ll h = bh.get_hash(j, j+i-1);
            hashes[i].ins(h);
        }
    }

    for (ll i =0; i < n-1; i++) {
        ll h = PolyHash(a[i].c_str()).get_hash(0, sz(a[i])-1);
        if (!hashes[sz(a[i])].count(h)) return false;
    }
