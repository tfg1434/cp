// fast enough for everything but WA
//
...
void solve() {
    def(string, s);
    def(string, t);
    swap(s, t); // s is the string we find, t is the one we search in...
    auto S = PolyHash(s.c_str());
    auto T = PolyHash(t.c_str());

    auto calc = [&]() {
        vi longest_forwards(sz(t));
        for (int i = 0; i < sz(t); i++) {
            int len = 0;
            for (int j = 17; j >= 0; j--) {
                int Len = len+(1<<j);
                if (i+Len-1 < sz(t) && Len <= sz(s) && S.get_hash(0, Len-1) == T.get_hash(i, i+Len-1))
                    len = Len;
            }
            longest_forwards[i] = min(len, sz(s)-1);
        }
        return longest_forwards;
    };
    auto longest_forwards = calc();
    reverse(all(t));
    reverse(all(s));
    auto longest_backwards = calc();
    reverse(all(longest_backwards));
    reverse(all(t));
    reverse(all(s));

    vi suf_max(sz(t));
    for (int i = sz(t)-1; i >= 0; i--) {
        if (i < sz(t)-1) suf_max[i] = suf_max[i+1];
        ckmax(suf_max[i], longest_backwards[i]);
    }
    for (int i = 0; i < sz(t); i++) {
        int min_j = i+sz(s)-1;
        if (min_j >= sz(t)) break;

        if (suf_max[min_j] >= sz(s)-longest_forwards[i]) {
            for (int j = i+longest_forwards[i]; ; j++) {
                if (T.get_hash(j, j+sz(s)-longest_forwards[i]-1) == S.get_hash(longest_forwards[i], sz(s)-1)) {
                    ps(i, longest_forwards[i], j, sz(s)-longest_forwards[i]);
                    return;
                }
            }
            assert(false);
    ...
    vi pre_max(sz(t)+1);
    for (int i = 0; i < sz(t); i++) {
        ckmax(pre_max[i+1], max(pre_max[i], longest_backwards[i]));
    }
    for (int i = 0; i < sz(t); i++) {
        if (pre_max[i] >= sz(s) - longest_forwards[i]) {
            for (int j = 0; ; j++) {
                if (S.get_hash(longest_forwards[i], sz(s)-1) == T.get_hash(j, j+sz(s)-longest_forwards[i]-1)) {
                    ps(i, longest_forwards[i], j, sz(s)-longest_forwards[i]);
                    return;
                }
            }
