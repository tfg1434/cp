...
void solve() {
    ...
    vi a(n); re(a);
    for (int i = 0; i < n; i++) {
        for (auto d : divs[a[i]]) where[d].pb(i);
    }
    ...
    for (int i = 0; i < n; i++) {
        for (auto d : divs[a[i]]) {
            int pos = -1;
            if (sz(where[d]) > 1) pos = where[d][0] == i ? where[d][1] : where[d][0];
            if (sz(where[d]) == 1 && where[d][0] != i) pos = where[d][0];
            if (pos != -1) ckmin(ans, (long long)(a[i]/d) * (a[pos]/d));
        }
    }
