...
struct Point {
    ...
    friend ostream& operator<<(ostream& out, const Point& point) {
        out << point.x << ' ' << point.y << ' ' << point.X << ' ' << point.Y << ' ' << point.e << ' ' << point.ind;
        return out;
    }
};
...
pi query(int v, int l, int r, int L, int R) {
    if (r <= L || R <= l) return {-INF, -1};
    if (L <= l && r <= R) return t[v];
    int m = (l+r)/2;
    return max(query(2*v+1, l, m, L, R), query(2*v+2, m, r, L, R));
}

void update_max(int v, int l, int r, int i, pi x) {
    if (l == r-1) {
        ckmax(t[v], x);
        return;
    }
    int m = (l+r)/2;
    if (i < m) update_max(2*v+1, l, m, i, x);
    else update_max(2*v+2, m, r, i, x);
...
void solve() {
    // init_tree(6);
    // update_max(0, 0, 6, 5, {-1, 69});
    // ps(query(0, 0, 6, 0, 6));
    // ps(query(0, 0, 6, 5, 6));
    // ps(query(0, 0, 6, 0, 5));

    def(int, n, k, h, w);
    vi xs(n), ys(n), energy(n);
    for (int i = 0; i < n; i++) re(xs[i], ys[i], energy[i]);
    n += 2;
    xs.pb(0); ys.pb(0); energy.pb(0);
    xs.pb(h); ys.pb(w); energy.pb(0);
    auto XS = xs, YS = ys;
    auto xs_sort = xs, ys_sort = ys;
    sort(all(xs_sort)); sort(all(ys_sort));
    for (auto&x : XS) x = lwb(xs_sort, x);
    for (auto&y : YS) y = lwb(ys_sort, y);
    V<Point> pts;
    for (int i = 0; i < n; i++) pts.pb({xs[i], ys[i], XS[i], YS[i], energy[i], i});
    sort(all(pts));

    bool ans = false;
    init_tree(sz(ys));
    vi from(n, -1); 
    update_max(0, 0, sz(ys), pts[0].y, {k, n-2});
    for (int i = 1; i < n; i++) {
        auto [x, y, X, Y, e, ind] = pts[i];
        auto q = query(0, 0, sz(ys), 0, Y+1);
        int mx = q.f - x - y;
        if (mx >= 0) {
            from[ind] = q.s;
            update_max(0, 0, sz(ys), Y, { e+mx + x + y, ind });
            ans |= (i == n-1);
        }
    }
    ...
    ps("YES");
    vi sol;
    for (int i = from[n-1]; i != n-2; i = from[i]) sol.pb(i+1);
    reverse(all(sol));
    ps(sz(sol));
    for (int i = 0; i < sz(sol); i++) cout << sol[i] << " \n"[i==sz(sol)-1];
}
