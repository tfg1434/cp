...
void solve() {
    ...
    // for (int i = 1; i < max_n; i++) {
        ...
        // }
    // }
    for (int i = 1; i <= max_n; i++) f.pb(f.bk*i%mod);
    ff.pb(pw(f.bk, mod-2));
    for (int i = max_n; i >= 1; i--) ff.pb(ff.bk * i % mod);
    reverse(all(ff));
    auto C = [&](int n, int r) {
        return f[n]*ff[n-r]%mod*ff[r]%mod;   
    };
    ...
    int root;
    for (int i = 0; i < n; i++) if (!appears[i]) root = i;
    ...
    yy([&](auto rec, int u) -> void {
        ...
        if (L[u] != -1 && R[u] != -1) {
            (ways *= C(sub[L[u]]+sub[R[u]], sub[L[u]])) %= mod;
        }
    })(root);
    // ps(ways);

    ps(f[n] * pw(ways, mod-2) % mod);
}
