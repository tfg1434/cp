...
void solve() {
    ...
    if (k == 1) {
        ...
        for (int i = 0; i < n-1; i++) delta[i+1]+=delta[i];

        V<vi> where(2*n+1);
        for (int i = 0; i <= 2*n; i++) where[i].pb(-1);
        for (int i = 0; i < n; i++) where[delta[i]+n].pb(i);
        for (int i = 0; i <= 2*n; i++) where[i].pb(n);

        int ans = 0;
        for (int i = 0; i <= 2*n; i++) {
            int len1 = 0, len2 = 0;
            for (int j = 0; j < sz(where[i])-1; j++) {
                ans += len2*(where[i][j+1] - where[i][j]);
                len2 += where[i][j+1] - where[i][j];
                swap(len1, len2);
            }
        }
