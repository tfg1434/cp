...
void solve() {
    ...
    auto dfs = yy([&](auto rec, ll r, ll c) -> void {
        ...
        f0(i, 4) {
            ...
            if (!ok(nr, nc) || vis[nr][nc] || !found[nr][nc]) continue;

            rec(nr, nc);
        }

        for (auto [nr, nc] : g[r][c]) if (!vis[nr][nc]) {
            if (found[nr][nc]) continue;
            found[nr][nc] = true;

            bool adj = false;
            f0(j, 4) {
                ll mr = nr+dy[j], mc = nc+dx[j];
                if (!ok(mr, mc)) continue;
    ...
    V<vl> mark(n+1, vl(n+1));
    set<pl> v;
    // for (auto[r, c] : st) if (!vis[r][c]) {
        // mark[r][c] = 1;
        // v.pb({r, c});
    // }
    f1(i, n) f1(j, n) if (vis[i][j]) {
        for (auto[r, c] : g[i][j]) if(!vis[r][c]) {
            v.ins({r, c});
            mark[r][c] = true;
        }
    }
    ...
    vl e(L);
    ll id = 1;
    auto dfs2 = yy([&](auto rec, ll r, ll c) -> void {
        cc[r][c] = id;
        e[id]++;

        f0(i, 4) {
            ll nr = r + dy[i], nc = c + dx[i];
            if (!ok(nr, nc) || !mark[nr][nc] || cc[nr][nc]) continue;
    ...
    gg(cnt);
    gg(*max_element(all(e)));
    ps(*max_element(all(e)) + cnt);
}
