...
void solve() {
    ...
    //sheriff positions
    double x1 = d, y1 = 0;
    auto f = [&](ll idx, double t, double ct) {
        double TS = idx == 0 ? ts : 0;
        double xx = x1, yy = y1;

        double dx = cos(a[idx].f);
        double dy = sin(a[idx].f);
        double x2 = dx*(ct+t)*a[idx].s;
        double y2 = dy*(ct+t)*a[idx].s;
        if (t > TS) {
    ...
    do {
        ...
        f0(i, 3) {
            ...
            for(ll j = 0; j < 100; j++) {
                double m1 = (A * 2 + B) / 3.0;
                double m2 = (A + 2 * B) / 3.0;
                if(f(i, m1, cur) > f(i, m2, cur)) A = m1;
                else B = m2;
            }
            // cur += sqrt((X-x1)*(X-x1) + (Y-y1)*(Y-y1)) / r;
            cur += A;
            cur++;
            gg(x1, y1);
            x1 = X, y1 = Y;
