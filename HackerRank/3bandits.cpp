...
void solve() {
    ...
    string _; re(_);
    double sOff, sSpd; re(sOff, sSpd);
    double x1 = sOff, y1 = 0;

    double X, Y;
    auto f = [&](double ct, double t, double idx) {
        double TS = idx == 0 ? ts : 0;
        double xx = x1, yy = y1;

        //calculate criminal position
        double dx = cos(a[idx].f);
        double dy = sin(a[idx].f);
        double x2 = dx*ct*a[idx].s;
        double y2 = dy*ct*a[idx].s;
        
        //calculate cop position
        double nx = cos(atan2(x2-x1, y2-y1));
        double ny = sin(atan2(x2-x1, y2-y1));
        xx += nx * t * sSpd;
        yy += ny * t * sSpd;

        X = x2, Y = y2;
        double x = (x2 - xx) * (x2-xx) + (y2-yy) * (y2 - yy);
        gg(x, t);
        return x;
    };
    ...
    do {
        ...
        FOR(idx, 0, 3) {
            for(int i = 0; i < 100; i++) {
                ...
                if(f(cur+m1, m1, idx) > f(cur+m2, m2, idx)) A = m1;
                else B = m2;
            }
            cur += A+1;
            x1 = X, y1 = Y;
