...
void solve() {
    ...
    auto count_dp = yy([&](auto rec, ll u, ll p) -> void {
        dp[u] = 1;
        each(v, g[u]) if (v != p) {
            rec(v, u);
            (dp[u] *= dp[v]+1) %= m;
    ...
    vl ans(n);
    auto reroot = yy([&](auto rec, ll u, ll p, ll super_dp) -> void {
        ans[u] = dp[u]*(super_dp+1) % m;
        vl sons;
        each(v, g[u]) if (v != p) sons.pb(v);

        vl pre(sz(sons)+1, 1), suf(sz(sons)+1, 1);
        F0R(i, sz(sons)) pre[i+1] = pre[i]*(dp[sons[i]]+1)%m;
        R0F(i, sz(sons)) suf[i] = suf[i+1]*(dp[sons[i]]+1)%m;
        F0R(i, sz(sons)) {
            rec(sons[i], u, (super_dp+1)*pre[i]%m * suf[i+1]%m);
        }
    });
    reroot(0, 0, 0);
