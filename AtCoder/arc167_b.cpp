...
// stupid time wasted debugging did + 1 % P...
// Learned about how to "factorize" number in N^(1/3)
// aided at the end by probablistic/deterministic 
// primality test
// Learned how to prime factorize a number upto 1e12
// in O(N^1/2)
// I saw edi but on this problem it's important to be 
// careful when it's not divisible by 2. To calculate 
// it, use 2*MOD as base
//
void solve() {
    ll a, b; re(a, b);
    vl ps, es;
  ...
  // for (auto x : m) {
      // // res = ((res % P) * ((((x.second % P) * (b)) % P + 1) % P)) % P;
      // res = ((res % P) * (x.second % P * b % P + 1 % P)) % P;
      // res = 
  // }
