...
const ll N = 101;
const ll M = 3125+2; //3127=53*59
ll dp[6][6][6][6][6];
ll dp2[6][6][6][6][6];
ll ma[N][6];

ll g(ll x) {return min(x, 5ll);}
...
void solve() {
    ...
    // V<vl> ma(n+1, vl(k+1));
    f1(i, n) {
        FOR(j, 0, k+1) re(ma[i][j]);
        // FOR(j, 1, k+1) gg(ma[i][j]);
    }
    ...
    f1(I, n) {
        // WHY is it not from->to arguments WTF
        memcpy(dp2, dp, sizeof dp);

        // gg(ma[I][0]);
        f0(i, 6) f0(j, 6) f0(k, 6) f0(l, 6) f0(m, 6) {
            // ps(dp[i+ma[I][1]][j+ma[I][2]][k+ma[I][3]][l+ma[I][4]][m+ma[I][5]]);
            ckmin(dp[g(i+ma[I][1])][g(j+ma[I][2])][g(k+ma[I][3])][g(l+ma[I][4])][g(m+ma[I][5])],
                    dp2[i][j][k][l][m]+ma[I][0]);
        }

        FOR(i, f[1], 6) FOR(j, f[2], 6) FOR(k, f[3], 6) FOR(l, f[4], 6) FOR(m, f[5], 6) {
            ckmin(ans, dp[i][j][k][l][m]);
        }
    }
