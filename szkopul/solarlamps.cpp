...
ll fen[N];
ll tot;
// increment all values >= x
void add(ll x) { 
    for (; x < N; x|=x+1) fen[x]++;
    tot++;
}
// OHH SNAP psums RUPQ does it by DEFAULT
ll ask(ll x) {
    ll res = 0;
...
// I learned a lot but still couldn't get it to pass more than samples
// Known issue: lasers break it, but other than that idk
void solve() {
    add(3); add(5); add(7);
    assert(ask(4) == 1);
    assert(ask(5) == 2);
    ...
    auto get_x1 = [&](ll X, ll Y) {
        return dx*Y-dy*X;
    };
    auto get_x2 = [&](ll X, ll Y) {
        return -(DX*Y-DY*X);
    };
    ...
    F0R(i, n) {
        auto[X, Y] = base[i];
        xy.pb({ lwb(xs, get_x1(X, Y)) , lwb(ys, get_x2(X, Y))});
        auto[x1, x2] = xy[i];
        x1s.pb(x1);
        // if (!x2s.count(x1)) x2s[x1].resize(0);
        x2s[x1].pb({ x2, i});
    }
    ...
    F0R(i, n) {
        ...
        for (ll stp = 18; stp >= 0; stp--) {
            while (k-p2(stp) > 0) {
                ...
                each(x1, x1s) {
                    if (x1 > xy[i].f) break;
                    for (auto [x2, idx] : x2s[x1]) if (idx < k-p2(stp) || ask(x2) >= thres[idx]){
                        // if (i == 3) ps("Point", (char)('A'+idx), "Incrementing y>=", x2);
                        add(x2);
                    }
                }
    ...
    partial_sum(all(ans), ostream_iterator<ll>(cout, " "));
    ps();
}
