...
void solve() {
    ...
    //size of largest IS with bits in mask.
    vl dp(1 << 20);
    //largest IS with bits in mask. size is dp[mask].
    vl mx(1<<20);

    FOR(i, 1, 1<<20) {
        f0(j, 20) if (i & (1 << j)){
            //if we include bit j in the IS, we need to remove
            //all its adjacent bits
            ll x = i ^ (1 << j);
    ...
    // calculate whether this subset is independent itself.
    dp[0] = true;
    f0(i, 1<<20) {
        dp[i] = true;
        f0(j, 20) {
