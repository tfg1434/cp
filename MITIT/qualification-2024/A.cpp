...
void solve() {
    ...
    rep(m) {
        ...
        if (x == y && y == z) cnt[{x, y, z}]++;
        else if (x == y) cnt[{x, z, 0}]++;
        else if (y == z) cnt[{x, y, 0}]++;
        else cnt[{x, y, z}]++;
    }

    for (auto[u, v] : cnt) if (v) {
        auto[x, y, z] = u;
        if (x == y && y == z) {
            //
        } else if (z == 0) {
            v += cnt[{x, x, x}] + cnt[{y, y, y}];
        } else {
            v += cnt[{x, x, x}] + cnt[{y, y, y}] + cnt[{z, z, z}];
            v += cnt[{x, y, 0}] + cnt[{y, z, 0}] + cnt[{x, z, 0}];
        }

        if (v % 2 == 1) {
            ps("YES");
            for (int i = 1; i <= n; i++) pr(x == i || y == i || z == i, ' ');
            ps();
            return;
        }
    } 
