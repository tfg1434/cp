...
void solve() {
    ...
    if (k == 0) {
        ...
        F0R(i, n) {
            while (!numdup && j+1 < n) {
                j++;
                freq[a[j]]++;
                if (freq[a[j]] == 2) numdup++;
            }

            if (numdup) ans += n-j;
            freq[a[i]]--;
            if (freq[a[i]] == 1) numdup--;
        }
    ...
    F0R(i, n) {
        ll mn = BIG, mx = n;
        if (sz(good)) mn = *bg(good);
        if (sz(bad)) mx = *bg(bad);

        if (mx > mn) ans += mx-mn;

        each(x, G[i]) good.erase(good.find(x));
        each(x, B[i]) bad.erase(bad.find(x));
    }
    ...
    // for (auto[p, v] : m) {
        // F0R(i, sz(v)-1) if ((v[i].f^v[i+1].f) == k) {
            // // both exclusive
            // ll prv = i ? v[i-1].s : -1;
            // ll nxt = i+2 < sz(v) ? v[i+2].s : n;
            // ans += C(nxt-prv-1);
            // gg(i, v[i], v[i+1], prv, nxt);
        // }

        // ll j = 0; // j is exclusive
        // map<ll, ll> mp;
        // vl V;
        // gg(v);
        ...
        // F0R(i, sz(v)-1) {
            // ll prv = -1;
            // while (j < sz(v) && (sz(mp) < 2 || mp.count(v[j]))) {
                // if (v[j] == prv) V.bk++;
                // else V.pb(1);
