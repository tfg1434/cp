...
void solve() {
    ...
    F0R(j, m) F0R(i, n) if (ma[i][j] == 'o') {
        if (!get(i-1, j) && !get(i+1, j)) num = 0;
        ...
        if (get(i-2, j, 'o') && get(i-1, j)) {
            num = 0;
        } 
        if (get(i, j-2, 'o') && get(i, j-1)) {
            if (R[i][j-2] && l) {
                ll x = (L[i][j-2] + R[i][j-2])*(l+r);
                (den *= x) %= P;
                (num *= x-1) %= P;
            }
        } 
        if (get(i-1, j-1, 'o') && get(i, j-1)) {
            if (!L[i-1][j-1] || !r) num = 0;
            if (R[i-1][j-1]) (den *= (L[i-1][j-1] + R[i-1][j-1])*(l+r)) %= P;
        }
        if (get(i+1, j-1, 'o') && get(i, j-1)) {
            if (!L[i+1][j-1] || !r) num = 0;
            if (R[i+1][j-1]) (den /= (L[i+1][j-1] + R[i+1][j-1])*(l+r)) %= P;
