...
struct sliding_window {
    ...
    void push_back(int x) {
        while (sz(s) && x > s.bk) s.pop_back();
        s.pb(x);
        all.pb(x);
    }
    void pop_front() {
        if (sz(all)) {
            if (sz(s) && all.ft == s.ft) s.pop_front();
            all.pop_front();
        }
    }
    int get_max() {
...
void work() {
    ...
    for (int i = 0; i < n; i++) window.pb(a[i] - i);
    for (int i = 0; i < n; i++) {
        if (ckmin(ans, i+window.get_max())) sol = i;
        window.pop_front();
        window.pb(a[i+n] - (i+n));
    }
}
...
void solve() {
    ...
    if (type == 1) {
        ...
        for (auto&x : a) cin >> x;
        rep(n-m) {
            a.pb((a[sz(a)-2]*x + a[sz(a)-1]*y + z) % mod + 1);
        }
    }
    work();
    cout << ans << ' ' << sol+1 << '\n';
