// btw purpose of m is for the subtasks
...
void solve() {
    ...
    if (n <= 17) {
        V<vl> masks(n+1);
        for (ll msk = 0; msk < p2(n); msk++) masks[pct(msk)].pb(msk);

        // dp(i, j, mask) = processed i elements, j good pairs, mask is the set of occupied b's
        // there are 2^n pairs (i, mask) because we only consider those masks with i bits on
        // dp(i, j, mask) -> dp(i+1, j+1, mask | 1), dp(i+1, j, mask | 1)
        V<V<vl>> dp(n+1, V<vl>(n+1, vl(p2(n))));
        dp[0][0][0] = 1;
        for (ll i = 0; i < n; i++) for (ll good = 0; good <= i; good++) for (auto msk : masks[i]) {
            // the (i+1th) bit pairs with index j in b
            for (ll j = 0; j < n; j++) if ((msk & p2(j)) == 0){
                if (a[i] > b[j]) {
                    (dp[i+1][good+1][msk | p2(j)] += dp[i][good][msk]) %= P;
                } else {
                    (dp[i+1][good][msk | p2(j)] += dp[i][good][msk]) %= P;
                }
            }
        }

        for (ll good = 0; good <= n; good++) {
            for (auto msk : masks[good]) {
                ps(good, bitset<5>(msk), dp[n][good][msk]);
                (ans[good] += dp[n][good][msk]) %= P;
            }
        }
    }

    for (ll i = m; i <= n; i++) cout << ans[i] << " \n"[i == n];
}
