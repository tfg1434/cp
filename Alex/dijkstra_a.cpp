...
void solve() {
    ...
    V<vl> G(n); // which taxis the taxi w/ index i can reach
    F0R(i, n) {
        vl dist(n, BIG);
        pqg<pl> q;
        q.push({0, i});
        ...
        while (sz(q)) {
            ...
            for (auto [w, v] : g[u]) if (ckmin(dist[v], d+w)) {
                q.push({dist[v], v});
            }
        }

        F0R(j, n) if (i != j) if (dist[j] <= maxdist[i]) 
            G[i].pb(j);
    }
    ...
    while (sz(q)) {
        auto [d, u] = q.top(); q.pop();
        if (d != dist[u]) continue;
        for(auto v : G[u]) if (ckmin(dist[v], d+cost[u])) {
            q.push({dist[v], v});
        }
    }

    if (dist[t] == BIG) dist[t] = -1;
    ps(dist[t]);
}
