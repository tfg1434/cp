...
void solve() {
    ...
    comps = n;
    def(int, G, S);
    V<array<int, 4>> edges; // gold, silver, u, v
    for (int i = 0; i < m; i++) {
        def(int, u, v, g, s); u--; v--;
        edges.pb({g, s, u, v});
    }
    ...
    multiset<int> max_s;
    int ans = INF;
    V<vpi> adj(n); // adj list for the current min span forest
    for (auto edge : edges) {
        int g = edge[0];
        int s = edge[1];
        int U = edge[2];
        ...
        if (find(U) != find(V)) {
            ...
            int max_w = 0;
            pi where;
            yy([&](auto rec, int u, int p) -> bool {
                if (u == V) return true;

                for (auto [w, v] : adj[u]) if (v != p) {
                    if (rec(v, u) && ckmax(max_w, w)) {
                        where = {u, v};
                    }
                }
            ...
            if (max_w > s) {
                max_s.erase(max_s.find(max_w));
                max_s.insert(s);
                {
                    auto it = find(all(adj[where.f]), pi{max_w, where.s});
                    if (it == end(adj[where.f])) while(true) {}
                    adj[where.f].erase(it);
                }
                {
                    auto it = find(all(adj[where.s]), pi{max_w, where.f});
                    if (it == end(adj[where.s])) while(true) {}
                    adj[where.s].erase(it);
                }
                adj[U].pb({s, V});
                adj[V].pb({s, U});
            }
        ...
        if (comps == 1) {
            // unprotected read is ok, since N>=2
            ckmin(ans, g*G + (*rbegin(max_s))*S);
        }
    }
