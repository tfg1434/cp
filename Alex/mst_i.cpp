...
void solve() {
    ...
    if (sz(red) && k == 0) { // too many red edges
        ps(-1);
        return;
    }

    auto cmp_wi = [&](int x, int y) {
        if (edges[x][0] != edges[y][0]) return edges[x][0] < edges[y][0];
        assert(x != y);
        return x < y;
    };
    sort(all(red), cmp_wi);
    sort(all(black), cmp_wi);

    auto cmp = [&](int c1, int c2, int c3, int x, int y) {
        auto w = edges[x][0] + c1*edges[x][3];
        auto W = edges[y][0] + c1*edges[y][3];
        if (w != W) return w < W;
        auto i2 = x*x+c2*edges[x][3];
        auto I2 = y*y+c2*edges[y][3];
        if (i2 != I2) return i2 < I2;
        auto i = x+c3*edges[x][3];
        auto I = y+c3*edges[y][3];
        if (i != I) return i < I;
        assert(x != y);
        return x < y;
    };

    __int128 hi = (__int128)m*m*m*m + m*m*m + m;
    __int128 lo = -hi;

    vi sol;
    auto check = [&](__int128 pivot) {
        memset(e, -1, sizeof e);
        sol.clear();
        int cnt_red = 0;

        int c3 = pivot % m; pivot /= m;
        int c2 = pivot % (m*m); pivot /= m*m;
        int c1 = pivot;

        int i = 0, j = 0;
        while (i < sz(black) || j < sz(red)) {
            if (i < sz(black) && (j >= sz(red) || cmp(c1, c2, c3, black[i], red[j]))) {
                auto [w, u, v, _] = edges[black[i]];
                if (unite(u, v)) {
                    sol.pb(black[i]);
                }
                i++;
            } else {
                auto [w, u, v, _] = edges[red[j]];
                if (unite(u, v)) {
                    cnt_red++; 
                    sol.pb(red[j]);
                }
    ...
    // for (__int128 i = lo; i < hi; i++) {
        // ps(i, check(i));
    // }
    auto Pivot = fstFalse(lo, hi, [&](__int128 pivot) {return check(pivot) >= k;});
    if (Pivot == lo) { // not enough red edges
        ps(-1);
        return;
    }
    Pivot--;
    check(Pivot);
    ps(sz(sol));
    for (int i = 0; i < sz(sol); i++) cout << sol[i]+1 << " \n"[i==sz(sol)-1];
}
