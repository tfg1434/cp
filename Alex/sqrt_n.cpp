...
// There are three types of blocks. Up, Down, -
// Up blocks will always yield x+1 no matter the output of the prev block
// Down blocks will always yield x no matter the output of the prev block
// - blocks will yield x + (output of previous block had +1)
//
// Variables
// The pair x, t for each block
// Set of the blocks with t > 1 (non - blocks)

const int K = 6;
const int max_n = 2e5;
const int max_blocks = max_n / K + 1; 
int x[max_blocks], t[max_blocks];

// distance to the smallest square >= x
...
void solve() {
    def(int, n, q);
    vi a(n); re(a);
    int off = 0; // forgot this
    while (n % K) a.insert(bg(a), 0), n++, off++;

    auto status = [&](int b) {
        if (!b) return -1;
        if (x[b-1] >= t[b]) return 1;
        if (x[b-1]+1 < t[b]) return -1;
        assert(x[b-1] == t[b]+1);
        return 0;
    };
    ...
    auto update_block = [&](int b) {
        if (status(b) != 0) st.erase(b);
        x[b] = (int)sqrt(a[b*K]);
        for (int j = 1; j < K; j++) {
            x[b] = (int)sqrt(x[b] + a[b*K+j]);
        }
        t[b] = next_square(x[b]);
        if (status(b) != 0) st.insert(b);
    };

    for (int b = 0; b*K < n; b++) update_block(b);
    ps(st);
