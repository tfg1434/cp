...
int calc_intersection(V<array<int, 4>> a) {
    int n = sz(a);
    init_tree(2*n);
    ys.clear();
...
void solve() {
    ...
    int cur_x = 0, cur_y = 0;
    for (int i = 0; i < n; i++) {
        def(char, c); def(int, amnt);
        if (c == 'N') {
            a.pb({cur_x, cur_y, cur_x+k, cur_y+amnt+k});
            cur_y += amnt;
        }
        if (c == 'S') {
            a.pb({cur_x, cur_y-amnt, cur_x+k, cur_y+k});
            cur_y -= amnt;
        }
        if (c == 'E') {
            a.pb({cur_x, cur_y, cur_x+k+amnt, cur_y+k});
            cur_x += amnt;
        }
        if (c == 'W') {
            a.pb({cur_x-amnt, cur_y, cur_x+k, cur_y+k});
            cur_x -= amnt;
        }
    }

    ps(calc_intersection(a));
}
