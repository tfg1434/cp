// I got f'ed because I didn't put parentheses around r-R*K, so it's UB/RTE when you do bg(a)+r-R*K
// https://discord.com/channels/@me/1139658190099841105/1232775359171399862
//
//
...
const int max_n = 1e5;
const int K = 300;
int f[max_n/K+1][max_n];
deque<int> d[max_n / K+1];
...
void solve() {
    ...
    auto decode = [&](int x) {
        assert(x > 0);
        return (x+last_ans-1)%n + 1;
    };
    auto end_of_block = [&](int b) {
        return min(n, (b+1)*K);
    };
    ...
    rep(q) {
        ...
        if (t == 2) {
            def(int, k); k = decode(k); k--;
            last_ans = 0;
            if (L == R) {
                for (int i = l; i < r; i++) last_ans += d[L][i-L*K] == k;
            } else {
                // for (int i = l%K; i < sz(d[L]); i++) last_ans += d[L][i] == k;
                // for (int i = 0; i < r%K; i++) last_ans += d[R][i] == k;
                // for (int b = L+1; b < R; b++) last_ans += f[b][k];
                if (l != L*K) {
                    while (l < end_of_block(L)) {
                        last_ans += d[L][l%K] == k;
                        l++;
                    }
                    L++;
                ...
                for (int b = L; b < R; b++) last_ans += f[b][k];
            }
            ...
            if (L == R) {
                int last_elem = d[L][r-R*K - 1];
                d[L].erase(bg(d[L])+(r-1-L*K));
                d[L].insert(bg(d[L])+(l-L*K), last_elem);
                continue;
            }

            for (int b = L; b < R; b++) {
                int x = d[b].bk; d[b].pop_back(); f[b][x]--;
                d[b+1].push_front(x), f[b+1][x]++;
            }
            int x = d[R][(r - R*K)];
            d[R].erase(bg(d[R])+(r-R*K)); f[R][x]--;
            d[L].insert(bg(d[L])+(l-L*K), x); f[L][x]++;
        }
    }
}
