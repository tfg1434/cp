...
void solve() {
    ...
    F0R(i, n) {
        ...
        vl t(k); re(t);
        for (ll j = 0; j < k; j++) {
            if (j > 0 && t[j-1]+1 == t[j]) bad_times[i].bk.s++;
            else bad_times[i].eb(t[j], t[j]+1);
        }
    }
    ...
    while (sz(q)) {
        ...
        auto idx = upb(bad_times[u], {d, BIG})-1;
        ll extra = 0;
        if (0 <= idx && idx < sz(bad_times[u])) 
            extra = max(0ll, bad_times[u][idx].s - d);
        for (auto[w, v] : g[u]) if (ckmin(dist[v], d+w+extra)) {
            q.push({dist[v], v});
        }
    }

    if (dist[n-1] == BIG) dist[n-1] = -1;
    ps(dist[n-1]);
}
