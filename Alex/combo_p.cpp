// MLE on test 21; I think it's the N <= 1e5 101 test
...
void solve() {
    ...
    vl f = {1};
    for (ll i = 1; i <= n; i++) f.pb(f.bk*i%P);
    vl ff = {pw(f.bk, P-2)};
    for (ll i = n; i >= 1; i--) ff.pb(ff.bk*i % P);
    reverse(all(ff));
    assert(f[n]*ff[n] % P == 1);
    auto C = [&](ll i, ll j) {
        return f[i]*ff[j]%P*ff[i-j]%P;
    };
    ...
    // dp[i][j] = processed first i elems, ASSIGNED j good pairs
    // if it's bad, we don't assign it
    V<vl> dp(n+1, vl(n+1));
    dp[0][0] = 1;
    for (ll i = 0; i < n; i++) for (ll j = 0; j <= i; j++) {
        (dp[i+1][j] += dp[i][j]) %= P;
        (dp[i+1][j+1] += max(0ll, x[i] - j) * dp[i][j]) %= P;
    }

    vl ans(n+1);
    for (ll good = n; good >= 0; good--) {
        for (ll k = good+1; k <= n; k++)
            ans[good] = ((ans[good]-ans[k]*C(k, good))%P+P)%P;
        (ans[good] += dp[n][good] * f[n-good]) %= P;
    }

    for (ll good = m; good <= n; good++) cout << ans[good] << " \n"[good==n];
}
