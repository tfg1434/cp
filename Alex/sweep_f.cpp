...
// false if there exists intersection in A that is not in B,
// else true
bool work(V<array<int, 4>> s) {
    int n = sz(s);
    bool res = true;
    V<array<int, 3>> events; // x, type, ind
    for (int i = 0; i < n; i++) {
        auto& [sa, ea, sb, eb] = s[i];
        ea++; eb++;
        events.pb({sa, 1, i});
        events.pb({ea, 0, i});
    }
    ...
    for (auto [x, type, ind] : events) {
        auto [sa, ea, sb, eb] = s[ind];
        if (type == 1) {
            if (sz(ls)) {
                res &= sb < *bg(rs) && *rbegin(ls) < eb;
            }
            rs.insert(eb);
            ls.insert(sb);
        } else {
