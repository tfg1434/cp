...
void init_tree(int m) {
    int size = 1;
    while (size < 2*m) size *= 2;
    t.resize(size); // we don't need to set to -inf!
    buf.resize(size);
    build(0, 0, m);
}
...
void solve() {
    ...
    V<array<int, 3>> c(p); 
    for (int i = 0; i < p; i++) {
        re(c[i][0], c[i][1], c[i][2]); // defense, attack, value
    }
    ...
    for (int i = 0; i < n; i++) events.pb({a[i].f, 0, i});
    for (int i = 0; i < p; i++) events.pb({c[i][0], 1, i});
    sort(all(events));
    for (auto [x, type, ind] : events) {
        if (type == 1) {
            range_add(0, 0, m, lwb(b, {c[ind][1], INF}), m, c[ind][2]);
        } else {
            ckmax(ans, t[0] - a[ind].s);
        }
    }
    ...
    // int j = 0;
    // for (int i = 0; i < n; i++) {
        // while (j < p && a[i].f > c[j][0]) {
            // range_add(0, 0, m, lwb(b, {c[j][1], INF}), m, c[j][2]);
            // j++;
        // }
        // ckmax(ans, t[0]-a[i].s);
    // }
    ...
    // Solution 3
    // I don't know why this doesn't work (i think a bug)
    //
    // sort(all(a));
    // sort(all(c), [&](const array<int, 3>& x, const array<int, 3>& y) {return x[1] < y[1];});
    // int i = 0;
    // for (auto [A, B, val] : c) {
        // while (i < sz(a) && a[i].f <= B) i++;
        // if (i == sz(a)) break;
        // range_add(0, 0, m, lwb(b, {A, INF}), m, val);
        // ckmax(ans, t[0] - a[i].s);
    // }
