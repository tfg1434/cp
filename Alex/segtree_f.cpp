...
struct max_tree {
    ...
    V<node_info> t;
    void build(ll v, ll l, ll r, const vl& a) {
        if (l == r-1) {
            t[v] = node_info(a[l], l);
            return;
        }
    ...
    void set(ll v, ll l, ll r, ll i, ll x) {
        if (l == r-1) {
            t[v] = node_info(x, i);
            return;
        }
...
void solve() {
    def(ll, n, q);
    vl a(n); re(a);
    auto MAX = max_tree(n, a);
    auto SUM = sum_tree(n, a);

    // ps(MAX.get_max(0, 0, n, 0, 3).mx);
    // ps(SUM.get_sum(0, 0, n, 0, 3));

    rep(q) {
        def(ll, t);
        if (t == 1) {
            def(ll, l, r); l--;
            ps(SUM.get_sum(0, 0, n, l, r));
        } else if (t == 2) {
            def(ll, l, r, mod); l--;
            auto mx = MAX.get_max(0, 0, n, l, r);
            while (mx.mx >= mod) {
                SUM.set(0, 0, n, mx.where, mx.mx%mod);
                MAX.set(0, 0, n, mx.where, mx.mx%mod);
                mx = MAX.get_max(0, 0, n, l, r);
            }
        } else {
            def(ll, i, x); i--;
            SUM.set(0, 0, n, i, x);
