...
struct node {
    ...
    friend ostream& operator <<(ostream& out, const node& me) {
        out << "Min: ";
        for (int i = 0; i < k; i++) out << me.mn[i] << " \n"[i==k-1];
        out << "Max: ";
        for (int i = 0; i < k; i++) out << me.mx[i] << " \n"[i==k-1];
        return out;
    }
};
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        node t;
        t.siz = 1;
        for (int j = 0; j < k; j++) t.mn[j] = t.mx[j] = a[i][j];

        set<node>::iterator it;
        while ((it = s.find(t)) != end(s)) {
            for (int j = 0; j < k; j++) {
                ckmin(t.mn[j], it->mn[j]);
                ckmax(t.mx[j], it->mx[j]);
            }
            t.siz += it->siz;
            s.erase(it);
        }
