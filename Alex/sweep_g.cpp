...
void solve() {
    ...
    for (auto it = bg(slopes); it != prev(end(slopes)); it++) {
        auto nxt = next(it);
        nxt->s += it->s;
    }
    ...
    vi bad_centers;
    map<int, int> heights;
    heights[bg(slopes)->f] = 0;
    for (auto it = bg(slopes); it != prev(end(slopes)); it++) {
        auto nxt = next(it);
        int h = heights[it->f] + (nxt->f - it->f)*(it->s);
        heights[nxt->f] = h;
        if (h > m) bad_centers.pb(nxt->f);
    }

    int max_r = -INF, max_l = -INF;
    for (auto j : bad_centers) {
        ckmax(max_r, heights[j]-m-j);
        ckmax(max_l, heights[j]-m+j);
    }

    for (int i = 0; i < n; i++) {
        auto [x, y] = rain[i];
        bool good = y >= x+max_r && y >= -x+max_l;
        pr(good ? "1" : "0");
    }
    ps();
}
