...
void solve() {
    ...
    if (n <= 17) {
        V<vl> dp(n+1, vl(p2(n)));
        dp[0][0] = 1;
        for (ll i = 0; i < n; i++) for (ll msk = 0; msk < p2(i); msk++) {
            // the (i+1th) bit pairs with index j in b
            for (ll j = 0; j <= i; j++) {
                // the (i+1)-th bit is on
                if (a[i] > b[j]) {
                    if (((msk & p2(j)) && a[j] > b[i]) || 
                            (((msk & p2(j)) == 0) && a[j] <= b[i])) {
                        (dp[i+1][msk | p2(i)] += dp[i][msk]) %= P;
                    }
                }

                // the (i+1)-th bit is off
                if (a[i] <= b[j]) {
                    if (((msk & p2(j)) && a[j] > b[i]) || 
                            (((msk & p2(j)) == 0) && a[j] <= b[i])) {
                        (dp[i+1][msk] += dp[i][msk]) %= P;
                    }
                }
            }
        }

        for (ll msk = 0; msk < p2(n); msk++) {
            (ans[pct(msk)] += dp[n][msk]) %= P;
        }
    }
