...
const int K = 300;
// precomp
int inter[max_n / K][max_n];
// variables
int large_cont[max_n/K], small_cont[max_n/K]; // large_cont[i] + small_cont[i] = sum of large_ind[i]
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        def(int, c);
        sets[i].resize(c); re(sets[i]);
        for (auto& x : sets[i]) x--;
        sort(all(sets[i]));
        if (sz(sets[i]) >= K) large_ind.pb(i);
    }

    for (int i = 0; i < sz(large_ind); i++) {
        for (int j = 0; j < m; j++) {
            vi res;
            auto st = set_intersection(all(sets[large_ind[i]]), all(sets[j]), back_inserter(res));
            inter[i][j] = sz(res);
        }
    }

    auto add_small = [&](int i, int x) {
        assert(sz(sets[i]) < K);
        for (auto j : sets[i]) a[j] += x;
        for (int j = 0; j < sz(large_ind); j++) small_cont[j] += x*inter[j][i];
    };

    auto add_large = [&](int i, int x) {
        assert(sz(sets[i]) >= K);
        auto I = lwb(large_ind, i);
        for (int j = 0; j < sz(large_ind); j++) large_cont[j] += inter[I][large_ind[j]]*x;
    };

    auto query_small = [&](int i) {
        assert(sz(sets[i]) < K);
        int res = 0;
        for (auto j : sets[i]) res += a[j];
        for (int j = 0; j < sz(large_ind); j++) res += large_cont[j]*inter[j][i];
        return res;
    };

    auto query_large = [&](int i) {
        assert(sz(sets[i]) >= K);
        auto I = lwb(large_ind, i);
        return large_cont[I] + small_cont[I];
    };
    ...
    rep(q) {
        ...
        if (c == '?') {
            def(int, i); i--;
            if (sz(sets[i]) >= K) ps(query_large(i));
            else ps(query_small(i));
        } else {
            def(int, i, x); i--;
            if (sz(sets[i]) >= K) add_large(i, x);
            else add_small(i, x);
        }
    }
}
