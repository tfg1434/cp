...
vl t;
// sum on the interval that intersects with [0, k)
ll order_of_key(ll v, ll l, ll r, ll k) {
    if (r <= 0 || k <= l) return 0;
    if (0 <= l && r <= k) return t[v];
    ll m = (l+r)/2;
    return order_of_key(2*v+1, l, m, k) + order_of_key(2*v+2, m, r, k);
}
// the (k+1)-th on index
ll find_by_order(ll v, ll l, ll r, ll k) {
    assert(t[v] >= k+1);
    if (l == r-1) return l;
    ll m = (l+r)/2;
    if (t[2*v+1] >= k+1) return find_by_order(2*v+1, l, m, k);
    else return find_by_order(2*v+2, m, r, k-t[2*v+1]);
}
void toggle_idx(ll v, ll l, ll r, ll i) {
    if (l == r-1) {
        t[v] ^= 1;
        return;
    }
    ll m = (l+r)/2;
    if (i < m) toggle_idx(2*v+1, l, m, i);
    else toggle_idx(2*v+2, m, r, i);
    t[v] = t[2*v+1] + t[2*v+2];
}
...
vl index(ll n, const vl& a) {
    vl res(n);
    build(0, 0, n);
    for (ll i = 0; i < n; i++) {
        res[i] = order_of_key(0, 0, n, a[i]);
        toggle_idx(0, 0, n, a[i]);
    }
    return res;
}

vl construct(ll n, vl idx) {
    vl perm(n);
    build(0, 0, n);
    for (ll i = 0; i < n; i++) {
        perm[i] = find_by_order(0, 0, n, idx[i]);
        toggle_idx(0, 0, n, perm[i]);
    }
    return perm;
}
...
void solve() {
    ...
    vl ans = construct(n, I);
    for (ll i = 0; i < n; i++) pr(ans[i], " \n"[i==n-1]);
}
