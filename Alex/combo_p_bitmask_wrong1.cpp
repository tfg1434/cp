...
void solve() {
    ...
    if (n <= 17) {
        ...
        for (ll msk = 0; msk < p2(n); msk++) {
            vl v;
            for (ll j = 0; j < n; j++) if (msk & p2(j)) v.pb(b[j]);
            sort(rall(v));
            ll len = sz(v);
            for (ll j = 0; j < n; j++) if ((msk & p2(j)) == 0) v.pb(b[j]);
            sort(bg(v)+len, end(v));

            ll ways = 1;
            for (ll j = 0; j < len; j++) ways *= max(0ll, n - upb(a, v[j]) - j);
            for (ll j = len; j < n; j++) ways *= max(0ll, upb(a, v[j]) - (j-len));
            ans[pct(msk)] += ways;
        }
    }
