// the same as .cpp i just removed a variable
...
void solve() {
    ...
    auto add_small = [&](int i, int x) {
        ...
        for (int j = 0; j < sz(large_ind); j++) sum[j] += x*inter[j][i];
    };
    ...
    auto query_small = [&](int i) {
        ...
        for (int j = 0; j < sz(large_ind); j++) res += sum[j]*inter[j][i];
        return res;
    };
    ...
    auto query_large = [&](int i) {
        assert(sz(sets[i]) >= K);
        int res = 0;
        for (int j = 0; j < sz(large_ind); j++) res += inter[j][i]*sum[j];
        return res;
    };
