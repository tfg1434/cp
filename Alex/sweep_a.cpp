...
int find_max(int v, int l, int r) {
    if (l == r-1) return l;

    int m = (l+r)/2;
    propagate(2*v+1, l, m);
    propagate(2*v+2, m, r);
    if (t[2*v+1] > t[2*v+2]) return find_max(2*v+1, l, m);
    return find_max(2*v+2, m, r);
}
...
void solve() {
    ...
    vi ys;
    for (auto y : y0) ys.pb(y);
    for (auto y : y1) ys.pb(y);
    sort(all(ys));
    for (auto& y : y0) y = lwb(ys, y);
    for (auto& y : y1) y = lwb(ys, y);
    ...
    for (int i = 0; i < n; i++) {
        events.pb({x0[i], 1, i}); 
        events.pb({x1[i], -1, i});// closing = -1 so it will go first
    }
    sort(all(events));
    ...
    for (int i = 0; i < sz(events); i++) {
        ...
        range_add(0, 0, sz(ys), y0[ind], y1[ind], type);

        if (ckmax(ans, t[0])) {
            sol = {x, ys[find_max(0, 0, sz(ys))]};
        }
    }
