...
const int max_n = 1e5;
const int K = 300;
const int max_gap = max_n / K + 5;
int buckets[max_n * (K + 1)];
gp_hash_table<int, int, splitmix64> dp[max_n];
int g[max_n][max_gap];
...
void solve() {
    ...
    int ans = 0;
    auto solve = [&](const vi& a) {
        for (int d = 0; d <= K; d++) {
            for (int i = 0; i < n; i++) {
                ckmax(ans, ++buckets[a[i]-d*i + (n-1)*d]);
            }
            for (int i = 0; i < n; i++) {
                buckets[a[i]-d*i + (n-1)*d]--;
            }
        }

        for (int i = 0; i < n; i++) {
            for (int j = 1; j < min(max_gap, n-i); j++) {
                if ((a[i+j]-a[i]) % j == 0) {
                    int d = (a[i+j]-a[i])/j;
                    if (d > K) {
                        g[i][j] = true;
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j <= min(max_gap, i); j++) if (g[i-j][j]) {
                int d = (a[i]-a[i-j])/j;
                assert((a[i]-a[i-j]) % j == 0);
                assert(d > K);

                ckmax(dp[i][d], dp[i-j][d]+1);
                ckmax(ans, dp[i][d]+1);
            }
        }

        for (int i = 0; i < n; i++) dp[i].clear();
        for (int i = 0; i < n; i++) for (int j = 1; j < max_gap; j++) g[i][j] = false;
    };
