...
struct segtree {
    ...
    vl buf;
    segtree() {}
    segtree(ll n, const str& s, char c) {
        ll size = 1;
    ...
    void build(ll v, ll l, ll r, const str& s, char c) {
        if (l == r-1) {
            t[v] = s[l] == c;
            return;
        }
    ...
    ll range_sum(ll v, ll l, ll r, ll L, ll R) {
        p(v, l, r);
        if (r <= L || R <= l) return 0;
    ...
    void range_set(ll v, ll l, ll r, ll L, ll R, ll x) {
        p(v, l, r);
        if (r <= L || R <= l) return;
        if (L <= l && r <= R) {
            buf[v] = x;
...
void solve() {
    ...
    def(str, s);
    V<segtree> trees(26);
    for (ll i = 0; i < 26; i++) trees[i] = segtree(n, s, 'a'+i);

    // ps(trees[25].range_sum(0, 0, n, 0, n));
    // trees[0].range_set(0, 0, n, 0, 3, 1);
    // trees[0].range_set(0, 0, n, 6, 7, 1);
    // ps(trees[0].range_sum(0, 0, n, 0, 7));
    // return;
    ...
    rep(q) {
        ...
        if (t == 1) {
            vl cnt;
            for (ll i = 0; i < 26; i++) {
                cnt.pb(trees[i].range_sum(0, 0, n, l, r));
                trees[i].range_set(0, 0, n, l, r, 0);
            }
            ll cur = l;
            for (ll i = 0; i < 26; i++) {
                trees[i].range_set(0, 0, n, cur, cur+cnt[i], 1);
                cur += cnt[i];
            }
        } else {
            vl cnt;
            ...
            ll cur = l;
            for (ll i = 0; i < 26; i++) {
                trees[25-i].range_set(0, 0, n, cur, cur+cnt[i], 1);
                cur += cnt[i];
            }
        }
        // str ans(n, '_');
        // for (ll i = 0; i < n; i++) {
            // for (ll j = 0; j < 26; j++) if (trees[j].range_sum(0, 0, n, i, i+1)) {
                // ans[i] = 'a'+j;
                // break;
            // }
        // }
