...
void solve() {
    ...
    F0R(i, m) {
        def(ll, u, v, w); u--; v--;
        g[u].pb({w, v, i}); g[v].pb({w, u, i});
    }
    def(ll, s); s--;
    // calculate dist...
    vl dist(n, BIG);
    pqg<pl> q; q.push({0, s}); dist[s] = 0;
    while (sz(q)) {
        auto [d, u] = q.top(); q.pop();
        if (d != dist[u]) continue;
        for (auto [w, v, _] : g[u]) if (ckmin(dist[v], d+w)) {
            q.push({dist[v], v});
        }
    }

    V<V<Edge>> dag(n), rev(n);
    F0R(i, n) for (auto[w, j, idx] : g[i]) if (dist[j] == dist[i] + w) {
        dag[i].pb({w, j, idx});
        rev[j].pb({w, i, idx});
    }
    ...
    F0R(i, n) if (sz(rev[i])) {
        ll mn_w = BIG, mn_idx;
        for (auto[w, j, idx] : rev[i]) {
            if (ckmin(mn_w, w)) mn_idx = idx;
        }
        sum += mn_w;
        sol.pb(mn_idx);
    }
