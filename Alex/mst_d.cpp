...
void solve() {
    ...
    for (auto [w, edge_list] : edges) {
        ...
        unordered_map<int, int> lvl, up;
        {
            unordered_map<int, bool> vis;
            auto dfs = yy([&](auto rec, int u, int p) -> void {
                if (p != -1) lvl[u] = lvl[p]+1;
                vis[u] = true;
                for (auto v : g[u]) if (!vis[v]) rec(v, u);
            });
            for (auto u : cc) if (!vis[u]) dfs(u, -1);
        }
        {
            unordered_map<int, bool> vis;
            auto dfs = yy([&](auto rec, int u, int p) -> void {
                ...
                if (cnt_par > 1) ckmin(up[u], lvl[u]-1);
            });
            for (auto u : cc) if (!vis[u]) dfs(u, -1);
        }
        ...
        for (auto [u, v, ind] : edge_list) {
            ...
            if (u == v) {
                ...
                // ps(u, v, lvl[u], lvl[v], up[v]);
                ans[ind] = up[v] < lvl[v] ? "at least one" : "any";
            }
        }
