...
struct chash {
    size_t operator()(pi p) const {
        return (p.f*9973 + p.s) % (int)(1e9+9);
    }
};
...
//
// Variables
// Answer for the pair (u, v) where (u, v) is queried
// DSU of the nodes connected by this color edge
...
void solve() {
    ...
    V<vpi> edges(m);
    rep(m) {
        def(int, u, v, col); u--; v--; col--;
        edges[col].eb(u, v);
    }

    def(int, q);
    unordered_map<pi, int, chash> ans; // map of (u < v) and answer
    vpi queries; // queries from input. used only when printing the answer
    for (int i = 0; i < q; i++) {
        def(int, u, v); u--; v--;
        if (u > v) swap(u, v);
    ...
    for (int col = 0; col < m; col++) {
        if (sz(edges[col]) > K) {
            // O(N)
            for (auto [u, v] : edges[col]) unite(u, v);
            for (auto& [uv, res] : ans) {
                auto [u, v] = uv;
                res += find(u) == find(v);
            }
            ...
            for (int i = 0; i < sz(nodes); i++) for (int j = i+1; j < sz(nodes); j++) if (find(nodes[i]) == find(nodes[j])) {
                if (ans.count({nodes[i], nodes[j]})) ans[{nodes[i], nodes[j]}]++;
            }

            for (auto u : nodes) e[u] = -1;
        }
    }
