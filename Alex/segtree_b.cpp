...
ll array_inv(const vl& a, const vl& b) {
    ll res = 0;
    ll j = 0;
    for (ll i = 0; i < sz(a); i++) {
...
node_info merge_node(const node_info& a, const node_info& b) {
    ...
    res.cnt = a.cnt + b.cnt + array_inv(a.a, b.a);
    return res;
}
...
node_info query(ll v, ll l, ll r, ll L, ll R) {
    ...
    ll m = (l+r)/2;
    auto left = query(2*v+1, l, m, L, R);
    auto right = query(2*v+2, m, r, L, R);
...
void solve() {
    ...
    ll ans = 0;
    for (ll l = 0; l < n; l++) {
        ll R = lstTrue(l+1, n, [&](ll r) {
            return query(0, 0, n, l, r).cnt <= k;
        });
        ans += R-l-1;
    }
    ps(ans);
