...
int emax[max_n];
bool del[max_m*2];
int fa[max_n], id[max_m*2]; 
int he[max_n], ne[max_m*2], to[max_m*2], we[max_m*2];
int c = 0;

void add_edge(int l, int u, int v) {
    ne[c] = he[u]; he[u] = c; to[c] = v; we[c++] = l;
    ne[c] = he[v]; he[v] = c; to[c] = u; we[c++] = l;
}

void get_max(int u, int p) {
    dbg(u, p);
    for (int i = he[u]; i != -1; i = ne[i]) {
        if (to[i] == p || del[i]) continue;
        if (we[i] > we[emax[p]]) emax[to[i]] = i;
        else emax[to[i]] = emax[p];
        get_max(to[i], u);
    }
}
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (unite(u, v)) {
            add_edge(w, u, v); sol.pb(i);id[c-1] = id[c-2] = i;
        }
    }
    sort(all(edges));
    ...
    if (k < 0) return (void) ps(-1);
    for (int i = 0; i < n-1; i++) if (find(i) != find(i+1)) return (void) ps(-1);
    
    rep(k) {
        if (!sz(INC_EDGES)) return (void) ps(-1);
        for (int i = he[0]; i != -1; i = ne[i]) {
            get_max(to[i], 0);
        }

        int mn = INF, pos = -1;
        for (int i = 0; i < sz(INC_EDGES); i++) {
            auto [w, u, v] = edges[INC_EDGES[i]];
            if (ckmin(mn, w-we[emax[max(u, v)]])) {
                pos = i;
            }
        }
        auto [w, u, v] = edges[INC_EDGES[pos]];
        add_edge(w, u, v); sol.pb(INC_EDGES[pos]);
        del[emax[max(u, v)]] = del[emax[max(u, v)]^1] = true;
        ps(id[emax[max(u, v)]]);
        ps(sol);
        sol.erase(find(all(sol), id[emax[max(u, v)]]));
        INC_EDGES.erase(bg(INC_EDGES) + pos);
    }

    ps(n-1);
    for (int i = 0; i < n-1; i++) cout << sol[i] + 1 << " \n"[i==n-2];
}
