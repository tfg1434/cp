...
void solve() {
    ...
    auto sp = [&]() {
        ...
        while (sz(q)) {
            ...
            for (auto v : g[u]) if (ckmin(dist[v], d+w[u][v])){
                q.push({dist[v], v});
            }
        }
        return dist;
    ...
    vpl inf_edges;
    for (auto[u, v] : es) if (is_inf[u][v]) {
        w[u][v] = w[v][u] = 1;
        inf_edges.eb(u, v);
    }

    ll Delta = fstTrue(0ll, (ll)(1e9-1)*sz(inf_edges), [&](ll delta){
        for (auto[u, v] : inf_edges) w[u][v] = w[v][u] = 1;
        for (auto[u, v] : inf_edges) {
            if (!delta) break;
            w[u][v] += min(delta, (ll)1e9-1);
            w[v][u] += min(delta, (ll)1e9-1); // tick tock
            delta -= w[u][v]-1;
        }
