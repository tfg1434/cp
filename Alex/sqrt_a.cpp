// Passes
// Had to remove recursion and use int
...
// Variables
// The number of jumps before it leaves the block (including the jump out)
// Which element it will go to when it leaves the block
//
// Reverse adj list only of those in the same block

const int K = 300;
const int max_n = 1e5;
int jumps[max_n], which[max_n];

void solve() {
    def(int, n, q);
    vi a(n); re(a);
    V<vi> rev(n);
    for (int i = 0; i < n; i++) if (i+a[i] < n && (i+a[i])/K == i/K)
        rev[i+a[i]].pb(i);
    for (int i = 0; i < n; i++) {
        int j;
        for (j = i; j/K == i/K && j < n; j += a[j]) { jumps[i]++; }
        which[i] = j;
    }
    ...
    rep(q) {
        ...
        if (t == 0) {
            def(int, i, x); i--;
            if (i+a[i] < n && (i+a[i])/K == i/K) rev[i+a[i]].erase(find(all(rev[i+a[i]]), i));
            if (i+x < n && (i+x)/K == i/K) rev[i+x].pb(i);
