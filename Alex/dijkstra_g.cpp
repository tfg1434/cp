...
void solve() {
    ...
    V<vpl> g(n);
    V<tuple<ll, ll, ll>> es;
    map<tuple<ll, ll, ll>, ll> cnt_edge;
    F0R(i, m) {
        def(ll, u, v, w); u--; v--;
    ...
    vl dist_s(n, BIG);
    {
        pqg<pl> q; q.push({0, s}); dist_s[s] = 0;
        while (sz(q)) {
            auto [d, u] = q.top(); q.pop();
            if (d != dist_s[u]) continue;
            for (auto [w, v] : g[u]) if (ckmin(dist_s[v], d+w)) {
                q.push({dist_s[v], v});
            }
        }
    }
    vl dist_t(n, BIG);
    {
        pqg<pl> q; q.push({0, t}); dist_t[t] = 0;
        while (sz(q)) {
            auto [d, u] = q.top(); q.pop();
            if (d != dist_t[u]) continue;
            for (auto [w, v] : g[u]) if (ckmin(dist_t[v], d+w)) {
                q.push({dist_t[v], v});
            }
        }
    }
    ...
    for (auto[w, u, v] : es) {
        ...
        if (dist_s[u]+dist_t[v]+w==dist_s[t]) {
            dag[u].pb(v);
            rev[v].pb(u);
            dag_edges.ins({u, v});
            dag_edges.ins({v, u});
    ...
    for (auto [w, u, v] : es) {
        if (dist_s[u] > dist_s[v]) swap(u, v);
        ll mx = dist_s[t] - dist_s[u] - dist_t[v];
        if (mx <= 0) {
            ps("NO");
            continue;
        }
        ll prod = cnt_s[u]*cnt_t[v];

        if (w == mx && dag_edges.count({u, v}))  {
            if (cnt_edge[{w, u, v}] == 1 && prod == cnt_s[t]) {
                ps("YES");
            } else {
                if (mx > 1) ps("CAN", 1);
                else ps("NO");
