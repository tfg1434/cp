...
node_info merge_node(node_info a, node_info b) {
    node_info res;
    ll extra = min(a.plus, b.minus);
    res.bbs = a.bbs + b.bbs + 2*extra;
    res.plus = a.plus + b.plus - extra;
    res.minus = a.minus + b.minus - extra;
    return res;
}

V<node_info> t;

void build(ll v, ll l, ll r, const str& s) {
    if (l == r-1) {
        t[v] = node_info(s[l]);
        return;
    }
