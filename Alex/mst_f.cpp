...
void solve() {
    ...
    map<int, unordered_map<int, vi>> queries;
    def(int, q);
    for (int i = 0; i < q; i++) {
        def(int, k);
        rep(k) {
            def(int, edge_ind); edge_ind--;
            queries[all_edges[edge_ind][0]][i].pb(edge_ind);
        }
    }

    vb good(q, true);
    for (auto [w, edge_inds] : edges) {
        int photo = snapshot();
        for (auto [query_ind, query_edges] : queries[w]) {
            for (auto edge_ind : query_edges) {
                auto [_, u, v] = all_edges[edge_ind];
                if (find(u) != find(v)) {
                    unite(u, v);
                } else {
                    good[query_ind] = false;
                }
            }
            undo(photo);
        }

        for (auto ind : edge_inds) {
            auto [_, u, v] = all_edges[ind];
            if (find(u) != find(v)) {
                unite(u, v);
            }
        }
    ...
    for (int i = 0; i < q; i++) ps(good[i] ? "YES" : "NO");
}
