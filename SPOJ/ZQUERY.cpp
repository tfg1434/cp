// .c++ is WA but the closer code, here i didn't realise
// you should use deque!
//
...
void solve() {
    ...
    vb vis(n+1);
    unordered_map<int, int> lmost, rmost;
    auto upd_r = [&](int i) {
        if (!vis[i]) {
            // add p[i] to interval
            if (!lmost.count(p[i])) lmost[p[i]] = i;
            ckmin(lmost[p[i]], i);
            lengths.insert(i - lmost[p[i]]);
        } else {
            // remove p[i] from interval
            lengths.erase(lengths.find(i - lmost[p[i]]));
            if (lmost[p[i]] == i) lmost.erase(p[i]);
        }
        vis[i] = !vis[i];
    };
    auto upd_l = [&](int i) {
        if (!vis[i]) {
            if (!rmost.count(p[i])) rmost[p[i]] = i;
            ckmax(rmost[r[i]], i);
            lengths.insert(rmost[p[i]] - i);
        } else {
            lengths.erase(lengths.find(rmost[p[i]] - i));
            if (rmost[p[i]] == i) rmost.erase(p[i]);
        }
        vis[i] = !vis[i];
    };
