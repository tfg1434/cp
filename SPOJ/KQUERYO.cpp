...
ll count_gtk(ll v, ll l, ll r, ll L, ll R, ll k) {
    if (r <= L || R <= l) return 0;
    if (L <= l && r <= R) return r-l - upb(t[v].a, k);
    ll m = (l+r)/2;
    return count_gtk(2*v+1, l, m, L, R, k) +
           count_gtk(2*v+2, m, r, L, R, k);
}
