...
const ll N = 5e3+5, WW = 1e4+5;
ll dp[N][WW], dp2[N][WW];

// dp(i, w) = best answer if you have w dollars and only buy items in 1..i
// dp2(i, w) = number of items to buy optimally

// dp(0, 0) = a[1]+a[2]+...+a[k]

// dp(i, w) = max 
// dp(i-1, w-ai)+a[k+dp2(i-1, w-ai)]
// dp(i-1, w)

// dp2(i, w) = whichever gave the better answer
void solve() {
    ll n, w, k; re(n, w, k);
    vpl vw(n+1);
    f1(i, n) re(vw[i].s, vw[i].f);

    sort(rall(vw)-1);
    gg(vw);
    f1(i, k) dp[0][0] += vw[i].f;
    f1(i, n) dp[i][0] = dp[0][0];
    ...
    FOR(i, 1, n+1) FOR(j, 0, w+1) {
        ...
        ll c1 = 0, two = 0;
        if (j>=vw[i].s && k+dp2[i-1][j-vw[i].s]+1 <= n) {
            two = dp2[i-1][j-vw[i].s];
            c1 = dp[i-1][j-vw[i].s]+vw[k+two+1].f;
        } 
