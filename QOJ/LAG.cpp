...
void solve() {
    ...
    V<db> dp(n+1); // dp[i] = E(coins collected if ur at the i-th elem)
    V<db> prob(n+1); // prob[i] = P(you are at the i-th elem)
    V<vl> sums(n+1);
    for (ll i = 1; i <= n; i++) sums[i].resize(n/i + 1);
    for (ll i = 1; i <= n; i++) {
        for (ll j = 1; j*j <= i; j++) if (i%j == 0) {
            sums[j][i/j] += w[i-1];
            if (j*j != i) sums[i/j][j] += w[i-1];
        }
    }
    for (ll i = 1; i <= n; i++) {
        for (ll j = sums[i].size()-1; j > 0; j--) {
            for (ll k = 2; k*j*i <= n; k++) {
                if (j%gcd(k*j, i) == 0)
                    sums[i][k*j] += sums[i][j];
            }
        }
    ...
    for (ll i = 1; i <= n; i++) {
        dp[i] += sums[1][i] / tot;
        ll newtot = tot-sums[1][i];
        db leave = 1.0;

        for (ll j = 2; j*i <= n; j++) {
            db p = sums[j][i] / newtot;
            dp[i*j] += dp[i]*p;
            prob[i*j] += prob[i]*p;
            leave -= p;
        }

        ans += prob[i]*leave*dp[i];
    }
