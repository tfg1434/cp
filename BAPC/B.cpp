...
// You have to use a different base here!
uint64_t hash_pair(pl p) {
    // return (base*p.f%mod + p.s) % mod; 
    return (9973*p.f%mod + p.s) % mod; 
}
...
struct PolyHash{
    ...
    PolyHash(const vector<T>& ar){
        ...
        for (int i = 1; i <= n; i++){
            pref[i] = modmul(pref[i - 1], base) + hash_pair(ar[i - 1]) + 997;
...
map<uint64_t, vpl> testhash;

unordered_map<uint64_t, ll> memo[2]; // init to -1
ll dp(ll prv, vpl& ff) {
    ll taken = n;
    for (auto[x, f] : ff) taken -= x*f;

    if (!sz(ff)) return 0;

    ll h = PolyHash(ff).get_hash(0, sz(ff)-1);
    // if (testhash.count(h)) {
        // ps(testhash[h]);
        // ps(ff);
        // assert(testhash[h] == ff);
    // } 
    if (memo[prv].count(h)) return memo[prv][h];

    ll mn = BIG;
    F0R(i, sz(ff)) {
        ll pfs = pre[taken+ff[i].f]-pre[taken];
        ll cost;
        if (prv == 1) cost = pfs;
        else cost = ff[i].f-pfs;

        vpl F = ff;
        F[i].s--;
        if (!F[i].s) F.erase(bg(F)+i);
        ckmin(mn, dp(1-prv, F)+cost);
    }
...
void solve() {
    re(n, m);
    a = vl(n); re(a);
    pre = vl(n+1); partial_sum(all(a), bg(pre)+1);
    b = vl(m); re(b);
    map<ll, ll> _; each(x, b) _[x]++;

    auto lval = vpl(all(_));
    ll ans = min(dp(0, lval), dp(1, lval));
    ps(ans);
}
...
signed main() {
    ...
    // vpl a {{1, 1}, {3, 1}};
    // vpl b {{1, 2}, {2, 1}};
    // ps(PolyHash(a).get_hash(0, 1));
    // ps(PolyHash(b).get_hash(0, 1));
