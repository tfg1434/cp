...
void gen(vector<vector<int>> &tp, vector<int> &mx, int ind, vector<int> sf) {
...
int main() {
  ...
  vector<vector<int>> of = states;
  reverse(states.begin(), states.end());

  int CN = of.size() - 1;

  vector<vector<int>> dp(2, vector<int>(states.size(), (int)1e9));
  dp[0][CN] = 0;
  dp[1][CN] = 0;
  ...
  for (auto S : states) {
    ...
    int s = lower_bound(of.begin(), of.end(), S) - of.begin();
    ...
    for (int i = 0; i < S.size(); i++) {
      ...
      int ni = lower_bound(of.begin(), of.end(), S) - of.begin();

      for (int j = 0; j < 2; j++) {
        int cost = a[ind + v[i]] - a[ind];
        if (j == 0) cost = (v[i] - cost);

        dp[j ^ 1][ni] = min(dp[j ^ 1][ni], dp[j][s] + cost);
      }
