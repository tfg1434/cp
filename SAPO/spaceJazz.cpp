...
const ll P = (int)1e9+7; // 998244353;
const ll MX = (int)2e5+5;
...
int main() {
    ...
    string s; re(s); 
    ll n = s.size();
    V<vl> dp(n, vl(n, 1)); //dp(i,i)=1
    FOR(i, 1, n) { //the length - 1
        //starting index
        f0(j, n-i) {
            FOR(k, j, j+i+1) { //all possible indices between
                // ASSUME it's optimal to match j with k
                dp[j][j+i] = min(dp[j][j+i], dp[j][max(j, k-1)] + dp[min(j+i,k+1)][j+i]);
            }
        }
    }
