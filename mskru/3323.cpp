...
ll find_kth_zero(ll v, ll l, ll r, ll L, ll R, ll k) {
    if (t[v].cnt < k) return -1;
    if (l == r-1) return l;
    ll m = (l+r)/2;
    if (t[2*v+1].cnt >= k) {
        return find_kth_zero(2*v+1, l, m, L, R, k);
    } else {
        return find_kth_zero(2*v+2, m, r, L, R, k-t[2*v+1].cnt);
    }
}
...
void init_tree(ll n, const vl& a) {
    ll size = 1;
...
void solve() {
    ...
    rep(q) {
        ...
        if (c == 's') {
            def(ll, l, r, k); l--;
            ll ans = find_kth_zero(0, 0, n, l, r, k);
            if (ans != -1) ans++;
            ps(ans);
        } else {
            def(ll, i, x); i--;
