...
void dfs(ll u, ll p) {
    ...
    // Update dists[u]
    each(v, sons) {
        dists[u].ckmax(dists[v].nth(0)+1);
    }

    // Case 1: Two disjoint subtrees with paths that go up and down
    // Definition: At most one path goes up to the implied root
    tmx<ll> updown, zero_one_sons;
    each(v, sons) {
        if (sz(dists[v].st) > 1) {
            updown.ckmax(dists[v].nth(0)+dists[v].nth(1));
        } else {
            zero_one_sons.ckmax(dists[v].nth(0)+1);
        }
    }

    if (sz(updown.st) > 1) {
        ckmax(ans, updown.nth(0)*updown.nth(1));
    }
    if (sz(updown.st) && sz(zero_one_sons.st)) {
        ckmax(ans, zero_one_sons.nth(0)*updown.nth(0));
    }

    // Case 2: One subtree in another, like braids
    // Definition: A path goes up and down thru the implied root
    tmx<pl> furthest_subtrees;
    each(v, sons) {
        furthest_subtrees.ckmax({ dists[v].nth(0), v });
    }

    each(v, sons) {
        ll cnt = 2;
        ll outer = 0;
        for (auto[dist_to_leaf, v1] : furthest_subtrees.st) {
            if (v1 != v) {
                cnt--;
                outer += dist_to_leaf+1;
            }
            if (!cnt) break;
        }
        if (cnt > 0) continue;

        ll inner = dists[v].nth(0);
        if (sz(dists[v].st) > 1) inner += dists[v].nth(1);
        ckmax(ans, outer*inner);
    }
}
