// two mistakes
// 1. Forgot r is an exclusive border and didn't decrement it first like i should
// 2. Forgot to respect the invariant and call update_block
...
void solve() {
    ...
    auto update_block = [&](int b) {
        mx[b] = -INF;
        for (int i = b*K; i < end_of_block(b); i++) {
            ckmax(mx[b], a[i]);
        }
    };
    auto add_seg = [&](int l, int r, int x) {
        int L = l/K, R = r/K;
        if (L == R) {
        ...
        L = l/K, R = r/K;
        for (int i = L; i < R; i++) buf[i] += x;
    };
    auto calc_max = [&](int l, int r) {
        int res = -INF;
        int L = l/K, R = r/K;
        if (L == R) {
            for (int i = l; i < r; i++) ckmax(res, a[i]+buf[L]);
            return res;
        } 

        if (l != L*K) {
            while (l < (L+1)*K) ckmax(res, a[l]+buf[L]), l++;
        }
        if (r != end_of_block(R)) {
            while (r > R*K) r--, ckmax(res, a[r]+buf[R]);
        }
        L = l/K, R = r/K;
        for (int i = L; i < R; i++) ckmax(res, mx[i]+buf[i]);
        return res;
    };

    // the invariant for blocks is that the value stored
    // must always reflect what is written in a
    for (int i = 0; i < blocks; i++) update_block(i);
