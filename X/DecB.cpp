...
void solve() {
    ...
    f0(i, T) {
        ...
        ll ans = BIG;
        auto dfs = yy([&](auto rec, ll u, ll d, ll f, ll cost) -> void {
            gg(u, d, f, cost);
            dp[u][d][f] = cost;
            ...
            // refuel
            if (dp[u][d][c[u]] > dp[u][d][f]+p[u]) rec(u, d, c[u], cost+p[u]);

            // visit neighbours
            for (auto[w, v] : g[u]) {
                if (f > 0 && cost < dp[v][d+w][f-1]) rec(v, d+w, f-1, cost);
            }
        });
