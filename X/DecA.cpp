...
bool solve() {
    ...
    if (count(all(b), 1) < 2*k) return false;

    // only want the first k and last k
    ll cnt = 0;
    f1(i, n) if (b[i]) {
        cnt++;
        a[i] = 1;
    ...
    // dont need multiset bc each one has a unique idx
    set<pl> ready, no;
    ll cur = 0;
    V<vl> ans(k+1);
    f1(i, n) {
        if (!a[i]) {
            if (sz(no)) {
                auto x = *bg(no);
                ((x.f+1)%k == k-1 ? ready : no).ins({ x.f+1, x.s });
                ans[x.s].pb(i);
                no.erase(x);
