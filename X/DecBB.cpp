...
void solve() {
    ...
    f0(i, T) {
        ...
        ll src, dol, dist; re(src, dol, dist);

        dp[src][0][0] = 0;
        auto dfs = yy([&](auto rec, ll u, ll d, ll f) -> ll {
            if (dp[u][d][f] < BIG) return dp[u][d][f];
            gg(u, d, f);

            // if (d+1 < N*N) dp[u][d][f] = rec(u, d+1, f);
            // if (f+1 < F) ckmin(dp[u][d][f], rec(u, d, f+1));
            if (f == c[u]) {
                ckmin(dp[u][d][f], rec(u, d, 0)+p[u]);
            }
            for (auto[w, v] : g[u]) {
                if (d>=w && f>0) ckmin(dp[u][d][f], rec(v, d-w, f-1));
                // if (f == 0) {
                    // ckmin(dp[u][d][f], rec(v, d-w, ))
                // }
            }
