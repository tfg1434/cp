...
void solve() {
    ...
    ll bad = n*(sz(ROW)+sz(COL)) - sz(ROW)*sz(COL);
    for (auto[cr, cc] : st) {
        ll alrR = 0; F0R(i, 3) alrR += ROW.count(cr*3+i);
        ll alrC = 0; F0R(i, 3) alrC += COL.count(cc*3+i);
        ll tot = 3*(alrR+alrC) - alrR*alrC;

        bad += 9-tot;
    }

    for (auto[r, c] : Q) {
        if (st.count({r/3, c/3})) continue;
        if (ROW.count(r) || COL.count(c)) continue;
        bad++;
    }
