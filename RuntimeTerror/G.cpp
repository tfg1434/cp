...
void solve() {
    ...
    auto in = [&](ll r, ll c) {
        return min(r,c) >= 1 && max(r,c) <= n;
    };

    V<str> ma(n+1); 
    F1R(i, n) re(ma[i]), ma[i] = ' '+ma[i];

    V<vl> pre(n+1, vl(n+1));
    F1R(i, n) F1R(j, n) pre[i][j] = pre[i-1][j]+pre[i][j-1]+(ma[i][j] == 'A')-pre[i-1][j-1];

    ll ans1 = 0;
    F1R(i, n) F1R(j, n) if (ma[i][j] == 'A'){
        ll k = 0;
        for (ll stp = 10; stp >= 0; stp--) {
            ll K = k+p2(stp);
            if (max(i,j)+K <= n && pre[i+K][j+K]-pre[i-1][j+K]-pre[i+K][j-1]+pre[i-1][j-1] == (K+1)*(K+1)) k = K;
        }
        ckmax(ans1, (k+1)*(k+1));
    }

    queue<pl> q;
    F1R(i, n) F1R(j, n) if (ma[i][j] != 'B') q.push({i, j});
    F1R(i, n) q.push({0, i}), q.push({i, 0}), q.push({n+1, i}), q.push({i, n+1});
    V<vl> vis(n+1, vl(n+1));
    auto dist = vis;
    ll cur = 0;
    while (sz(q)) {
        ...
        rep(m) {
            ...
            F0R(i, 4) {
                ...
                if (in(nr, nc) && !vis[nr][nc]) q.push({nr, nc});
            }
        }
        cur++;
    }
    ...
    ll mx = 0;
    F1R(i, n) F1R(j, n) if (ma[i][j] == 'B') ckmax(mx, dist[i][j]);

    ll ans2 = (2*mx-1)*(2*mx-1) - 2*mx*(mx-1);
    if (mx == 0) ans2 = 0;
    ps(ans1, ans2);
}
