...
ll len(vl& p) {
    ll res = 1;
    F0R(i, sz(p)-1) res += max(p[i], p[i+1]);
    return res;
}
...
void solve() {
    ...
    dp[0][0][0] = 1;
    F0R(i, n) R0F(j, n+1) F0R(d, L) {
        // new
        (dp[i+1][j+1][d+1] += dp[i][j][d]%P) %= P;
        // augment
        (dp[i+1][j][d+a[i+1]] += dp[i][j][d]*2*j%P) %= P;
        // new & merge
        if (j > 0) (dp[i+1][j-1][d+2*a[i+1]-1] += dp[i][j][d]*j%P*(j-1)%P) %= P;

        // // new
        // (dp[i+1][j+1][d+1] += dp[i][j][d]*(j+1)%P) %= P;
        // // augment
        // (dp[i+1][j][d+a[i+1]] += dp[i][j][d]*2*j%P) %= P;
        // // merge
        // if (i > 1 && j > 0) (dp[i][j-1][d+a[i]] += dp[i][j][d]) %= P;
    }
