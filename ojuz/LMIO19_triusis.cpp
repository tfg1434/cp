...
void upd(node*t, ll tl, ll tr, ll p, ll x) {
    if (tl == tr-1) {
        a[tl] = x;
        return;
    }
    ll tm = (tl+tr)/2;
    if (p <= tm) upd(t->l, tl, tm, p, x);
    else upd(t->r, tm+1, tr, p, x);
}
...
// we can solve this for moving down at most M too,
// we will use the same idea as LCS problem!
void solve() {
    ll n, m; re(n, m);
    ...
    // auto tr = MaxSegTree(N);
    // ll ans = 0;
    // FOR(i, 1, n+1) if (a[i] <= 0) {
        // ll x = tr.range_max(a[i], N)+1;
        // ckmax(ans, x);
        // tr.set(a[i], x);
        // gg(x);
    // }
    ...
    // node* tr = build(0, N);
    // F0R(i, n) {
        // ll mx = query(tr, 0, N, max(a[i]-m, 0ll), N);
        // upd(tr, 0, N, a[i], mx+1);
    // }

    // ll best = query(tr, 0, N, 0, N);
    // ps(n-best);
}
