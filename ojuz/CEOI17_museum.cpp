...
const ll N = 1e4+2;
ll dp[2][N][N]; // dp(0/1, i, j) = whether it returns, start, unique visited
...
void solve() {
    ...
    yy([&](auto rec, ll u, ll p) -> void {
        ...
        dp[0][u][1] = 0;
        for (auto[w, v]: g[u]) if (v != p) {
            auto cost = dp[1][v][sub[v]];
            FOR(i, 1, sub[u]+1) {
                FOR(j, 1, sub[v]+1) {
                    if (i+j > sub[u]) continue;
                    ckmin(dp[0][u][i+j], dp[1][u][i] + dp[0][v][j]+w);
                }

                if (i+sub[v] <= sub[u]) ckmin(dp[1][u][i+sub[v]], dp[1][u][i]+cost+2*w);
                if (i+sub[v] <= sub[u]) ckmin(dp[0][u][i+sub[v]], dp[0][u][i]+cost+2*w);
            }
        }
