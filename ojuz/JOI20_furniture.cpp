...
void solve() {
    ...
    FOR(i, 0, m+2) R[0][i] = R[n+1][i] = 1;
    FOR(i, 0, n+2) R[i][0] = R[i][m+1] = 1;
    auto in = [&](ll r, ll c) {
    ...
    auto dfs = yy([&](auto rec, ll r, ll c) -> void {
        ...
        for (ll dr : {-1, 0, 1}) for (ll dc : {-1, 0, 1}) if (abs(dr)+abs(dc) == 1) {
            ...
            if (in(nr, nc) && R[nr][nc] == 0) rec(nr, nc);
        }
    });

    auto upd = [&](ll r, ll c) -> bool {
        if (R[r][c] == 1) {
            return true;
            ...
            F1R(i, n) {
                if (i == r) continue;
                ll j = r+c-i;
                // if (j < 1) break; stupid bug
                if (in(i, j)) flag |= R[i][j] == 0;
            }
            ...
            for (ll dr : {-1, 0, 1}) for (ll dc : {-1, 0, 1}) if (abs(dr)+abs(dc) == 1) {
                ll nr = r+dr, nc = c+dc;
                if ((nr==1&&nc==1)||(nr==n&&nc==m)) continue;
                if (in(nr, nc) && R[nr][nc] == 0) dfs(nr, nc);
            }
            return true;
        }
    };
