...
void init() {
    ...
    inv[N-1] = pw(fact[N-1], P-2);
    R0F(i, N-1) inv[i] = inv[i+1]*(i+1)%P;
}
...
void solve() {
    ...
    rep(q) {
        ...
        if (t == 1) {
            def(ll, u1,v1,u2,v2);
            g[u1].erase(v1); g[v1].erase(u1);
            g[u2].ins(v2); g[v2].ins(u2);
        } else {
            def(ll, u);
            vl gaps;
            vl tmp;
            each(v, g[u]) tmp.pb(v < u ? v+n : v);
            sor(tmp);
            for (int i = 1; i < sz(tmp); i++) gaps.pb(tmp[i]-tmp[i-1]-1);
            if (sz(tmp)) gaps.pb(tmp[0]-u-2);
            if (sz(tmp)) gaps.pb(u+n-tmp.bk-2);

            ll sum = 0;
            each(x, gaps) sum += x;
            ll res = fact[sum];
            each(x, gaps) (res *= inv[x]) %= P; 
