...
// I missed the observation that we inc immediately after the last
// value where we choose to end the LIS
// That leads you to realise we can fix the last value, then
// we're able to calculate R by doing lwb on a[i]-X
// I also forgot to consider during the implementation the case with L only
// and no increment. (the X=0 case)
void solve() {
    ll n, X; re(n, X);
    vl a(n); re(a);
    vl pre(n), suf(n);
