...
// parent = dag = basically think of a tree with root on right
// dpChildren = think of a tree with root on left

ll dpB[MAXN];
ll dpF[MAXN];
// dfs(s)
// I want the root on left...
void dfs(ll i) {
    // if (dpB[i] < BIG) return;

    dpB[i] = min(dpB[i], distV[i]);
    for (int j : dpChildren[i]) {
        dpB[j] = min(dpB[j], dpB[i]);
        dfs(j);
    }
};

// This function could iterate through each adj list multiple times:
// For example, 
void dfs2(ll i) {
    // if (dpF[i] < BIG) return;

    dpF[i] = min(dpF[i], distV[i]);
    for (int j : parent[i]) {
        dpF[j] = min(dpF[j], dpF[i]);
        dfs2(j);
    }
};


// This function guarantees that we only iterate through each adjacency list once,
// so our time complexity will be O(M)
ll memo[MAXN];
ll dp(int u) {
...
signed main() {
    ...
    ll ans = distU[v];
    F0R(i, n) if (reachable[i]) {
        ans = min(ans, distU[i] + min(memo[i], memo2[i]));
    }
    cout << ans << "\n";
