...
ll n, m;
const ll N = 1e5+2, M = 2e5+2;
map<ll, ll> dp[N];
map<ll, ll> sumColor[N];
map<ll, V<Edge>> g[N];
...
void solve() {
    ...
    while (sz(q)) {
        ...
        if (c) {
            for (auto to : g[u][c]) {
                D = d+sumColor[u][c] - to.p;
                if (!dp[to.v].count(0) || D < dp[to.v][0]) {
                    dp[to.v][0] = D;
                    q.push({D, {to.v, 0}});
                }
            }
            ...
            for (auto [c, edges] : g[u]) {
                for (auto to : edges) {
                    // Case 1. We don't flip the edge uv
                    D = d+sumColor[u][c]-to.p;
                    if (!dp[to.v].count(0) || D < dp[to.v][0]) {
                        dp[to.v][0] = D;
                        q.push({D, {to.v, 0}});
                    ...
                    // Case 3. We make a covenant that you will visit c on the next move
                    D = d;
                    if (!dp[to.v].count(c) || D < dp[to.v][c]) {
                        dp[to.v][c] = D;
                        q.push({D, {to.v, c}});
                    }
                }
