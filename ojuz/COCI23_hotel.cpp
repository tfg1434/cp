...
void flood_fill() {
    while (sz(bfs)) {
        auto u = bfs.ft; bfs.pop();

        if (!dist[u]) continue;

        // We are going to push all nodes that are one step closer to the hotel
        // onto the queue. In this loop we push the vertices with known distances
        while (sz(v[dist[u]-1])) {
            ll x = v[dist[u]-1].bk;
            v[dist[u]-1].pop_back();
        ...
        // Push the nodes with unknown distances onto the queue
        for (auto v : g[u]) {
            if (dist[v] < dist[u]) {
                dist[v] = dist[u] - 1;
                source[v] |= source[u];
...
// By starting the bfs from the furthest node from the hotel, we only have to consider nodes
// with smaller distance. Why? There is no larger d[u] to correct you. And we only get valid
// hotels because it has to agree with all source nodes.
void solve() {
    re(n, m);
    F0R(i, m) {
        def(ll, u, v); --u; --v;
    ...
    // Start the BFS from the nodes with the maximum known dist
    // Source[u] is the set of all nodes that agree with this node
    for (auto u : v[j]) {
        bfs.push(u);
        dist[u] = j;
        vis[u] = 1;
    ...
    vl ans;
    F0R(i, n) if (ok && !dist[i] && source[i].count() == known) {
        ans.pb(i);
    }
    ps(sz(ans));
    for (auto x : ans) pr(x+1, ' ');
    ps();
}
