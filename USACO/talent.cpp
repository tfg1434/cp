...
template <class K, class V> using Map = tree<K, V, less<K>, 
    rb_tree_tag, tree_order_statistics_node_update>;

const ll N = 251;
map<ll, ok> dp[N]; //(talent, minweight)
const ll M = 1e3+5;

// missing 3 tcs? i thought it should run in time
// for some reason when i change the map<> to Map<> 
// Pbds it becomes super slow for some reason
void solve() {
    ll n, W; re(n, W);

    ll ans = 0;
    vpl a(n+1); f1(i, n) re(a[i]);
    f1(i, n) {
        auto [w,t] = a[i];
        if (w >= W) ckmax(ans, 1000*t/w);
    }
    ...
    f1(i, n) {
        ...
        for (auto [x, y] : dp[i-1]) {
            ...
            if (x+t < 2e6 && y+w < 2*M) {
                ...
                ckmin(dp[i][xx].val, yy);
                if (yy >= W) ckmax(ans, 1000*(xx)/(yy));
            }
        }
    }
