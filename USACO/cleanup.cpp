// It's possible to place down all keys iff it's possible to pick up all keys once they're placed down
...
void solve() {
    ...
    while (sz(q)) {
        ...
        each(v, g[u]) if (vis[v] && !VIS[v]) {
            if (has[c[v]] || f[v] == c[v]) q.push(v);
            else mp[c[v]].pb(v);
        }

        has[f[u]] = 1;
        each(v, mp[f[u]]) q.push(v);
        mp[f[u]].clear();
    }
