...
void solve() {
    ...
    vl vis(n+1), maxX(n+1), minX(n+1, BIG), maxY(n+1), minY(n+1, BIG);
    ll id = 1;
    auto dfs = yy([&](auto rec, ll u, ll p) -> void {
        vis[u] = id;
        ckmax(maxX[id], x[u]); ckmax(maxY[id], y[u]);
        ckmin(minX[id], x[u]); ckmin(minY[id], y[u]);
        each(v, g[u]) if (v != p && !vis[v]) {
            rec(v, u); 
        }
    });
    ...
    FOR(i, 1, id) {
        // gg(i, maxX[i], minX[i]);
        ckmin(ans, 2*(maxX[i] - minX[i] + maxY[i] - minY[i]));
    }
