...
"7 8\n";
    ...
    vl deg(n+1), DEG(n+1), HAS(n+1), par(n+1), PAR(n+1);
    V<vl> g(n+1), G(n+1);
    vl ISC(n+1);
    vpl ans;
    F0R(i, n-1) {
    ...
    F1R(i, n) if (HAS[i] && i != one) {
        unite(par[i], PAR[i]);
    }

    V<vl> bfs;
    queue<ll> q; q.push(one);
    ll lvl = 0;
    while (sz(q) ){
        ll m = sz(q);
        map<ll, vl> M;
    ...
    R0F(dep, lvl) {
        ...
        for (auto[cc, vec] : M) {
            ...
            each(u, vec) to.pb(u);
            sor(to);
            F0R(i, sz(to)) FOR(j, i+1, sz(to)) {
                unite(par[to[i]], par[to[j]]);
    ...
    F0R(dep, lvl) {
        ...
        for (auto[cc, vec] : M) {
            ...
            each(u, vec) to.pb(u);
            sor(to);
            F0R(i, sz(to)-1) ans.eb(to[i], to.bk);
        }
    }

    ps(sz(ans));
    for (auto[u, v] : ans) ps(u, v);
}
