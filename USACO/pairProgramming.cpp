...
ll ad(ll &a, ll b) { return a = (a + b) % P; }

ll sb(ll &a, ll b) { return a = (((a - b) % P) + P) % P; }

void solve() {
    ll n; re(n);
    string a, b; re(a, b); a = ' '+a; b = ' '+b;
    gg(a, b);

    V<vl> dp(n+1, vl(n+1)), zero(n+1, vl(n+1));
    zero[0][0] = 1;
    // f1(i, n) zero[i][0] = a[i] != '+';
    // f1(j, n) zero[0][j] = b[j] != '+';
    // f1(i, n) f1(j, n) {
    ...
    // f1(i, n) dp[i][0] = !zero[i][0];
    // f1(j, n) dp[0][j] = !zero[0][j];
    for (int i = 1; i <= n; i++) {
    ...
    // f1(i, n) f1(j, n) {
        ...
        // if (a[i] == '+') dp[i][j] += zero[i-1][j];
        // if (a[j] == '+') dp[i][j] += zero[i][j-1];

        // // subtract intersection
        // // what if a[i]=b[j]='0'??
        // if (a[i] == '+' && b[j] == '+' || 
            // a[i] == '1' && b[j]=='+' || a[i] == '+' && b[j] == '1') {
            // dp[i][j] -= dp[i-1][j-1]+zero[i-1][j-1];
        // } else if (isdigit(a[i]) && isdigit(b[j])) {
            // dp[i][j] -= dp[i-1][j-1];
        // }
    // }

    ps(ad(dp[n][n], zero[n][n]));
}
