...
const ll F = 2002;
ll dp[F][2], DP[F][2], suffdp[F][F];

void solve() {
    ll N, A, B; re(N, A, B);
    vl X(N+1), C(N+1), P(N+1);
    V<pair<ll, pl>> tmp(N+1);
    F1R(i, N) re(tmp[i].s.f, tmp[i].s.s, tmp[i].f); // sort by X[i]
    sor(tmp);
    F1R(i, N) P[i] = tmp[i].s.f, C[i] = tmp[i].s.s, X[i] = tmp[i].f;
    gg(P);

    // what is the maximum sum of popularity on the suff
    // that you can buy with only a dollars?
    R1F(i, N) {
        F0R(a, A+1) {
            ckmax(suffdp[i][a], suffdp[i+1][a]);
            // purchase the ith item
            if (a>=C[i]) ckmax(suffdp[i][a], suffdp[i+1][a-C[i]]+P[i]);
        }
    }
    gg(suffdp[1][3]);
    ...
    F0R(i, N) {
        FOR(b, 1, B+1) {
            ckmax(DP[b][1], dp[b][1]);
            if (X[i+1]*C[i+1] < b) ckmax(DP[b-X[i+1]*C[i+1]][1], dp[b][1]+P[i+1]);
            else {
                ll dol = C[i+1] - b/X[i+1]; // how much you pay
                // ckmax(DP[0][0], dp[b][1]+P[i+1]);
                ll nxt = dp[b][1]+P[i+1];
                // gg(i, nxt, suffdp[i+2][A-dol]);
                // gg(A, dol);
                if (A>dol && i+2<=N) {
                    if (ckmax(ans, nxt+suffdp[i+2][A-dol])) {
                        gg(nxt);
                        gg(suffdp[i+2][A-dol]);
                        gg(A, dol);
                    }
