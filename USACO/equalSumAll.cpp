...
template <class T> using Set = tree<T, null_type, less<T>, 
    rb_tree_tag, tree_order_statistics_node_update>;
// tcT> using Map = tree<T, null_type, less<T>, rb_tree_tag, tree_order_statistics_node_update>;
...
void solve() {
    ...
    vl pre(n+1); f1(i, n) pre[i] = pre[i-1]+a[i];

    Set<pair<ll, pl>> st;
    FOR(j, 1, n+1) FOR(k, j, n+1) {
        st.ins({ pre[k]-pre[j-1], {j, k} });
    }
    ...
    f1(i, n) {
        ...
        vl prv(2);
        for (auto [x, p] : st) {
            if (prv[!in(i, p)]) ckmin(ans, x-prv[!in(i, p)]);
            prv[in(i, p)] = x;
        }
