...
// Thinking
// I missed that we can realize that there are at most only two places a cow can come from... 
// I did realize there are at most 500 cows on each side so conventional
// range dp is perhaps possible
void solve() {
    ll N; cin >> N;
    vl pos;
    f0(i, N) {
    ...
    sor(pos);
    ll n = sz(pos);
    ll RO = lwb(pos, 0ll), LO = RO-1;

    V<V<vl>> dp(n, V<vl>(n, vl(2, BIG)));
    if (pos[0] < 0) dp[LO][LO][0] = dp[LO][LO][1] = n * -pos[LO];
    if (pos.bk > 0) dp[RO][RO][0] = dp[RO][RO][1] = n * pos[RO];
    FOR(len, 2, n+1) f0(l, n) {
        ll r = l + len - 1;
        if (r >= n) break;
        ll rem = n - (len-1);

        ckmin(dp[l][r][1], dp[l][r-1][1] + rem*(pos[r] - pos[r-1]));
        ckmin(dp[l][r][1], dp[l][r-1][0] + rem*(pos[r] - pos[l]));
        ckmin(dp[l][r][0], dp[l+1][r][0] + rem*(pos[l+1] - pos[l]));
        ckmin(dp[l][r][0], dp[l+1][r][1] + rem*(pos[r] - pos[l]));
    }

    ps(min(dp[0][n-1][0], dp[0][n-1][1]));
}
