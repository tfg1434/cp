...
void solve() {
    ...
    // vpl centres;
    V<vpl> centres(n/2+1);
    f0(i, n) f0(j, n) if (dw[i][j] >= ds[i][j]/d) {
        centres[dw[i][j]].pb({i,j});
        gg(i,j, dw[i][j]);
    }
    ...
    vpl v, s;
    ROF(i, 1, n/2+1) {
        each(x, centres[i]) v.pb(x);
        for (auto[r, c] : v) {
            ans[r][c] = 1; 
            f0(j, 4) {
                ll nr = r + dy[j], nc = c + dx[j];
                if (valid(nr, nc) && !ans[nr][nc] && a[nr][nc] != '#') {
                    s.pb({nr, nc});
                }
            }
        } 
