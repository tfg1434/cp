...
void solve() {
    ...
    ll well_connected = 0;
    while (sz(q)) {
        auto [d, ur] = q.top(); q.pop();
        auto [u, root] = ur;

        if (sz(visitors[u]) == k || visitors[u].count(root)) {
            continue;
        }
        visitors[u].ins(root);
        if (u >= c && sz(visitors[u]) == k) {
            well_connected++;
        }
