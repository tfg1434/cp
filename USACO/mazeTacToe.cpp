...
struct B{
    ...
    bool win() {
        ...
        return help(a[0]) || help(a[1]) || help(a[2])
            || help(t[0]) || help(t[1]) || help(t[2])
            || help(d1) || help(d2);
    }

    ll hash() {
        ll res = 0, p = binpow(3, 8);
        f0(i, 3) f0(j, 3) {
            res += a[i][j]*p;
            p /= 3;
        }
...
bool vis[26][26][19685];
bool mark[19685];

void solve() {
    ll n; re(n);
    V<V<M>> ma(n+1, V<M>(n+1));
    V<vb> wall(n+1, vb(n+1));
    ...
    ll ans = 0;
    auto dfs = yy([&](auto rec, ll r, ll c, B b)->void {
        vis[r][c][b.hash()] = 1;
        if (check(b.hash())) {
            mark[b.hash()] = 1;
            // ans++;
        ...
        f0(i, 4) {
            ll nr = r+dy[i], nc = c+dx[i];
            if (ok(nr, nc) && !wall[nr][nc]) {
                auto c = b;
    ...
    // auto dfs = yy([&](auto rec, ll r, ll c, V<pl>& adj, bool flag=true)->void {
        // vis[r][c] = 1;
        ...
        // f0(i, 4) {
            // ll nr = r+dy[i], nc = c+dx[i];
            // if (ok(nr, nc) && !vis[nr][nc] && !wall[nr][nc]) rec(nr, nc, adj, false);
        // }
    // });

    // V<V<vpl>> g(n+1, V<vpl>(n+1));
    // for (auto[r, c] : tmp) {
    ...
    // set<ll> st;
    // auto dfs2 = yy([&](auto rec, ll r, ll c, B b)->void {
        // vis[r][c] = 1;

        // auto [i,j,t] = ma[r][c]; 
        // // if it's not start cell
        // if (i != 0 || j != 0) {
            // i--; j--;
            // if (!b.a[i][j]) b.a[i][j] = t;
        // }
