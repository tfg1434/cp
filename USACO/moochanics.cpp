...
void solve() {
    ...
    auto dist = [&](ll i) {
        ll t = -1;
        if (i % 2 == 0) 
            t = max(1ll, 2*((p[nxt[i]]-p[i] + s[i]+s[nxt[i]]-1) / (s[i]+s[nxt[i]])) - 1);
        else 
            t = max(2ll, 2*((p[nxt[i]]-p[i] + s[i]+s[nxt[i]]-1) / (s[i]+s[nxt[i]])));
        return t;
    };

    F0R(i, n-1) {
        elem[i] = *st.ins({ dist(i), i }).f;
    }
    ...
    rep(n/2) {
        ...
        ans[pos] = ans[nxt[pos]] = t;

        if (prv[pos] >= 0) {
            st.erase(elem[prv[pos]]);
            elem[prv[pos]] = {-1, -1};
        }
        if (nxt[nxt[pos]] < n) {
            st.erase(elem[nxt[pos]]);
            elem[nxt[pos]] = {-1, -1};
        }

        // update prv and nxt...
        if (prv[pos] >= 0) nxt[prv[pos]] = nxt[nxt[pos]];
        if (nxt[nxt[pos]] < n) prv[nxt[nxt[pos]]] = prv[pos];

        if (prv[pos] >= 0 && nxt[prv[pos]] < n) {
            auto it = st.ins({dist(prv[pos]), prv[pos]}).f;
            elem[prv[pos]] = *it; 
        }
    }
