...
ll shash(ll i, ll j, const V<vl>& a) {
    vl v;
    F0R(k, 5) if (j & p2(k)) {
        v.pb(a[i][k]);
    }

    ll h = PolyHash(v).get_hash(0, sz(v)-1);
    // ps(v, h);
    return h;
}
...
S5 get_subset(vl &a, int x)
{
  S5 result = { 0, {0,0,0,0,0} };
  for (int j=0; j<5; j++)
    if ((1<<j)&x) result.v[result.n++] = a[j];
  return result;
}
...
void solve() {
    ...
    ll ans = 0;
    set<ll> vis;
    F0R(i, n) F0R(j, p2(5)) if (j > 0) {
        ll h = shash(i, j, a);

        // I made two mistakes in the implementation:
        // 1. If four other cows had this subset, I counted it as 4 
        //    instead of C(4,2)
        // 2. I got bitten by PolyHash if you try to do exclusive, then it
        //    always returns 0 
        ans -= (pct(j)&1?1:-1)*(m[h]-1);

        // Is equivalent
        // if (vis.count(h)) continue;
        // vis.ins(h);
        // ans -= (pct(j)&1?1:-1)*m[h]*(m[h]-1)/2;
    }
    ans /= 2;
