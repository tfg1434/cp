...
void solve() {
    string s; re(s); ll n = sz(s);
    vl a(n);
    map<ll, ll> mp; ll m = 0;
    vector<vl> adj(21, vl(21));
    f0(i, n) {
        if (!mp.count(s[i])) mp[s[i]] = m++;
        a[i] = mp[s[i]];
        if (i > 0) adj[a[i-1]][a[i]]++;
    }

    vl dp(1 << m);
    auto dfs = yy([&](auto rec, ll x) -> ll {
        if (x == 0) return 1;
        if (dp[x] != 0) return dp[x];

        ll res = BIG;
        f0(i, m) {
            if (x & (1 << i)) { // if cow i is last cow in cowphabet
                ll cost = rec(x ^ (1 << i));
                f0(j, m) if (x & (1 << j)) cost += adj[i][j];
                res = min(res, cost);
            }    
        }
