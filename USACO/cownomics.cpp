...
const ll N = 502;
str a[2][N];
ll merge(uint64_t x, ll y, ll z) {
    // V<uint64_t> v {x, (uint64_t)y, (uint64_t)z};
    // return PolyHash(v).get_hash(0, 2);
    return (base*base%mod*x%mod + base*y%mod + z%mod) % mod;
}
...
void solve() {
    ...
    F0R(i, n) re(a[0][i]);
    F0R(i, n) re(a[1][i]);
    F0R(i, n) h0[i] = PolyHash(a[0][i].c_str());
    F0R(i, n) h1[i] = PolyHash(a[1][i].c_str());
    ...
    auto check = [&](ll len) {
        ...
        F0R(j, m-len+1) {
            F0R(i, n) {
                auto hash = h0[i];
                ll h = merge(h0[i].get_hash(j, j+len-1), j, j+len-1);
                st.ins(h);
            }
        }
        ...
        F0R(j, m-len+1) {
            bool ok = true;
            F0R(i, n) {
                ll h = merge(h1[i].get_hash(j, j+len-1), j, j+len-1);
                ok &= !st.count(h);
            }
