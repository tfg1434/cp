...
// It makes sense to use bitmask dp here
// 3^n time complexity trick to iterate over 
...
ll work(ll mask) {
    vl v; f0(i, n) if (mask&(1<<i)) v.pb(i);
    vl nadj(sz(v));
    f0(i, sz(v)) f0(j, sz(v)) {
        if ((adj[v[i]] & (1<<v[j]))) nadj[i] |= (1<<j);
    }
