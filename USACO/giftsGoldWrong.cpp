...
void solve() {
    ll n; re(n);
    ll S = (1<<(n-1));
    V<vl> a(n, vl(n)); f0(i, n) f0(j, n) re(a[i][j]);
    V<vl> out(n+1), in(n+1);
    f0(i, n) {
        f0(j, n) {
            if (a[i][j] == i) break;
    ...
    f0(i, S) {
        ...
        ll x = 0;
        each(u, out[c]) each(v, in[c]) {
            x += cnt[m][u][v];
        }
        dp[i] = dp[m]+x;
        memcpy(cnt[i], cnt[m], sizeof cnt[i]);

        // Update cnt
        // Case 1a: c->j, j is in M
        each(j, M) each(v, out[c]) {
            cnt[i][c][j] += cnt[m][v][j];
        }
        // Case 1b: j->c, j is in M
        each(j, M) each(v, in[c]) {
            cnt[i][j][c] += cnt[m][j][v];
        }
        ...
        // Case 2: u->v, neither endpoint is c
        each(j, M) each(k, M) if (j != k) {
            cnt[i][j][k] += ci[j]*co[k];
        }
    }
