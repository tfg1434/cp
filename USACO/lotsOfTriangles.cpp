...
// dp[i][j] = [bs] = these bits are to the left of i,j
// dp[i][j] ^ dp[j][k] ^ do[k][i]
const ll N = 302;
using bs = bitset<N>;
bs dp[N][N];
...
void solve() {
    ...
    F0R(i, n) FOR(j, i+1, n) FOR(k, j+1, n) {
        ...
        bs mask = dp[t[0]][t[1]]&dp[t[1]][t[2]]&dp[t[2]][t[0]];
        sol[mask.count()]++;

        // ll cnt = 0;
        // for (auto p : a) {
            // if (cross(x, y, p) <= 0) continue;
            // if (cross(y, z, p) <= 0) continue;
            // if (cross(z, x, p) <= 0) continue;
            // cnt++;
        // }
        // sol[cnt]++;
    }
