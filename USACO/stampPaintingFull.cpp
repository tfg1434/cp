...
// The idea to get all tcs is to observe that we have many states and few 
// transitions. Perhaps we could make less states and more transitions, 
// then optimize the transitions with some sort of ds
//
// dp(i) is the number of ways to pick the numbers 1..i st no k consec #s
// are equal
void solve() {
    re(n, m, k);
    ...
    FOR(i,k, n+1) {
        // k = 1??
        // m = 1??
        ll x = (f[i-1]-f[max(0ll, i-k)]) % P;
        dp[i] = (m-1)*x%P;
        f[i] = f[i-1]+dp[i]%P;
        // gg(i, dp[i]);
    }

    ps(((binpow(m, n)-dp[n])+2*P)%P);
}
