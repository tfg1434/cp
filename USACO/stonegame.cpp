...
void solve() {
    ...
    sor(a);
    each(x, a) freq[x]++;
    FOR(i, 1, M) pre[i] = pre[i-1]+freq[i];

    bool lose = true;
    each(x, a) lose &= freq[x] % 2 == 0;
    if (lose) {
        ps(0);
        return;
    }
    ...
    F1R(t, a.bk) {
        ...
        ll cnt = 0;
        for (ll i = t; i < M; i += t) {
            ll hi = min(M, i+t); // exclusive ub
            v.pb(pre[hi-1]-pre[i-1]);
            cnt += v.bk&1;
            // if (i == t) {
                // ok &= pre[hi-1]-pre[i-1] & 1;
                // cont = pre[hi-1]-pre[i-1];
            // } 
            // else ok &= !(pre[hi-1]-pre[i-1] & 1);
        }
