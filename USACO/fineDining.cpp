...
// Thinking
// Multisource dijkstra <==> multisource bfs. intuitively it makes sense
// once you go to hay you HAVE TO go to N. we abuse that.
// How to do toposort with bfs. store indegree
//
// Implementation
// v.resize(a, val) only assigns val to newly allocated values
void solve() {
    ll n, m, k; re(n,m,k);
    V<vpl> g(n+1);
    f1(i, m) {
    ...
    while (!q.empty()) {
        ...
        for(auto[t, v]: g[u]) if (ckmin(dist[v], d+t)) {
            q.push({dist[v], v});
        }
    }
    ...
    while (!q.empty()) {
        ...
        for(auto[t, v]: g[u]) if (ckmin(ans[v], d+t)) {
            q.push({ans[v], v});
        }
    }
