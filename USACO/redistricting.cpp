...
void solve() {
    ll n; re(n);
    str s; re(s); s = ' '+s;
    ll h = count(all(s), 'H'), g = count(all(s), 'G');
    if (h > g) {
        ps(n);
        return;
    }
    ...
    vl f(n+1), dp(n+1), mp(2*n+1);
    F1R(i, n) {
        f[i] = f[i-1]+(s[i]=='H'?1:-1);
        dp[i] = dp[i-1];

        // ll j = mp[f[i]+n];
        // if (j > 0) ckmax(dp[i], dp[j-1]+get(i-(j-1), f[i]-f[j-1]));
        // ckmax(dp[i], dp[j]+get(i-j, f[i]-f[j]));
        // if (j+1 < i) ckmax(dp[i], dp[j+1]+get(i-j-1, f[i]-f[j+1]));
