...
// Digit DP. Why didn't I get it?
// Misunderstood the statement
// I was like trying to ignore cases that go over, that doesnt' matter,
// Just count it in the cmp=2 case of the dp
// subtlety cmp=012 vs greater
void solve() {
    vl po3{1};
    f0(i, MAXD) po3.pb(3*po3.bk);

    ll q; re(q);
    f0(_, q) {
        ll D, X, Y; re(D, X, Y);
        V<V<vl>> dp(3, V<vl>(3, vl(3)));
        dp[1][0][0] = 1;

        f0(i, 39) {
            V<V<vl>> DP(3, V<vl>(3, vl(3)));
            ll xi = X/po3[i]%3, yi = Y/po3[i]%3, di = D/po3[i]%3;
            f0(cmp, 3) f0(xc, 3) f0(yc, 3) f0(j, 3) {
                ll XI = (xi+xc+j)%3, XX = (xi+xc+j)/3;
                ll YI = (yi+yc+j)%3, YY = (yi+yc+j)/3;
                ll CMP = cmp;
                if (j > di) CMP = 2;
                if (j < di) CMP = 0;

                if (XI%2 == YI%2) {
                    DP[CMP][XX][YY] += dp[cmp][xc][yc];
                }
            }
