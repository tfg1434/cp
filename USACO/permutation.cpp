...
ftype side(point2d p1, point2d p2, point2d p) {
    return (p2.y - p1.y)*(p.x - p1.x) + (-p2.x + p1.x)*(p.y - p1.y);

}

//need to be in CCW order
bool in(point2d a, point2d b, point2d c, point2d p) {
    // auto v1 = b-a, v2 = c-b, v3 = a-c;
    return side(a, b, p) >= 0 && side(b, c, p) >= 0 && side(a, c, p) >= 0;
}

bool ccw(point2d a, point2d b) {
    point2d o{0, 0};
    bool ba = a < o, bb = b < o;
