...
struct State {
    ...
    bool operator==(State o) const {
        return mp(p1, p2) == mp(o.p1, o.p2) && mp(d1, d2) == mp(o.d1, o.d2);
    }
};
...
struct chash {
    ...
    size_t operator()(State s) const {
        // return 0;
        vl v = {s.p1.f, s.p1.s, s.p2.f, s.p2.s, s.d1, s.d2};
        auto h = PolyHash(v);
        return h.get_hash(0, sz(v)-1);
    }
};
...
void solve() {
    ...
    auto in = [&](pl p) {
        return min(p.f, p.s) >= 0 && max(p.f, p.s) < n;
    };
    auto fin = [&](pl p) {
        return p.f == n-1 && p.s == n-1;
    };
    auto mov = [&](pl p, ll d) {
        pl P = p;
        if (d == 0) P.s++;
    ...
    while (sz(q)) {
        ...
        F0R(i, m) {
            ...
            // R
            S = {p1, p2, (d1+1)%4, (d2+1)%4};
            if (!vis[S]) q.push(S);

            // L
            S = {p1, p2, (d1+3)%4, (d2+3)%4};
            if (!vis[S]) q.push(S);
        }
