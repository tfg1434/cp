...
ll sign(Point a, Point b, Point c) {
...
bool on(Point P, Point X, Point Y) {
    return ((X.x <= P.x && P.x <= Y.x) || (Y.x <= P.x && P.x <= X.x)) &&
           ((X.y <= P.y && P.y <= Y.y) || (Y.y <= P.y && P.y <= X.y));
}

bool xx(Point P, Point Q, Point X, Point Y) {
    vl a = { sign(P, X, Y), sign(Q, X, Y), sign(X, P, Q), sign(Y, P, Q) };
    if (all_of(all(a), [&](ll x) { return !x; })) {
        return on(P, X, Y) || on(Q, X, Y) || on(X, P, Q);
    }
    return (a[0] != a[1] && a[2] != a[3]);
}
bool xx(Seg pq, Seg xy) {
    return xx(pq.x, pq.y, xy.x, xy.y);
}
...
void solve() {
    ...
    f0(i, n) {
        ...
        ev.pb({ x1, 1, i });
        ev.pb({ x2, -1, i });
        a[i] = Seg{Point{x1, y1}, Point{x2, y2}, i};
    }
    sor(ev);
    ...
    for (auto [x, t, i] : ev) {
        ...
        if (t == 1) {
            st.insert(a[i]);
            auto it = st.lb(a[i]);
            if (next(it) != end(st) && xx(*it, *next(it))) {
                i1 = i; i2 = next(it)->id; break;
            }
            if (it != bg(st) && xx(*it, *prev(it))) {
                i1 = i; i2 = prev(it)->id; break;
            }
        } else {
            st.erase(a[i]);
            auto it = st.ub(a[i]);
            if (it != end(st) && it != bg(st) && xx(*it, *prev(it))) {
                i1 = i, i2 = prev(it)->id; break;
            }
        }
    }
