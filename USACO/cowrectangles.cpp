...
void solve() {
    ...
    each(y1, ys) each(y2, ys) if (y1 <= y2) {
        ll cur = 0, lastX = 0, prv = 0, dum = -1;
        V<Cow> v;
        for (auto[x, y, t] : cs) if (in(y, y1, y2)) {
            if (sz(v) && x != v.bk.x) {
                ll h = 0, g = 0;
                for (auto [X, Y, T] : v) (T == 'G' ? g : h)++;
                if (g) cur = 0;
                else {
                    if (cur == 0) prv = dum;
                    lastX = x;
                    cur += h;
                    if (cur == ans) {
                        ckmin(area, (y2-y1)*(lastX-prv));
                    } 
                    if (ckmax(ans, cur)) {
                        gg(ans, y1, y2, lastX, prv);
                        area = (y2-y1)*(lastX-prv);
                    }
                }
                v.clear();
            } 
