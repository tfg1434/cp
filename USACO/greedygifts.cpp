...
void solve() {
    ...
    // check the first j elements
    auto check = [&](ll j) {
        vl freq(n);
        for (int i = 0; i < j; i++) freq[a[i]]++;
        vl pre(n); partial_sum(all(freq), bg(pre));
        for (int i = 0; i < n; i++) if (pre[i] > i) {
            return true;
        }
    ...
    // the length of the smallest prefix
    // that will trigger a cycle
    ll idx = fstTrue(1ll, n, check);
