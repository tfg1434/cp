...
void solve() {
    ...
    //first i snakes, j changes, last change to sz[k].
    V<V<vl>> dp(n+1, V<vl>(K+1, vl(m, BIG)));
    V<vl> mn(n+1, vl(K+1, BIG));
    f0(k, m) if (b[k] >= a[1]) {
        dp[1][0][k] = b[k]-a[1];
        ckmin(mn[1][0], dp[1][0][k]);
    }

    FOR(i, 2, n+1) {
        f0(j, K+1) {
            f0(k, m) {
                //don't change net sizes
                if (b[k] >= a[i]) ckmin(dp[i][j][k], dp[i-1][j][k] + b[k] - a[i]);
                //change net sizes here
                if (j > 0 && b[k] >= a[i]) ckmin(dp[i][j][k], mn[i-1][j-1] + b[k] - a[i]);

                ckmin(mn[i][j], dp[i][j][k]);
            }
        }
    }
