...
ll C(ll n, ll k) {
    // gg(n, k);
    // gg(((fact[n] * inv[k])%P * inv[n-k]) % P);
    return ((fact[n] * inv[k])%P * inv[n-k]) % P;
}

void solve() {
    fact[0] = 1;
    FOR(i, 1, MAXA) fact[i] = (fact[i-1]*i) % P;
    inv[MAXA-1] = binpow(fact[MAXA-1], P-2);
    ROF(i, 0, MAXA-1) inv[i] = (inv[i+1]*(i+1)) % P;
    ...
    // assert(a%2 == 0 && b%2 == 0);
    // if (a == b) ps(1);
    // else if (a > b) ps(C(a/2, abs(a-b)/2));
    // else ps(C(a/2+abs(a-b)/2-1, abs(a-b)/2));
