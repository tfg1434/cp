...
void solve() {
    ...
    auto ask = yy([&](auto rec, ll tl, ll tr, ll c, ll lvl=0) -> vl {
        if (tl > r || tr < l) return vl();
        if (lvl == n) {
            vl ret {c};
            return ret;
        } 
        if (a[lvl].f != c) return rec(tl, tr, c, lvl+1);

        V<vl> tmp;
        vl to = a[lvl].s;
        ll cur = tl;
        F0R(i, sz(to)) {
            tmp.pb(rec(cur, cur+len[to[i]][lvl+1]-1, to[i], lvl+1));
            cur += len[to[i]][lvl+1];
            if (cur > BIG) break;
        }
