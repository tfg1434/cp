...
// be careful when you're looking at complexity, amortized or once
// i thought the inc would be n lg n per query but actually it's 
// n lg n amortized because the size of sets never increases (except)
// for x
void solve() {
    ll n; re(n);
    vl h(n); re(h);
    ...
    auto sweep = [&](ll i) {
        FOR(j, i+1, n) {
            if (!sz(a[i]) ||
                (h[*rbegin(a[i])]-h[i])*(j-i) <= (h[j]-h[i])*(*rbegin(a[i])-i)) {
                a[i].ins(j);
            }
        }
    };
    ...
    F0R(i, q) {
        ...
        F0R(j, x) {
            ll dy = h[x]-h[j];
            ll dx = x-j;
            auto l = prev(a[j].ub(x));
            // if the new slope is too small to add, continue
            if (dy*(*l-j) < dx*(h[*l]-h[j])) continue;

            auto p = a[j].ins(x);
            // perhaps delete some values coming after i
            auto it = next(p.f);
            while ((it=next(p.f)) != end(a[j]) && 
                dy*(*it-j) > ((h[*it]-h[j]) * dx)) a[j].erase(next(p.f));
        }
