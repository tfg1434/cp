...
ll x, y;
const ll N = 204;
ll f[N][N], tmp[N][N], F[N][N], pre[N][N], b[N][N], lo[N][N], hi[N][N], dp[N];
pair<pl,pl> where[N];

void solve() {
    ll n, k; re(n, k); 
    F0R(i, n) {
        ll x1, y1, x2, y2; re(x1, y1, x2, y2);
        x1++; y1++; x2++; y2++;
        ll x = min(x1, x2), y = min(y1, y2);
        ll X = max(x1, x2), Y = max(y1, y2);
        X--; Y--; // as we are given coordinates and we need area
        F[x][y] = 1;
        F[x][Y+1] = -1;
        F[X+1][y] = -1;
        F[X+1][Y+1] = 1;
    ...
    auto ask = [&](ll y1, ll y2, ll x) {
        return pre[x][y2]-pre[x][y1-1]-pre[x-1][y2]+pre[x-1][y1-1];
    };
    ...
    auto axis = [&]() {
        ...
        FOR(y1, 1, N) FOR(y2, y1, N) {
            ll prv = 0, mx = 0, last = 1, mxl = 0, mxr = 0;
            FOR(i, 1, N) {
                ll val = ask(y1, y2, i);
                if (prv+val <= 0) {
                    if (val <= 0) last = i+1;
        ...
        pair<pl ,pl> A, B;
        FOR(y1, 1, N) FOR(y2, y1, N) {
            if (ckmax(res, b[y1][y2]+dp[y1-1])) {
                A = where[y1-1], B = {{lo[y1][y2], hi[y1][y2]}, {y1,y2}};
            }
        }

        auto get = [&](pair<pl,pl> p) {
            ll ret = 0;
            auto [xs, ys] = p;
            FOR(i, xs.f, xs.s+1) FOR(j, ys.f, ys.s+1) {
                if (f[i][j] == -1) ret++;
            }
            return ret;
        };
        lost += get(A); lost += get(B);
    ...
    ll ans = axis();
    FOR(i, 1, N) FOR(j, 1, N) tmp[j][i] = f[i][j];
    swap(f, tmp);
    ckmax(ans, axis());
    ps(ans);
}
