...
void update(ll node, ll l, ll r, ll x, ll y, ap val) {
    // if (node == 1) ps(x, y, val.base+val.dif*(r-l), -val.dif);
...
void solve() {
    def(ll, n);
    vl b(n); re(b);
    vpl a(n); F0R(i, n) a[i] = {b[i], i};
    vl nse(n, BIG); // defined as the next smaller element
    {
        vl s;
        F0R(i, n) {
            while (sz(s) && a[i] < a[s.bk]) {
                nse[s.bk] = i;
                s.pop_back();
            }
            s.pb(i);
    ...
    vl pse(n, BIG); // defined as the previous smaller element
    {
        vl s;
        R0F(i, n)  {
            while (sz(s) && a[i] < a[s.bk]) {
    ...
    // we're only going to do the steps 1..n-1
    F0R(i, n) {
        ll mx, f;
        // defined as how many elements you can highlight
        // before it reaches a smaller element
        mx = min(n, nse[i]-i);
        ...
        if (mx == 1) {
            ...
            f = i-pse[i];
            if (f > 1) {
                update(1, 1, n-1, 1, f-1, {b[i], 0});
            } 
            continue;
        }

        update(1, 1, n-1, 1, mx-1, {b[i]*mx, -b[i]});
        if (mx < n) update(1, 1, n-1, mx, n-1, {mx*b[i], 0});
        
        // defined as the first step they will cut into you
        if (pse[i] != BIG) {
            f = i-pse[i];
            // apply to the growing ones
            update(1, 1, n-1, f, mx+f-1, {-b[i]*mx, b[i]});
            // apply to the not growing ones
            if (mx+f <= n-1) update(1, 1, n-1, mx+f, n-1, {-b[i]*mx, 0});
        }
    }

    F1R(i, n-1) ps(query(1, 1, n-1, i, i));
    ps(*min_element(all(b))*n);
}
