...
// I mostly got this problem except I didn't realize it's easier to 
// check contiguous segment in beginning
// and to check order using list instead of set (storing depth)
//
// The test data gives a lot of hints. If there's some sufficient conditions,
// look for it in the test data.
ll tc= 0;
bool solve() {
    tc++;
    ll n, m; re(n, m);
    ...
    vl done(n+1), par(n+1, -1);
    f1(i, m) {
        vl vis(n+1), stk;

        auto setP = [&](ll x, ll v) {
            if (par[x] != -1 && par[x] != v) return false;
            par[x] = v;
            return true;
        };
        ...
        f1(j, sz(a[i])-1) {
            ...
            vis[x]++;
            if (vis[x] == 1){
                if (!count(all(prv), x)) done[x]++;
                if (done[x] == 2) {
                    return false;
                }

                if (sz(stk)) {
                    if (!setP(x, stk.bk)) return false;
                } else {
                    if (!setP(x, 0))return false;
                }
        ...
        for (int i : cur) {
            while (ptr != ord.size() &&
                   !count(cur.begin(), cur.end(), ord[ptr]))
                ptr++;
            if (!count(ord.begin(), ord.end(), i))
                ord.insert(ord.begin() + ptr, i);
            else if (ptr == ord.size() || ord[ptr] != i) {
                return false;
            }
            ptr++;
        }
        ...
        // auto it = bg(cur);
        // // check it's consistent with order of ord
        // f0(j, sz(ord)) {
            // if (!count(all(cur), ord[j])) continue;

            // auto nt = find(it, end(cur), ord[j]);
            // if (nt == end(cur)) return false;

            // if (it != nt) it++;
            // while (it != nt) {
                // ord.ins(ord.begin()+j, *it);
                // it++;
            // }
        // }
