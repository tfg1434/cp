...
void solve() {
    ll t, n, k; re(t, n, k);
    // vl a(n+1), w(n+1); f1(i,n) re(a[i], w[i]);
    ...
    FOR(i, 1, sz(bp)) if (bp[i]-bp[i-1] & 1) {
        FOR(j, bp[i])
    }

    // V<I> b;
    // ll mx = a[n].s, mn = a[n].s;
    // ll cnt = 1;
    // ROF(i, 1, n) {

        // if (abs(a[i+1].f - a[i].f) > k) {
            // b.pb({ mn, mx, cnt });
            // mn = BIG, mx = 0;
        // } 
