...
// dp[i][j][k] = the minimum penalty if we want to take k more drops
// and there are no values on the segment i..j
// so the base cases are when k=0 and the penalty is 0
void solve() {
    ll n, m; re(n, m);
    vl a(n); re(a);
    sor(a);
    ...
    FOR(k, 0, n) F0R(i, n) FOR(j, i, n) {
        ...
        if (i-1 >= 0) ckmin(l, L[i-1][j][k-1] + k*(a[i]-a[i-1]));
        if (j+1 <  n) ckmin(l, R[i][j+1][k-1] + k*(a[j+1]-a[i]));
        if (i-1 >= 0) ckmin(r, L[i-1][j][k-1] + k*(a[j]-a[i-1]));
        if (j+1 <  n) ckmin(r, R[i][j+1][k-1] + k*(a[j+1]-a[j]));
        // if (k == n-1) gg(i, j, l, r);
    }
    ...
    gg(L[0][1][2], R[1][2][2]);
    ll ans = 0;
    // 0 <= k < n because we added the drop s
    F0R(i, n) {
        ckmax(ans, m*i-L[s][s][i]);
        ckmax(ans, m*i-R[s][s][i]);
    }
    ps(ans);
}
