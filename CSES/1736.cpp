...
class SegmentTree // range set value, range query for sum segtree, can be easily modified for other things
{
    ...
    public:
        ...
        void lz(ll node, ll L, ll R) {
            ...
            if(L != R) {
                ll x = SPLIT(lazy[node], R-L+1, M-L+1);
                lazy[node << 1] += x;
                lazy[node << 1|1] += lazy[node]-x;
            }
            lazy[node] = 0;
        }
        ...
        void update(ll node, ll L, ll R, ll Lq, ll Rq, ll val) {
            ...
            ll ml = max(L, Lq), mr = min(R, Rq);
            ll M = (L+R)/2;
            ll x = SPLIT(val, mr-ml+1, M-ml+1);

            update(node << 1, L, M, Lq, Rq, x);
            update(node << 1|1, M+1, R, Lq, Rq, val-x);
            segtree[node] = segtree[node << 1] + segtree[node << 1|1];
        }
...
void solve() {
    ...
    vl t(n+1); F1R(i, n) re(t[i]);
    SegmentTree tr; tr.init(n);
    tr.build(1, 1, n, t);
    F0R(i, q) {
        def(ll, T);
        def(ll, l, r);
        if (T == 1) {
            ll d = r-l+1;
            tr.update(1, 1, n, l, r, d*(d+1)/2);
        } else {
            ps(tr.query(1, 1, n, l, r));
        }
    }
}
