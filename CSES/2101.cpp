...
void solve() {
    ...
    for (ll i = 17; i >= 0; i--) {
        ...
        ll k = 0;
        for (ll j = 0; j < q; j++) {
            while (k < m && k < qs[j].ans + p2(i)) {
                // unite(connect[k].f, connect[k].s);
                unite(find(connect[k].f), find(connect[k].s));
                k++;
            }

            if (find(qs[j].u) != find(qs[j].v)) {
                qs[j].ans += p2(i);
            }
        }
    }
    ...
    vl sol(q);
    for (auto[u, v, idx, res] : qs) {
        sol[idx] = res >= m ? -1 : res+1;
        if (u == v) sol[idx] = 0;
    }
