...
void add(ll y, ll x) {
    y += ZERO;
    for (; y < N; y|=y+1) fen[y] += x;
}
ll ask(ll y) {
    y += ZERO;
    ll res = 0;
    for (; y >= 0; y=(y&(y+1))-1) res += fen[y];
    return res;
}
...
void solve() {
    ...
    F0R(i, n) {
        ...
        if (x1 == x2) {
            if (y1 > y2) swap(y1, y2);
            vert[x1+ZERO].pb({y1, y2});
        } else {
            assert(y1==y2);
            if (x1 > x2) swap(x1, x2);
            // set of active y-values (there can only be one segment for each y value)
            sta[x1+ZERO].pb(y1);
            fin[x2+ZERO].pb(y1);
        }
    }
    ...
    for (ll i = -1e6; i <= 1e6; i++) {
        ...
        for (auto[y1, y2] : vert[i+ZERO]) {
            assert(y2 >= y1);
            // ps(i, y2, y1, ask(y2), ask(y1-1));
            ans += ask(y2)-ask(y1-1);
        }
