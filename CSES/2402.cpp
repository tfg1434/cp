...
bool solve() {
    ...
    ll nxt = 1;
    f1(i, n) {
        // a[i] is min: can go in stack 1
        // all < a[i] need to be in different component than a[i]
        // since a[i] is greater but was inserted after
        ...
        if (a[i] < *bg(acv)) {
            ...
            if (sz(acv) >= 2) {
                auto it = lb(all(acv), a[i]);
                assert(it != bg(acv));
                for (auto x = next(bg(acv)); x != it; x++) {
                    if (find(*x) != find(*bg(a)) && ) {
                        return false;
                    }
