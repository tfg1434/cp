...
void solve() {
    ...
    f1(i, n) {
        f0(j, n+1) FOR(k, -x, x+1) {
            ...
            dp[j][k] += y;
            // Finish an open team
            if (j > 0 && k+a[i]<=X) dp[j-1][k+a[i]] += j*y;
            // Continue an open team
            dp[j][k] += j*y;
            // Create a new open team
            if (j < n && k-a[i]>=-X){
                dp[j+1][k-a[i]] += y;
            }
        }
        f0(j, n+1) FOR(k, -x, x+1) dp[j][k] %= P;
