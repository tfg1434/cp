...
bool solve() {
    ...
    f1(i, n) {
        gg(i, a[i]);
        if (a[i] == nxt) {
            // by definition of nxt
            // assert(a[A.bk] > a[i] && a[B.bk] > a[i]);
            ans[i] = 1;
            nxt++;

            while((!A.empty() && a[A.bk] == nxt) || (!B.empty() && a[B.bk] == nxt)) {
                if (!A.empty() && a[A.bk] == nxt) A.pop_back();
                else B.pop_back();
            ...
            // it doesn't matter which stack i put it in!
            // actually it does matter
            if (A.empty() || a[A.bk] > a[i]) A.pb(i), ans[i] = 1;
            else if (B.empty() || a[B.bk] > a[i]) B.pb(i), ans[i] = 2;
            else return false;
        }
    }
